...
local function EXMYTOOLTIP_GetAdjustedCompletionDate(completionDate, offsetHours)
    ...
    if C_DateAndTime and C_DateAndTime.AdjustTimeByMinutes then
        return C_DateAndTime.AdjustTimeByMinutes(normalizedDate, (offsetHours or 0) * 60)
    end
...
function EXMYTOOLTIP.UpdateTooltip(self)
    ...
    if intimeInfo then
        ...
        if completionDate then
            local adjustedDate = EXMYTOOLTIP_GetAdjustedCompletionDate(completionDate, offset)
            if adjustedDate then
                GameTooltip:AddLine(
                    string.format(L["完成日期: %02d/%02d/%02d %02d:%02d"], adjustedDate.year % 100,
                        adjustedDate.month, adjustedDate.monthDay, adjustedDate.hour, adjustedDate.minute), 1, 1, 1)
            end
        end
    else
