
NSRT = {
["AssignmentSettings"] = {
},
["CooldownList"] = {
},
["ProfileKeys"] = {
["Lonelylitten-Stormrage"] = "default",
["Zappyxatu-Stormrage"] = "default",
},
["PlayerStatsDisplay"] = {
["enabled"] = false,
["relativeTo"] = "CENTER",
["xOffset"] = 0,
["FrameStrata"] = "MEDIUM",
["yOffset"] = 250,
["Stats"] = {
["Avoidance"] = false,
["Speed"] = false,
["Leech"] = false,
["Vers"] = false,
["Haste"] = true,
["Crit"] = true,
["Mastery"] = true,
},
["CustomAnchorFrame"] = "UIParent",
["TextFont"] = "Expressway",
["FontSize"] = 14,
["Anchor"] = "CENTER",
["TextAlign"] = "CENTER",
["TextFontFlags"] = "OUTLINE",
["TextColor"] = {
1,
1,
1,
1,
},
},
["HasNewAlertStructure"] = true,
["PhaseTimings"] = {
},
["PaceComparison"] = {
["NewThreshold"] = {
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
["Bosses"] = {
[3421] = {
["enabled"] = false,
["thresholds"] = {
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 94,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 96.2,
["time"] = 10,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 86.2,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 91.9,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 80.7,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 88.5,
["time"] = 30,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 77.9,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 86.5,
["time"] = 40,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 76.5,
["time"] = 50,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 85.5,
["time"] = 50,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 73.9,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 83.5,
["time"] = 60,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 71.1,
["time"] = 70,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 81.7,
["time"] = 70,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 67.9,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 79.9,
["time"] = 80,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 65.5,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 78.2,
["time"] = 90,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 63.9,
["time"] = 100,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 76.9,
["time"] = 100,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 62.3,
["time"] = 110,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 75.6,
["time"] = 110,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 59.8,
["time"] = 120,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 73.8,
["time"] = 120,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 57.2,
["time"] = 130,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 71.5,
["time"] = 130,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 53.8,
["time"] = 140,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 70.4,
["time"] = 140,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 50.4,
["time"] = 150,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 70.3,
["time"] = 150,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 48.6,
["time"] = 160,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 69,
["time"] = 160,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 46.5,
["time"] = 170,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 66,
["time"] = 170,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 45,
["time"] = 180,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 53.5,
["time"] = 180,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 43.4,
["time"] = 190,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 61.2,
["time"] = 190,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 42.1,
["time"] = 200,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 59.5,
["time"] = 200,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 40.6,
["time"] = 210,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 57.2,
["time"] = 210,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 38.8,
["time"] = 220,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 54.8,
["time"] = 220,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 36.9,
["time"] = 230,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 52.3,
["time"] = 230,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 35.7,
["time"] = 240,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 50.6,
["time"] = 240,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 34.4,
["time"] = 250,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 48.8,
["time"] = 250,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 33.5,
["time"] = 260,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 47.3,
["time"] = 260,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 31.77,
["time"] = 270,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 45.3,
["time"] = 270,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 29.8,
["time"] = 280,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 43,
["time"] = 280,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 27.9,
["time"] = 290,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 40.8,
["time"] = 290,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 25.6,
["time"] = 300,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 40.7,
["time"] = 300,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 23.7,
["time"] = 310,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 40.2,
["time"] = 310,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 21.5,
["time"] = 320,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 37.3,
["time"] = 320,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 19.1,
["time"] = 330,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 34,
["time"] = 330,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 17,
["time"] = 340,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 30.5,
["time"] = 340,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 15.6,
["time"] = 350,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 27.9,
["time"] = 350,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 14.4,
["time"] = 360,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 26.4,
["time"] = 360,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 12.5,
["time"] = 370,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 23.9,
["time"] = 370,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 9.8,
["time"] = 380,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 20.5,
["time"] = 380,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 7.8,
["time"] = 390,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 16.6,
["time"] = 390,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 5.9,
["time"] = 400,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 12.9,
["time"] = 400,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 4.2,
["time"] = 410,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 10.3,
["time"] = 410,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 3.2,
["time"] = 420,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 8.9,
["time"] = 420,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 1.1,
["time"] = 430,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 5.4,
["time"] = 430,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 440,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 1.7,
["time"] = 440,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 446,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 446,
["unit"] = "boss2",
},
},
["userModified"] = false,
},
[3429] = {
["enabled"] = false,
["thresholds"] = {
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 92.5,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 79.5,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 72.1,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 66.6,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 63.4,
["time"] = 50,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 59,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 54.5,
["time"] = 70,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 49.5,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 45.4,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 41.3,
["time"] = 100,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 34.8,
["time"] = 110,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 30.8,
["time"] = 120,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 24.6,
["time"] = 130,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 17.7,
["time"] = 140,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 10.6,
["time"] = 150,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 5,
["time"] = 160,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 170,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 96,
["time"] = 10,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 89,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 82.2,
["time"] = 30,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 75.8,
["time"] = 40,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 70.3,
["time"] = 50,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 64.4,
["time"] = 60,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 59.2,
["time"] = 70,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 59.2,
["time"] = 80,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 49.5,
["time"] = 90,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 43.8,
["time"] = 100,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 39.9,
["time"] = 110,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 34.5,
["time"] = 120,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 30,
["time"] = 130,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 25.1,
["time"] = 140,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 20.3,
["time"] = 150,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 18,
["time"] = 155,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 18,
["time"] = 165,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 15.7,
["time"] = 170,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 10.3,
["time"] = 180,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 5.9,
["time"] = 190,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 3.3,
["time"] = 200,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 1.6,
["time"] = 210,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 0,
["time"] = 214,
["unit"] = "boss2",
},
{
["phase"] = 2.5,
["expected"] = 0,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 8,
["time"] = 5,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 11.6,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 13.1,
["time"] = 15,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 16.1,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 20,
["time"] = 25,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 24.7,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 30.7,
["time"] = 35,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 30.7,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 69.7,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 28.3,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 67.5,
["time"] = 10,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 26.5,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 64.7,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 25.1,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 62.2,
["time"] = 30,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 23.8,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 59.1,
["time"] = 40,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 22.5,
["time"] = 50,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 59,
["time"] = 50,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 21.5,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 56.1,
["time"] = 60,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 20.6,
["time"] = 70,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 52.9,
["time"] = 70,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 19,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 49.9,
["time"] = 80,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 17.1,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 46.3,
["time"] = 90,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 14.9,
["time"] = 100,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 42.4,
["time"] = 100,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 12.8,
["time"] = 110,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 39.2,
["time"] = 110,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 11.2,
["time"] = 120,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 36.8,
["time"] = 120,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 9.7,
["time"] = 130,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 34.4,
["time"] = 130,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 8.2,
["time"] = 140,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 31.6,
["time"] = 140,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 6.3,
["time"] = 150,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 28.6,
["time"] = 150,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 5.3,
["time"] = 160,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 23.5,
["time"] = 160,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 4.1,
["time"] = 170,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 18.5,
["time"] = 170,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 2.7,
["time"] = 180,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 12.2,
["time"] = 180,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 1.6,
["time"] = 190,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 8.1,
["time"] = 190,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 0.2,
["time"] = 200,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 4.4,
["time"] = 200,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 0,
["time"] = 205,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 1.6,
["time"] = 205,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 0,
["time"] = 208,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 0,
["time"] = 208,
["unit"] = "boss2",
},
},
["userModified"] = false,
},
[3497] = {
["enabled"] = false,
["thresholds"] = {
},
["userModified"] = false,
},
[3445] = {
["enabled"] = false,
["thresholds"] = {
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 88,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 89,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 79,
["time"] = 46,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 79,
["time"] = 46,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 79,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 79,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 74,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 73,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 70,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 70,
["time"] = 40,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 66,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 66,
["time"] = 60,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 59,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 59,
["time"] = 80,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 57,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 58,
["time"] = 90,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 57,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 58,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 52,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 53,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 47,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 48,
["time"] = 40,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 42,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 42,
["time"] = 60,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 37,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 39,
["time"] = 80,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 36,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 37,
["time"] = 90,
["unit"] = "boss2",
},
{
["phase"] = 4,
["expected"] = 36,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 4,
["expected"] = 37,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 4,
["expected"] = 30,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 4,
["expected"] = 31,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 4,
["expected"] = 23,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 4,
["expected"] = 26,
["time"] = 40,
["unit"] = "boss2",
},
{
["phase"] = 4,
["expected"] = 18,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 4,
["expected"] = 21,
["time"] = 60,
["unit"] = "boss2",
},
{
["phase"] = 4,
["expected"] = 13,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 4,
["expected"] = 14,
["time"] = 80,
["unit"] = "boss2",
},
{
["phase"] = 4,
["expected"] = 12,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 4,
["expected"] = 12,
["time"] = 90,
["unit"] = "boss2",
},
{
["phase"] = 5,
["expected"] = 12,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 5,
["expected"] = 12,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 5,
["expected"] = 8,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 5,
["expected"] = 7,
["time"] = 10,
["unit"] = "boss2",
},
{
["phase"] = 5,
["expected"] = 4,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 5,
["expected"] = 6,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 5,
["expected"] = 0,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 5,
["expected"] = 2,
["time"] = 30,
["unit"] = "boss2",
},
},
["userModified"] = false,
},
[3379] = {
["enabled"] = false,
["thresholds"] = {
},
["userModified"] = false,
},
[3492] = {
["enabled"] = false,
["thresholds"] = {
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 98.3,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 95.7,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 93.6,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 92.3,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 91.2,
["time"] = 50,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 89.3,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 87.8,
["time"] = 70,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 86.8,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 86.1,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 84.8,
["time"] = 100,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 83.4,
["time"] = 110,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 82.4,
["time"] = 120,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 81.5,
["time"] = 130,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 80.8,
["time"] = 140,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 80.4,
["time"] = 145.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 73.8,
["time"] = 155.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 66.1,
["time"] = 165.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 66.1,
["time"] = 284.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 65.3,
["time"] = 294.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 55.8,
["time"] = 304.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 44.5,
["time"] = 314.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 44,
["time"] = 319,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 44,
["time"] = 375,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 42.9,
["time"] = 385,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 41.3,
["time"] = 395,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 39.5,
["time"] = 405,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 38.1,
["time"] = 415,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 36.7,
["time"] = 425,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 35.3,
["time"] = 435,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 34.7,
["time"] = 445,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 33.2,
["time"] = 455,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 30.9,
["time"] = 465,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 28.8,
["time"] = 475,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 27.5,
["time"] = 485,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 26.6,
["time"] = 495,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 25.4,
["time"] = 505,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 23.8,
["time"] = 515,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 22.2,
["time"] = 525,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 20.4,
["time"] = 535,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 18.9,
["time"] = 545,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 18,
["time"] = 555,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 17.3,
["time"] = 565,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 15.6,
["time"] = 575,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 15.1,
["time"] = 583.6,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 8.7,
["time"] = 593.6,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 603.6,
["unit"] = "boss1",
},
},
["userModified"] = false,
},
[3455] = {
["enabled"] = false,
["thresholds"] = {
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 96,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 90.6,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 86.5,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 82.8,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 80.5,
["time"] = 50,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 78.7,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 76.6,
["time"] = 70,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 74.2,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 72.5,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 70.8,
["time"] = 100,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 68.8,
["time"] = 110,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 66.3,
["time"] = 120,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 64.2,
["time"] = 130,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 61.6,
["time"] = 140,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 59.4,
["time"] = 150,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 57,
["time"] = 160,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 55,
["time"] = 170,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 53.2,
["time"] = 180,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 51.3,
["time"] = 190,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 49,
["time"] = 200,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 46.6,
["time"] = 210,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 44.5,
["time"] = 220,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 42.7,
["time"] = 230,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 41.1,
["time"] = 240,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 38.8,
["time"] = 250,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 36.1,
["time"] = 260,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 34.4,
["time"] = 270,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 32.7,
["time"] = 280,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 29.6,
["time"] = 290,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 27.2,
["time"] = 300,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 25.1,
["time"] = 310,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 23.3,
["time"] = 320,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 21.2,
["time"] = 330,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 19.3,
["time"] = 340,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 17.6,
["time"] = 350,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 16.1,
["time"] = 360,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 14,
["time"] = 370,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 10.2,
["time"] = 380,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 6.3,
["time"] = 390,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 3.4,
["time"] = 400,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 0.9,
["time"] = 410,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 415,
["unit"] = "boss1",
},
},
["userModified"] = false,
},
[3420] = {
["enabled"] = false,
["thresholds"] = {
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 97,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 91,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 88,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 85,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 83,
["time"] = 50,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 80,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 78,
["time"] = 70,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 76,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 74,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 73,
["time"] = 100,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 69,
["time"] = 110,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 65,
["time"] = 120,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 62,
["time"] = 130,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 60,
["time"] = 140,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 58,
["time"] = 150,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 56,
["time"] = 160,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 54,
["time"] = 170,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 53.2,
["time"] = 180,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 51.4,
["time"] = 190,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 48.9,
["time"] = 200,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 47,
["time"] = 210,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 45.5,
["time"] = 220,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 43.6,
["time"] = 230,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 39.9,
["time"] = 240,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 36.2,
["time"] = 250,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 34.1,
["time"] = 260,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 32,
["time"] = 270,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 30.1,
["time"] = 280,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 27.8,
["time"] = 290,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 25,
["time"] = 300,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 24,
["time"] = 310,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 22,
["time"] = 320,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 20,
["time"] = 330,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 18,
["time"] = 340,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 16,
["time"] = 350,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 13.6,
["time"] = 360,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 8.4,
["time"] = 370,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 4.2,
["time"] = 380,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 1.1,
["time"] = 390,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 395,
["unit"] = "boss1",
},
},
["userModified"] = false,
},
[3470] = {
["enabled"] = false,
["thresholds"] = {
},
["userModified"] = false,
},
},
["Display"] = {
["relativeTo"] = "CENTER",
["xOffset"] = -400,
["FontFlags"] = "OUTLINE",
["FontSize"] = 28,
["yOffset"] = 400,
["RefreshInterval"] = 1,
["Font"] = "Expressway",
["LineSpacing"] = 4,
["AheadColor"] = {
0,
1,
0,
1,
},
["Anchor"] = "CENTER",
["BehindColor"] = {
1,
0.5,
0,
1,
},
["CloseBehindColor"] = {
1,
1,
0,
1,
},
["FarBehindColor"] = {
1,
0,
0,
1,
},
},
["SelectedBoss"] = 3379,
},
["StoredSharedReminder"] = "",
["NSUI"] = {
["scale"] = 1,
["reminders_frame"] = {
},
["timeline_window"] = {
["scale"] = 1,
["NSUITimelineWindow"] = {
["scale"] = 1,
},
},
["AutoComplete"] = {
["Addon"] = {
},
},
},
["SharedNotes"] = {
},
["Settings"] = {
["MissingRaidBuffs"] = true,
["CooldownThreshold"] = 20,
["Minimap"] = {
["showInCompartment"] = true,
["hide"] = false,
},
["GlobalFontFlags"] = "OUTLINE",
["GlobalEncounterFontSize"] = 20,
["AcceptNickNames"] = 4,
["Language"] = "enUS",
["TTS"] = true,
["Debug"] = false,
["GlobalFont"] = "Expressway",
["UnreadyOnCooldown"] = false,
["GenericDisplay"] = {
["Anchor"] = "CENTER",
["relativeTo"] = "CENTER",
["xOffset"] = -200,
["yOffset"] = 400,
},
["NickNamesSyncSend"] = 3,
["GlobalNickNames"] = false,
["TTSVolume"] = 50,
["CheckCooldowns"] = false,
["GlobalFontSize"] = 20,
["VersionCheckPresets"] = {
},
["NickNamesSyncAccept"] = 2,
["TTSOverlap"] = true,
["TTSVoice"] = 1,
["ShareNickNames"] = 4,
},
["CurrentProfile"] = "default",
["StoredPersonalReminder"] = {
},
["LastUsedConsumables"] = {
["Lonelylitten-Stormrage"] = {
["flask"] = 245931,
},
["Zappyxatu-Stormrage"] = {
["flask"] = 245929,
},
},
["ProfileVersion"] = 2,
["MainProfile"] = "default",
["AutoLoadNote"] = {
},
["ActivePersonalReminder"] = {
["Zappyxatu-Stormrage"] = {
[3429] = "Ele Altar",
[3492] = "Ele Ula",
},
},
["Reminders"] = {
},
["QoL"] = {
["AutoPromoteRankIndex"] = 1,
["AutoPromote"] = false,
["AutoAcceptGuildInvite"] = false,
["AutoRepair"] = false,
["AutoInviteGuildOnly"] = true,
["GatewayUseableDisplay"] = false,
["AutoPromoteOfficers"] = true,
["AutoInviteGuildRankIndex"] = 1,
["ResetBossDisplay"] = false,
["LootBossReminder"] = false,
["ConsumableNotificationDurationSeconds"] = 5,
["AutoInviteKeywords"] = "inv",
["TextDisplay"] = {
["relativeTo"] = "CENTER",
["yOffset"] = 0,
["Anchor"] = "CENTER",
["xOffset"] = 0,
["FontSize"] = 30,
["FontFlags"] = "OUTLINE",
},
["AutoPromoteNames"] = "",
["AutoInvite"] = false,
["IconDisplay"] = {
["Width"] = 40,
["relativeTo"] = "TOP",
["Scpaing"] = 5,
["GrowDirection"] = "DOWN",
["Height"] = 40,
["Anchor"] = "TOP",
["xOffset"] = 0,
["yOffset"] = -350,
},
["AddSpellIDToTooltips"] = false,
},
["AuraTrackingSettings"] = {
["Player"] = {
["NameYOffset"] = 4,
["InverseCooldownSwipe"] = true,
["DispelBorderSize"] = 3,
["DurationFontSize"] = 32,
["xOffset"] = -238,
["SpellIDPlayerFilter"] = "Disabled",
["NamePosition"] = "TOP",
["Unit"] = "player",
["GridSpacing"] = -1,
["BorderSwipeMode"] = "Static",
["Limit"] = 10,
["GrowDirection"] = "RIGHT",
["EnableCooldownSwipe"] = true,
["DurationAnchorPoint"] = "CENTER",
["TextFontFlags"] = "OUTLINE",
["DispelBorderMode"] = "ColoredWithIcon",
["TextFont"] = "Expressway",
["Zoom"] = 25,
["StackXOffset"] = -1,
["UnitType"] = "Automatic",
["NameEnabled"] = false,
["BorderSize"] = 1,
["ColorDurationThreshold"] = 3,
["Width"] = 100,
["GridGrowDirection"] = "UP",
["HideDurationText"] = false,
["TrackingMode"] = "SpellIDs",
["StackFontSize"] = 32,
["NameXOffset"] = 0,
["Spacing"] = -1,
["MultiTankGrow"] = "Same",
["enabled"] = false,
["SpellIDs"] = {
},
["StackYOffset"] = 1,
["ShowDecimalSeconds"] = false,
["DurationColor"] = {
1,
1,
0.25,
1,
},
["StackColor"] = {
1,
1,
1,
1,
},
["CustomAnchorFrame"] = "UIParent",
["relativeTo"] = "CENTER",
["SortMode"] = "AuraInstanceID",
["AuraFilters"] = {
},
["ShowWhitelistedPlayerBuffs"] = true,
["Anchor"] = "CENTER",
["BorderColor"] = {
0,
0,
0,
1,
},
["DurationXOffset"] = 0,
["FrameStrata"] = "MEDIUM",
["DecimalThreshold"] = 3,
["StackAnchorPoint"] = "BOTTOMRIGHT",
["SpellIDsEdited"] = false,
["DurationThresholdColor"] = {
1,
0,
0,
1,
},
["HideStackText"] = false,
["OnlyShowFirstTank"] = false,
["CandidateFilters"] = {
},
["ColorDurationUnderThreshold"] = false,
["builtin"] = "Player",
["loadConditions"] = {
["Classes"] = {
},
["SpecIDs"] = {
},
["Roles"] = {
},
["EncounterIDs"] = {
},
["Names"] = {
},
},
["NameFontSize"] = 30,
["AurasPerRowColumn"] = 20,
["Name"] = "Player Debuffs",
["yOffset"] = 314,
["Height"] = 100,
["DurationYOffset"] = 0,
["HideTooltip"] = false,
["HideLongDurationAuras"] = false,
},
["Custom"] = {
},
["UI"] = {
["StyleCopySource"] = "Player",
["Selected"] = "Player",
},
["External"] = {
["BorderSwipeMode"] = "Static",
["GrowDirection"] = "UP",
["DispelBorderSize"] = 3,
["DurationFontSize"] = 50,
["xOffset"] = 319,
["SpellIDPlayerFilter"] = "Disabled",
["NamePosition"] = "LEFT",
["Unit"] = "player",
["GridSpacing"] = -1,
["EnableCooldownSwipe"] = true,
["Limit"] = 10,
["InverseCooldownSwipe"] = true,
["DurationAnchorPoint"] = "CENTER",
["TextFont"] = "Expressway",
["TextFontFlags"] = "OUTLINE",
["DispelBorderMode"] = "ColoredWithIcon",
["ColorDurationUnderThreshold"] = false,
["Zoom"] = 25,
["StackXOffset"] = -1,
["UnitType"] = "Automatic",
["NameEnabled"] = true,
["BorderSize"] = 1,
["ColorDurationThreshold"] = 3,
["Width"] = 120,
["GridGrowDirection"] = "UP",
["HideDurationText"] = false,
["TrackingMode"] = "SpellIDs",
["StackFontSize"] = 50,
["NameXOffset"] = 0,
["Spacing"] = -1,
["MultiTankGrow"] = "Same",
["IncludeImmunities"] = true,
["SpellIDs"] = {
},
["StackYOffset"] = 1,
["OnlyShowFirstTank"] = false,
["ShowDecimalSeconds"] = false,
["StackColor"] = {
1,
1,
1,
1,
},
["DurationColor"] = {
1,
1,
0.25,
1,
},
["SortMode"] = "AuraInstanceID",
["relativeTo"] = "CENTER",
["DecimalThreshold"] = 3,
["ShowWhitelistedPlayerBuffs"] = false,
["DurationXOffset"] = 0,
["BorderColor"] = {
0,
0,
0,
1,
},
["Anchor"] = "CENTER",
["AuraFilters"] = {
},
["CustomAnchorFrame"] = "UIParent",
["StackAnchorPoint"] = "BOTTOMRIGHT",
["SpellIDsEdited"] = false,
["DurationThresholdColor"] = {
1,
0,
0,
1,
},
["FrameStrata"] = "MEDIUM",
["HideStackText"] = true,
["CandidateFilters"] = {
},
["enabled"] = false,
["builtin"] = "External",
["loadConditions"] = {
["Classes"] = {
},
["SpecIDs"] = {
},
["Roles"] = {
},
["EncounterIDs"] = {
},
["Names"] = {
},
},
["NameFontSize"] = 30,
["AurasPerRowColumn"] = 20,
["Name"] = "External & Immunity",
["yOffset"] = 152,
["Height"] = 120,
["DurationYOffset"] = 0,
["HideTooltip"] = true,
["NameYOffset"] = 0,
},
["Tank"] = {
["DispelBorderSize"] = 3,
["DurationFontSize"] = 32,
["xOffset"] = -242,
["SpellIDPlayerFilter"] = "Disabled",
["yOffset"] = -590,
["GridSpacing"] = -1,
["StackXOffset"] = -1,
["GrowDirection"] = "LEFT",
["enabled"] = false,
["Zoom"] = 25,
["NameEnabled"] = true,
["GridGrowDirection"] = "UP",
["CustomAnchorFrame"] = "UIParent",
["NameXOffset"] = 0,
["AuraFilters"] = {
},
["ShowDecimalSeconds"] = false,
["StackColor"] = {
1,
1,
1,
1,
},
["BorderColor"] = {
0,
0,
0,
1,
},
["Anchor"] = "CENTER",
["SpellIDsEdited"] = false,
["DurationThresholdColor"] = {
1,
0,
0,
1,
},
["builtin"] = "Tank",
["StackFontSize"] = 32,
["UnitType"] = "Automatic",
["Height"] = 100,
["MultiTankXOffset"] = 500,
["DurationYOffset"] = 0,
["DurationAnchorPoint"] = "CENTER",
["EnableCooldownSwipe"] = true,
["Limit"] = 10,
["InverseCooldownSwipe"] = true,
["TextFontFlags"] = "OUTLINE",
["DispelBorderMode"] = "ColoredWithIcon",
["HideStackText"] = false,
["FrameStrata"] = "MEDIUM",
["Width"] = 100,
["TrackingMode"] = "SpellIDs",
["DecimalThreshold"] = 3,
["DurationColor"] = {
1,
1,
0.25,
1,
},
["SortMode"] = "AuraInstanceID",
["OnlyShowFirstTank"] = false,
["Name"] = "Co-Tank Debuffs",
["StackYOffset"] = 1,
["ShowWhitelistedPlayerBuffs"] = false,
["SpellIDs"] = {
},
["ColorDurationUnderThreshold"] = false,
["DurationXOffset"] = 0,
["loadConditions"] = {
["Classes"] = {
},
["SpecIDs"] = {
},
["Roles"] = {
},
["EncounterIDs"] = {
},
["Names"] = {
},
},
["BorderSwipeMode"] = "Static",
["StackAnchorPoint"] = "BOTTOMRIGHT",
["TextFont"] = "Expressway",
["relativeTo"] = "CENTER",
["Spacing"] = -1,
["HideDurationText"] = false,
["CandidateFilters"] = {
},
["BorderSize"] = 1,
["MultiTankGrow"] = "RIGHT",
["ColorDurationThreshold"] = 3,
["NameFontSize"] = 30,
["AurasPerRowColumn"] = 20,
["Unit"] = "cotank",
["NamePosition"] = "TOP",
["NameYOffset"] = 4,
["HideLongDurationAuras"] = false,
["HideTooltip"] = false,
["MultiTankYOffset"] = 0,
},
["Groups"] = {
["Built-in"] = {
["collapsed"] = false,
},
},
},
["NickNames"] = {
},
["InviteList"] = {
},
["Profiles"] = {
["default"] = {
["CooldownList"] = {
},
["ReadyCheckSettings"] = {
["TierCheck"] = false,
["RepairCheck"] = false,
["SoulstoneCheck"] = false,
["ConsumablesDisplay"] = true,
["RaidBuffCheck"] = false,
["EnchantCheck"] = false,
["SkipGatewayKeybindCheck"] = false,
["SymbioticRelationshipCheck"] = false,
["CraftedCheck"] = false,
["SourceOfMagicCheck"] = false,
["GatewayShardCheck"] = false,
["MissingItemCheck"] = false,
["BlisteringScalesCheck"] = false,
["GemCheck"] = false,
["ItemLevelCheck"] = false,
},
["PlayerStatsDisplay"] = {
["TextFont"] = "Expressway",
["Stats"] = {
["Mastery"] = true,
["Speed"] = false,
["Leech"] = false,
["Avoidance"] = false,
["Haste"] = true,
["Crit"] = true,
["Vers"] = false,
},
["xOffset"] = 0,
["FrameStrata"] = "MEDIUM",
["yOffset"] = 250,
["relativeTo"] = "CENTER",
["CustomAnchorFrame"] = "UIParent",
["enabled"] = false,
["FontSize"] = 14,
["Anchor"] = "CENTER",
["TextAlign"] = "CENTER",
["TextFontFlags"] = "OUTLINE",
["TextColor"] = {
1,
1,
1,
1,
},
},
["AssignmentSettings"] = {
},
["PhaseTimings"] = {
},
["PaceComparison"] = {
["Bosses"] = {
[3421] = {
["thresholds"] = {
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 94,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 96.2,
["time"] = 10,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 86.2,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 91.9,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 80.7,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 88.5,
["time"] = 30,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 77.9,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 86.5,
["time"] = 40,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 76.5,
["time"] = 50,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 85.5,
["time"] = 50,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 73.9,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 83.5,
["time"] = 60,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 71.1,
["time"] = 70,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 81.7,
["time"] = 70,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 67.9,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 79.9,
["time"] = 80,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 65.5,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 78.2,
["time"] = 90,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 63.9,
["time"] = 100,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 76.9,
["time"] = 100,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 62.3,
["time"] = 110,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 75.6,
["time"] = 110,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 59.8,
["time"] = 120,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 73.8,
["time"] = 120,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 57.2,
["time"] = 130,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 71.5,
["time"] = 130,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 53.8,
["time"] = 140,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 70.4,
["time"] = 140,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 50.4,
["time"] = 150,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 70.3,
["time"] = 150,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 48.6,
["time"] = 160,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 69,
["time"] = 160,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 46.5,
["time"] = 170,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 66,
["time"] = 170,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 45,
["time"] = 180,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 53.5,
["time"] = 180,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 43.4,
["time"] = 190,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 61.2,
["time"] = 190,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 42.1,
["time"] = 200,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 59.5,
["time"] = 200,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 40.6,
["time"] = 210,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 57.2,
["time"] = 210,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 38.8,
["time"] = 220,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 54.8,
["time"] = 220,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 36.9,
["time"] = 230,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 52.3,
["time"] = 230,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 35.7,
["time"] = 240,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 50.6,
["time"] = 240,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 34.4,
["time"] = 250,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 48.8,
["time"] = 250,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 33.5,
["time"] = 260,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 47.3,
["time"] = 260,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 31.77,
["time"] = 270,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 45.3,
["time"] = 270,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 29.8,
["time"] = 280,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 43,
["time"] = 280,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 27.9,
["time"] = 290,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 40.8,
["time"] = 290,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 25.6,
["time"] = 300,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 40.7,
["time"] = 300,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 23.7,
["time"] = 310,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 40.2,
["time"] = 310,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 21.5,
["time"] = 320,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 37.3,
["time"] = 320,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 19.1,
["time"] = 330,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 34,
["time"] = 330,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 17,
["time"] = 340,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 30.5,
["time"] = 340,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 15.6,
["time"] = 350,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 27.9,
["time"] = 350,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 14.4,
["time"] = 360,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 26.4,
["time"] = 360,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 12.5,
["time"] = 370,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 23.9,
["time"] = 370,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 9.8,
["time"] = 380,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 20.5,
["time"] = 380,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 7.8,
["time"] = 390,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 16.6,
["time"] = 390,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 5.9,
["time"] = 400,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 12.9,
["time"] = 400,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 4.2,
["time"] = 410,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 10.3,
["time"] = 410,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 3.2,
["time"] = 420,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 8.9,
["time"] = 420,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 1.1,
["time"] = 430,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 5.4,
["time"] = 430,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 440,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 1.7,
["time"] = 440,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 446,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 446,
["unit"] = "boss2",
},
},
["enabled"] = false,
["userModified"] = false,
},
[3429] = {
["thresholds"] = {
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 92.5,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 79.5,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 72.1,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 66.6,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 63.4,
["time"] = 50,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 59,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 54.5,
["time"] = 70,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 49.5,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 45.4,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 41.3,
["time"] = 100,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 34.8,
["time"] = 110,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 30.8,
["time"] = 120,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 24.6,
["time"] = 130,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 17.7,
["time"] = 140,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 10.6,
["time"] = 150,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 5,
["time"] = 160,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 170,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 96,
["time"] = 10,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 89,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 82.2,
["time"] = 30,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 75.8,
["time"] = 40,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 70.3,
["time"] = 50,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 64.4,
["time"] = 60,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 59.2,
["time"] = 70,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 59.2,
["time"] = 80,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 49.5,
["time"] = 90,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 43.8,
["time"] = 100,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 39.9,
["time"] = 110,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 34.5,
["time"] = 120,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 30,
["time"] = 130,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 25.1,
["time"] = 140,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 20.3,
["time"] = 150,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 18,
["time"] = 155,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 18,
["time"] = 165,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 15.7,
["time"] = 170,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 10.3,
["time"] = 180,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 5.9,
["time"] = 190,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 3.3,
["time"] = 200,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 1.6,
["time"] = 210,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 0,
["time"] = 214,
["unit"] = "boss2",
},
{
["phase"] = 2.5,
["expected"] = 0,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 8,
["time"] = 5,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 11.6,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 13.1,
["time"] = 15,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 16.1,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 20,
["time"] = 25,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 24.7,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 2.5,
["expected"] = 30.7,
["time"] = 35,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 30.7,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 69.7,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 28.3,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 67.5,
["time"] = 10,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 26.5,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 64.7,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 25.1,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 62.2,
["time"] = 30,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 23.8,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 59.1,
["time"] = 40,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 22.5,
["time"] = 50,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 59,
["time"] = 50,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 21.5,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 56.1,
["time"] = 60,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 20.6,
["time"] = 70,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 52.9,
["time"] = 70,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 19,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 49.9,
["time"] = 80,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 17.1,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 46.3,
["time"] = 90,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 14.9,
["time"] = 100,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 42.4,
["time"] = 100,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 12.8,
["time"] = 110,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 39.2,
["time"] = 110,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 11.2,
["time"] = 120,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 36.8,
["time"] = 120,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 9.7,
["time"] = 130,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 34.4,
["time"] = 130,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 8.2,
["time"] = 140,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 31.6,
["time"] = 140,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 6.3,
["time"] = 150,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 28.6,
["time"] = 150,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 5.3,
["time"] = 160,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 23.5,
["time"] = 160,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 4.1,
["time"] = 170,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 18.5,
["time"] = 170,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 2.7,
["time"] = 180,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 12.2,
["time"] = 180,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 1.6,
["time"] = 190,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 8.1,
["time"] = 190,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 0.2,
["time"] = 200,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 4.4,
["time"] = 200,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 0,
["time"] = 205,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 1.6,
["time"] = 205,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 0,
["time"] = 208,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 0,
["time"] = 208,
["unit"] = "boss2",
},
},
["enabled"] = false,
["userModified"] = false,
},
[3497] = {
["thresholds"] = {
},
["enabled"] = false,
["userModified"] = false,
},
[3445] = {
["thresholds"] = {
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 88,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 89,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 1,
["expected"] = 79,
["time"] = 46,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 79,
["time"] = 46,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 79,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 79,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 74,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 73,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 70,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 70,
["time"] = 40,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 66,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 66,
["time"] = 60,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 59,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 59,
["time"] = 80,
["unit"] = "boss2",
},
{
["phase"] = 2,
["expected"] = 57,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 2,
["expected"] = 58,
["time"] = 90,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 57,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 58,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 52,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 53,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 47,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 48,
["time"] = 40,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 42,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 42,
["time"] = 60,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 37,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 39,
["time"] = 80,
["unit"] = "boss2",
},
{
["phase"] = 3,
["expected"] = 36,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 3,
["expected"] = 37,
["time"] = 90,
["unit"] = "boss2",
},
{
["phase"] = 4,
["expected"] = 36,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 4,
["expected"] = 37,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 4,
["expected"] = 30,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 4,
["expected"] = 31,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 4,
["expected"] = 23,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 4,
["expected"] = 26,
["time"] = 40,
["unit"] = "boss2",
},
{
["phase"] = 4,
["expected"] = 18,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 4,
["expected"] = 21,
["time"] = 60,
["unit"] = "boss2",
},
{
["phase"] = 4,
["expected"] = 13,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 4,
["expected"] = 14,
["time"] = 80,
["unit"] = "boss2",
},
{
["phase"] = 4,
["expected"] = 12,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 4,
["expected"] = 12,
["time"] = 90,
["unit"] = "boss2",
},
{
["phase"] = 5,
["expected"] = 12,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 5,
["expected"] = 12,
["time"] = 0,
["unit"] = "boss2",
},
{
["phase"] = 5,
["expected"] = 8,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 5,
["expected"] = 7,
["time"] = 10,
["unit"] = "boss2",
},
{
["phase"] = 5,
["expected"] = 4,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 5,
["expected"] = 6,
["time"] = 20,
["unit"] = "boss2",
},
{
["phase"] = 5,
["expected"] = 0,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 5,
["expected"] = 2,
["time"] = 30,
["unit"] = "boss2",
},
},
["enabled"] = false,
["userModified"] = false,
},
[3379] = {
["thresholds"] = {
},
["enabled"] = false,
["userModified"] = false,
},
[3492] = {
["thresholds"] = {
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 98.3,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 95.7,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 93.6,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 92.3,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 91.2,
["time"] = 50,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 89.3,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 87.8,
["time"] = 70,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 86.8,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 86.1,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 84.8,
["time"] = 100,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 83.4,
["time"] = 110,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 82.4,
["time"] = 120,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 81.5,
["time"] = 130,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 80.8,
["time"] = 140,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 80.4,
["time"] = 145.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 73.8,
["time"] = 155.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 66.1,
["time"] = 165.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 66.1,
["time"] = 284.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 65.3,
["time"] = 294.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 55.8,
["time"] = 304.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 44.5,
["time"] = 314.5,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 44,
["time"] = 319,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 44,
["time"] = 375,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 42.9,
["time"] = 385,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 41.3,
["time"] = 395,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 39.5,
["time"] = 405,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 38.1,
["time"] = 415,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 36.7,
["time"] = 425,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 35.3,
["time"] = 435,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 34.7,
["time"] = 445,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 33.2,
["time"] = 455,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 30.9,
["time"] = 465,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 28.8,
["time"] = 475,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 27.5,
["time"] = 485,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 26.6,
["time"] = 495,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 25.4,
["time"] = 505,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 23.8,
["time"] = 515,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 22.2,
["time"] = 525,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 20.4,
["time"] = 535,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 18.9,
["time"] = 545,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 18,
["time"] = 555,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 17.3,
["time"] = 565,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 15.6,
["time"] = 575,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 15.1,
["time"] = 583.6,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 8.7,
["time"] = 593.6,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 603.6,
["unit"] = "boss1",
},
},
["enabled"] = false,
["userModified"] = false,
},
[3455] = {
["thresholds"] = {
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 96,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 90.6,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 86.5,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 82.8,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 80.5,
["time"] = 50,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 78.7,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 76.6,
["time"] = 70,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 74.2,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 72.5,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 70.8,
["time"] = 100,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 68.8,
["time"] = 110,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 66.3,
["time"] = 120,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 64.2,
["time"] = 130,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 61.6,
["time"] = 140,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 59.4,
["time"] = 150,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 57,
["time"] = 160,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 55,
["time"] = 170,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 53.2,
["time"] = 180,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 51.3,
["time"] = 190,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 49,
["time"] = 200,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 46.6,
["time"] = 210,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 44.5,
["time"] = 220,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 42.7,
["time"] = 230,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 41.1,
["time"] = 240,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 38.8,
["time"] = 250,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 36.1,
["time"] = 260,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 34.4,
["time"] = 270,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 32.7,
["time"] = 280,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 29.6,
["time"] = 290,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 27.2,
["time"] = 300,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 25.1,
["time"] = 310,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 23.3,
["time"] = 320,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 21.2,
["time"] = 330,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 19.3,
["time"] = 340,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 17.6,
["time"] = 350,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 16.1,
["time"] = 360,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 14,
["time"] = 370,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 10.2,
["time"] = 380,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 6.3,
["time"] = 390,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 3.4,
["time"] = 400,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 0.9,
["time"] = 410,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 415,
["unit"] = "boss1",
},
},
["enabled"] = false,
["userModified"] = false,
},
[3420] = {
["thresholds"] = {
{
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 97,
["time"] = 10,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 91,
["time"] = 20,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 88,
["time"] = 30,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 85,
["time"] = 40,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 83,
["time"] = 50,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 80,
["time"] = 60,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 78,
["time"] = 70,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 76,
["time"] = 80,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 74,
["time"] = 90,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 73,
["time"] = 100,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 69,
["time"] = 110,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 65,
["time"] = 120,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 62,
["time"] = 130,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 60,
["time"] = 140,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 58,
["time"] = 150,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 56,
["time"] = 160,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 54,
["time"] = 170,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 53.2,
["time"] = 180,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 51.4,
["time"] = 190,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 48.9,
["time"] = 200,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 47,
["time"] = 210,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 45.5,
["time"] = 220,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 43.6,
["time"] = 230,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 39.9,
["time"] = 240,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 36.2,
["time"] = 250,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 34.1,
["time"] = 260,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 32,
["time"] = 270,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 30.1,
["time"] = 280,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 27.8,
["time"] = 290,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 25,
["time"] = 300,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 24,
["time"] = 310,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 22,
["time"] = 320,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 20,
["time"] = 330,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 18,
["time"] = 340,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 16,
["time"] = 350,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 13.6,
["time"] = 360,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 8.4,
["time"] = 370,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 4.2,
["time"] = 380,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 1.1,
["time"] = 390,
["unit"] = "boss1",
},
{
["phase"] = 1,
["expected"] = 0,
["time"] = 395,
["unit"] = "boss1",
},
},
["enabled"] = false,
["userModified"] = false,
},
[3470] = {
["thresholds"] = {
},
["enabled"] = false,
["userModified"] = false,
},
},
["NewThreshold"] = {
["phase"] = 1,
["expected"] = 100,
["time"] = 0,
["unit"] = "boss1",
},
["Display"] = {
["relativeTo"] = "CENTER",
["xOffset"] = -400,
["FarBehindColor"] = {
1,
0,
0,
1,
},
["CloseBehindColor"] = {
1,
1,
0,
1,
},
["yOffset"] = 400,
["BehindColor"] = {
1,
0.5,
0,
1,
},
["Font"] = "Expressway",
["LineSpacing"] = 4,
["AheadColor"] = {
0,
1,
0,
1,
},
["Anchor"] = "CENTER",
["RefreshInterval"] = 1,
["FontSize"] = 28,
["FontFlags"] = "OUTLINE",
},
["SelectedBoss"] = 3379,
},
["NSUI"] = {
["scale"] = 1,
["reminders_frame"] = {
},
["timeline_window"] = {
["scale"] = 1,
["NSUITimelineWindow"] = {
["scale"] = 1,
},
},
["AutoComplete"] = {
["Addon"] = {
},
},
},
["SharedNotes"] = {
},
["Settings"] = {
["MissingRaidBuffs"] = true,
["CooldownThreshold"] = 20,
["Minimap"] = {
["hide"] = false,
["showInCompartment"] = true,
},
["GlobalFontFlags"] = "OUTLINE",
["GlobalEncounterFontSize"] = 20,
["AcceptNickNames"] = 4,
["Language"] = "enUS",
["CheckCooldowns"] = false,
["Debug"] = false,
["GlobalFont"] = "Expressway",
["ShareNickNames"] = 4,
["NickNamesSyncSend"] = 3,
["TTSVoice"] = 1,
["GlobalNickNames"] = false,
["TTSVolume"] = 50,
["NickNamesSyncAccept"] = 2,
["GlobalFontSize"] = 20,
["VersionCheckPresets"] = {
},
["TTS"] = true,
["TTSOverlap"] = true,
["GenericDisplay"] = {
["Anchor"] = "CENTER",
["relativeTo"] = "CENTER",
["xOffset"] = -200,
["yOffset"] = 400,
},
["UnreadyOnCooldown"] = false,
},
["StoredPersonalReminder"] = {
},
["StoredSharedReminder"] = "",
["AutoLoadNote"] = {
},
["ActivePersonalReminder"] = {
["Zappyxatu-Stormrage"] = {
[3429] = "Ele Altar",
[3492] = "Ele Ula",
},
},
["QoL"] = {
["AutoPromoteRankIndex"] = 1,
["AutoInvite"] = false,
["AutoAcceptGuildInvite"] = false,
["AutoRepair"] = false,
["AutoInviteGuildOnly"] = true,
["GatewayUseableDisplay"] = false,
["AutoPromoteOfficers"] = true,
["AutoInviteGuildRankIndex"] = 1,
["ResetBossDisplay"] = false,
["LootBossReminder"] = false,
["ConsumableNotificationDurationSeconds"] = 5,
["AutoInviteKeywords"] = "inv",
["TextDisplay"] = {
["relativeTo"] = "CENTER",
["FontFlags"] = "OUTLINE",
["xOffset"] = 0,
["FontSize"] = 30,
["Anchor"] = "CENTER",
["yOffset"] = 0,
},
["AutoPromoteNames"] = "",
["AutoPromote"] = false,
["IconDisplay"] = {
["yOffset"] = -350,
["relativeTo"] = "TOP",
["Scpaing"] = 5,
["GrowDirection"] = "DOWN",
["Height"] = 40,
["xOffset"] = 0,
["Anchor"] = "TOP",
["Width"] = 40,
},
["AddSpellIDToTooltips"] = false,
},
["InterruptSettings"] = {
["xOffset"] = -600,
["InterruptDefaultTextColor"] = {
1,
1,
1,
1,
},
["NumberAnchor"] = "CENTER",
["yOffset"] = 400,
["NumberFontSize"] = 60,
["InterruptDefaultColor"] = {
1,
0,
0,
1,
},
["NumberRelativeTo"] = "CENTER",
["NameyOffset"] = 10,
["Anchor"] = "CENTER",
["NameRelativeTo"] = "TOP",
["InterruptNowColor"] = {
0,
1,
0,
1,
},
["InterruptNextColor"] = {
1,
1,
0,
1,
},
["NumberxOffset"] = 0,
["relativeTo"] = "CENTER",
["NameAnchor"] = "BOTTOM",
["NamexOffset"] = 0,
["NumberFontFlags"] = "OUTLINE",
["NumberyOffset"] = 0,
["InterruptNowTextColor"] = {
1,
0,
0,
1,
},
["Width"] = 100,
["NameFontFlags"] = "OUTLINE",
["InterruptNextTextColor"] = {
1,
0,
0,
1,
},
["NameFontSize"] = 30,
["InterruptSound"] = "|cFF4BAAC8Interrupt|r",
["Height"] = 100,
["NameFont"] = "Expressway",
["ShowBar"] = false,
["NumberFont"] = "Expressway",
},
["InviteList"] = {
},
["ProfileVersion"] = 2,
["Alerts"] = {
["Language"] = "enUS",
["Groups"] = {
["3455|Vashnik"] = {
["collapsed"] = false,
},
},
["ReloeReminders"] = false,
},
["PersonalReminders"] = {
["Ele Altar"] = "EncounterID:3429;Name:Ele Altar;Difficulty:Heroic\nph:1;time:0.6;tag:Zappyxatu;spellid:114050;\nph:2.5;time:14.7;tag:Zappyxatu;spellid:114050;",
["Ele Ula"] = "EncounterID:3492;Name:Ele Ula;Difficulty:Heroic\nph:1;time:5.0;tag:Zappyxatu;spellid:191634;\nph:1;time:7.7;tag:Zappyxatu;spellid:114050;\nph:1;time:43.9;tag:Zappyxatu;spellid:191634;\nph:1;time:131.6;tag:Zappyxatu;spellid:191634;\nph:1;time:136.4;tag:Zappyxatu;spellid:114050;\nph:1;time:178.0;tag:Zappyxatu;spellid:191634;\nph:1;time:235.4;tag:Zappyxatu;spellid:191634;\nph:1;time:282.1;tag:Zappyxatu;spellid:191634;\nph:1;time:285.1;tag:Zappyxatu;spellid:114050;\nph:1;time:332.2;tag:Zappyxatu;spellid:191634;\nph:1;time:403.6;tag:Zappyxatu;spellid:191634;\nph:1;time:405.7;tag:Zappyxatu;spellid:114050;\nph:1;time:439.5;tag:Zappyxatu;spellid:191634;\nph:1;time:490.2;tag:Zappyxatu;spellid:191634;\nph:1;time:570.5;tag:Zappyxatu;spellid:191634;\nph:1;time:574.6;tag:Zappyxatu;spellid:114050;",
},
["PersonalNotes"] = {
},
["ReminderSettings"] = {
["IconSettings"] = {
["yTimer"] = 0,
["Glow"] = 0,
["borderColors"] = {
0,
0,
0,
1,
},
["xTimer"] = 0,
["xOffset"] = -381,
["yOffset"] = 160,
["GrowDirection"] = "Down",
["Anchor"] = "CENTER",
["TimerFontSize"] = 40,
["FontFlags"] = "OUTLINE",
["relativeTo"] = "CENTER",
["Decimals"] = 3,
["Spacing"] = -1,
["HideSwipe"] = false,
["HideTimerText"] = false,
["textColors"] = {
1,
1,
1,
1,
},
["Width"] = 80,
["yTextOffset"] = 0,
["xTextOffset"] = 0,
["colors"] = {
1,
1,
1,
1,
},
["Font"] = "Expressway",
["Height"] = 80,
["Sticky"] = 5,
["FontSize"] = 30,
["Zoom"] = 0,
},
["OverwriteSharedNoteOnImport"] = false,
["TTSOverSoundfile"] = false,
["TextTTS"] = true,
["GlowSettings"] = {
["Thickness"] = 4,
["Lines"] = 10,
["colors"] = {
0,
1,
0,
1,
},
["xOffset"] = 0,
["Length"] = 10,
["Frequency"] = 0.2,
["yOffset"] = 0,
},
["TextCountdown"] = 0,
["UnitIconSettings"] = {
["Position"] = "CENTER",
["Height"] = 25,
["yOffset"] = 0,
["xOffset"] = 0,
["Width"] = 25,
},
["HideTextTimerText"] = false,
["OverwritePersonalNoteOnImport"] = false,
["SpellTTSTimer"] = 5,
["SpellCountdown"] = 0,
["TextSettings"] = {
["CenterAligned"] = true,
["relativeTo"] = "CENTER",
["Decimals"] = 3,
["HiddenTextFormat"] = "%icon%text",
["xOffset"] = -3,
["TextFormat"] = "%icon%text (%p)",
["textColors"] = {
1,
1,
1,
1,
},
["yOffset"] = 78,
["GrowDirection"] = "Up",
["Font"] = "Expressway",
["colors"] = {
1,
1,
1,
1,
},
["Spacing"] = 1,
["Anchor"] = "CENTER",
["Sticky"] = 0,
["FontSize"] = 50,
["FontFlags"] = "OUTLINE",
},
["BarSettings"] = {
["yTimer"] = 0,
["FontSize"] = 22,
["borderColors"] = {
0,
0,
0,
1,
},
["xTimer"] = -2,
["xOffset"] = 381,
["yOffset"] = -111,
["Spacing"] = -1,
["barColors"] = {
1,
0,
0,
1,
},
["Texture"] = "Atrocity",
["GrowDirection"] = "Up",
["Anchor"] = "CENTER",
["HiddenTimerFormat"] = "",
["TimerFontSize"] = 22,
["FontFlags"] = "OUTLINE",
["Font"] = "Expressway",
["TextFormat"] = "%text",
["Decimals"] = 3,
["TimerFormat"] = "%p",
["backgroundColors"] = {
0,
0,
0,
0.8,
},
["yIcon"] = 0,
["colors"] = {
1,
0,
0,
1,
},
["Width"] = 300,
["yTextOffset"] = 0,
["xTextOffset"] = 2,
["xIcon"] = 0,
["textColors"] = {
1,
1,
1,
1,
},
["Height"] = 40,
["Sticky"] = 5,
["relativeTo"] = "CENTER",
["HiddenTextFormat"] = "%text",
},
["PersNote"] = true,
["TextTTSTimer"] = 5,
["OnlyReceiveGuild"] = false,
["enabled"] = true,
["CircleSettings"] = {
["FontSize"] = 18,
["xOffset"] = -1,
["yOffset"] = -76,
["ringColors"] = {
1,
1,
1,
1,
},
["GrowDirection"] = "Up",
["Anchor"] = "CENTER",
["TextPosition"] = "Top",
["Size"] = 80,
["TextFormat"] = "%icon%text (%p)",
["Decimals"] = 3,
["Font"] = "Expressway",
["Spacing"] = 5,
["showBackground"] = true,
["yTextOffset"] = 4,
["xTextOffset"] = 0,
["textColors"] = {
1,
1,
1,
1,
},
["FontFlags"] = "OUTLINE",
["Texture"] = "Interface\\AddOns\\NorthernSkyRaidTools\\Media\\Textures\\circle_8px.png",
["Sticky"] = 0,
["relativeTo"] = "CENTER",
["HiddenTextFormat"] = "%icon%text",
},
["SpellDisplayType"] = "Icon",
["SpellName"] = true,
["ClearOnKill"] = true,
["DebuffOverviewSettings"] = {
["yTimer"] = 0,
["FontSize"] = 22,
["borderColors"] = {
0,
0,
0,
1,
},
["xTimer"] = -2,
["xOffset"] = 393,
["yOffset"] = 65,
["IconPosition"] = "Left",
["barColors"] = {
1,
0,
0,
1,
},
["GrowDirection"] = "Up",
["Anchor"] = "CENTER",
["TimerFontSize"] = 22,
["Texture"] = "Atrocity",
["relativeTo"] = "CENTER",
["backgroundColors"] = {
0,
0,
0,
0.8,
},
["Width"] = 300,
["yTextOffset"] = 0,
["xTextOffset"] = 2,
["textColors"] = {
1,
1,
1,
1,
},
["Height"] = 40,
["Spacing"] = -1,
["FontFlags"] = "OUTLINE",
["Font"] = "Expressway",
},
["ReminderFrame"] = {
["enabled"] = false,
["FontSize"] = 14,
["xOffset"] = 0,
["Moveable"] = false,
["Width"] = 500,
["Font"] = "Expressway",
["BGcolor"] = {
0,
0,
0,
0.3,
},
["relativeTo"] = "TOPLEFT",
["Height"] = 600,
["Anchor"] = "TOPLEFT",
["yOffset"] = 0,
["FontFlags"] = "OUTLINE",
},
["TextDuration"] = 10,
["ExtraReminderFrame"] = {
["enabled"] = false,
["FontSize"] = 14,
["xOffset"] = 0,
["Width"] = 500,
["Font"] = "Expressway",
["BGcolor"] = {
0,
0,
0,
0.3,
},
["relativeTo"] = "TOPLEFT",
["Height"] = 600,
["Anchor"] = "TOPLEFT",
["yOffset"] = 0,
["FontFlags"] = "OUTLINE",
},
["HideTimerText"] = false,
["SpellTTS"] = true,
["NoteCountdown"] = true,
["AutoShare"] = true,
["Sticky"] = 5,
["SpellDuration"] = 10,
["PersonalReminderFrame"] = {
["enabled"] = true,
["FontSize"] = 16,
["xOffset"] = -64,
["Moveable"] = false,
["Width"] = 500,
["Font"] = "Expressway",
["BGcolor"] = {
0,
0,
0,
0.3,
},
["relativeTo"] = "BOTTOM",
["Height"] = 600,
["Anchor"] = "BOTTOM",
["yOffset"] = 0,
["FontFlags"] = "OUTLINE",
},
},
["LastUsedConsumables"] = {
["Lonelylitten-Stormrage"] = {
["flask"] = 245931,
},
["Zappyxatu-Stormrage"] = {
["flask"] = 245929,
},
},
["Reminders"] = {
},
["HasNewAlertStructure"] = true,
},
},
["ReminderSettings"] = {
["IconSettings"] = {
["yTimer"] = 0,
["FontSize"] = 30,
["borderColors"] = {
0,
0,
0,
1,
},
["xTimer"] = 0,
["xOffset"] = -381,
["yOffset"] = 160,
["GrowDirection"] = "Down",
["Anchor"] = "CENTER",
["TimerFontSize"] = 40,
["FontFlags"] = "OUTLINE",
["relativeTo"] = "CENTER",
["Decimals"] = 3,
["Zoom"] = 0,
["Glow"] = 0,
["xTextOffset"] = 0,
["colors"] = {
1,
1,
1,
1,
},
["Width"] = 80,
["yTextOffset"] = 0,
["HideTimerText"] = false,
["textColors"] = {
1,
1,
1,
1,
},
["Font"] = "Expressway",
["Height"] = 80,
["Sticky"] = 5,
["HideSwipe"] = false,
["Spacing"] = -1,
},
["OverwriteSharedNoteOnImport"] = false,
["BarSettings"] = {
["yTimer"] = 0,
["FontSize"] = 22,
["borderColors"] = {
0,
0,
0,
1,
},
["xTimer"] = -2,
["xOffset"] = 381,
["yOffset"] = -111,
["GrowDirection"] = "Up",
["barColors"] = {
1,
0,
0,
1,
},
["TextFormat"] = "%text",
["Spacing"] = -1,
["Anchor"] = "CENTER",
["HiddenTimerFormat"] = "",
["TimerFontSize"] = 22,
["FontFlags"] = "OUTLINE",
["xTextOffset"] = 2,
["relativeTo"] = "CENTER",
["Decimals"] = 3,
["Height"] = 40,
["backgroundColors"] = {
0,
0,
0,
0.8,
},
["yIcon"] = 0,
["xIcon"] = 0,
["Width"] = 300,
["yTextOffset"] = 0,
["Font"] = "Expressway",
["textColors"] = {
1,
1,
1,
1,
},
["colors"] = {
1,
0,
0,
1,
},
["TimerFormat"] = "%p",
["Sticky"] = 5,
["Texture"] = "Atrocity",
["HiddenTextFormat"] = "%text",
},
["AutoShare"] = true,
["GlowSettings"] = {
["Thickness"] = 4,
["Lines"] = 10,
["colors"] = {
0,
1,
0,
1,
},
["xOffset"] = 0,
["Length"] = 10,
["yOffset"] = 0,
["Frequency"] = 0.2,
},
["TextCountdown"] = 0,
["UnitIconSettings"] = {
["Position"] = "CENTER",
["xOffset"] = 0,
["Width"] = 25,
["Height"] = 25,
["yOffset"] = 0,
},
["HideTextTimerText"] = false,
["Sticky"] = 5,
["SpellTTSTimer"] = 5,
["SpellDuration"] = 10,
["TextSettings"] = {
["FontFlags"] = "OUTLINE",
["TextFormat"] = "%icon%text (%p)",
["Decimals"] = 3,
["relativeTo"] = "CENTER",
["xOffset"] = -3,
["FontSize"] = 50,
["colors"] = {
1,
1,
1,
1,
},
["yOffset"] = 78,
["Spacing"] = 1,
["Font"] = "Expressway",
["textColors"] = {
1,
1,
1,
1,
},
["GrowDirection"] = "Up",
["Anchor"] = "CENTER",
["Sticky"] = 0,
["HiddenTextFormat"] = "%icon%text",
["CenterAligned"] = true,
},
["TTSOverSoundfile"] = false,
["PersNote"] = true,
["TextTTSTimer"] = 5,
["OnlyReceiveGuild"] = false,
["enabled"] = true,
["NoteCountdown"] = true,
["SpellDisplayType"] = "Icon",
["ReminderFrame"] = {
["enabled"] = false,
["relativeTo"] = "TOPLEFT",
["xOffset"] = 0,
["Moveable"] = false,
["Width"] = 500,
["Font"] = "Expressway",
["BGcolor"] = {
0,
0,
0,
0.3,
},
["FontSize"] = 14,
["Height"] = 600,
["yOffset"] = 0,
["Anchor"] = "TOPLEFT",
["FontFlags"] = "OUTLINE",
},
["ClearOnKill"] = true,
["SpellTTS"] = true,
["DebuffOverviewSettings"] = {
["yTimer"] = 0,
["FontSize"] = 22,
["borderColors"] = {
0,
0,
0,
1,
},
["xTimer"] = -2,
["xOffset"] = 393,
["yOffset"] = 65,
["IconPosition"] = "Left",
["barColors"] = {
1,
0,
0,
1,
},
["GrowDirection"] = "Up",
["Anchor"] = "CENTER",
["TimerFontSize"] = 22,
["Texture"] = "Atrocity",
["relativeTo"] = "CENTER",
["backgroundColors"] = {
0,
0,
0,
0.8,
},
["Width"] = 300,
["yTextOffset"] = 0,
["xTextOffset"] = 2,
["textColors"] = {
1,
1,
1,
1,
},
["Height"] = 40,
["Font"] = "Expressway",
["FontFlags"] = "OUTLINE",
["Spacing"] = -1,
},
["TextDuration"] = 10,
["ExtraReminderFrame"] = {
["enabled"] = false,
["relativeTo"] = "TOPLEFT",
["xOffset"] = 0,
["Width"] = 500,
["Font"] = "Expressway",
["BGcolor"] = {
0,
0,
0,
0.3,
},
["FontSize"] = 14,
["Height"] = 600,
["yOffset"] = 0,
["Anchor"] = "TOPLEFT",
["FontFlags"] = "OUTLINE",
},
["HideTimerText"] = false,
["SpellName"] = true,
["CircleSettings"] = {
["FontSize"] = 18,
["xOffset"] = -1,
["yOffset"] = -76,
["ringColors"] = {
1,
1,
1,
1,
},
["GrowDirection"] = "Up",
["Anchor"] = "CENTER",
["TextPosition"] = "Top",
["Size"] = 80,
["TextFormat"] = "%icon%text (%p)",
["Decimals"] = 3,
["relativeTo"] = "CENTER",
["xTextOffset"] = 0,
["Sticky"] = 0,
["yTextOffset"] = 4,
["Font"] = "Expressway",
["textColors"] = {
1,
1,
1,
1,
},
["Texture"] = "Interface\\AddOns\\NorthernSkyRaidTools\\Media\\Textures\\circle_8px.png",
["FontFlags"] = "OUTLINE",
["showBackground"] = true,
["Spacing"] = 5,
["HiddenTextFormat"] = "%icon%text",
},
["SpellCountdown"] = 0,
["OverwritePersonalNoteOnImport"] = false,
["TextTTS"] = true,
["PersonalReminderFrame"] = {
["enabled"] = true,
["relativeTo"] = "BOTTOM",
["xOffset"] = -64,
["Moveable"] = false,
["Width"] = 500,
["Font"] = "Expressway",
["BGcolor"] = {
0,
0,
0,
0.3,
},
["FontSize"] = 16,
["Height"] = 600,
["yOffset"] = 0,
["Anchor"] = "BOTTOM",
["FontFlags"] = "OUTLINE",
},
},
["ReadyCheckSettings"] = {
["TierCheck"] = false,
["RepairCheck"] = false,
["SoulstoneCheck"] = false,
["EnchantCheck"] = false,
["RaidBuffCheck"] = false,
["ConsumablesDisplay"] = true,
["SkipGatewayKeybindCheck"] = false,
["GemCheck"] = false,
["CraftedCheck"] = false,
["BlisteringScalesCheck"] = false,
["GatewayShardCheck"] = false,
["MissingItemCheck"] = false,
["SourceOfMagicCheck"] = false,
["SymbioticRelationshipCheck"] = false,
["ItemLevelCheck"] = false,
},
["PersonalReminders"] = {
["Ele Altar"] = "EncounterID:3429;Name:Ele Altar;Difficulty:Heroic\nph:1;time:0.6;tag:Zappyxatu;spellid:114050;\nph:2.5;time:14.7;tag:Zappyxatu;spellid:114050;",
["Ele Ula"] = "EncounterID:3492;Name:Ele Ula;Difficulty:Heroic\nph:1;time:5.0;tag:Zappyxatu;spellid:191634;\nph:1;time:7.7;tag:Zappyxatu;spellid:114050;\nph:1;time:43.9;tag:Zappyxatu;spellid:191634;\nph:1;time:131.6;tag:Zappyxatu;spellid:191634;\nph:1;time:136.4;tag:Zappyxatu;spellid:114050;\nph:1;time:178.0;tag:Zappyxatu;spellid:191634;\nph:1;time:235.4;tag:Zappyxatu;spellid:191634;\nph:1;time:282.1;tag:Zappyxatu;spellid:191634;\nph:1;time:285.1;tag:Zappyxatu;spellid:114050;\nph:1;time:332.2;tag:Zappyxatu;spellid:191634;\nph:1;time:403.6;tag:Zappyxatu;spellid:191634;\nph:1;time:405.7;tag:Zappyxatu;spellid:114050;\nph:1;time:439.5;tag:Zappyxatu;spellid:191634;\nph:1;time:490.2;tag:Zappyxatu;spellid:191634;\nph:1;time:570.5;tag:Zappyxatu;spellid:191634;\nph:1;time:574.6;tag:Zappyxatu;spellid:114050;",
},
["PersonalNotes"] = {
},
["InterruptSettings"] = {
["xOffset"] = -600,
["InterruptDefaultTextColor"] = {
1,
1,
1,
1,
},
["InterruptNextColor"] = {
1,
1,
0,
1,
},
["yOffset"] = 400,
["NumberFontSize"] = 60,
["InterruptDefaultColor"] = {
1,
0,
0,
1,
},
["NumberRelativeTo"] = "CENTER",
["NameFontFlags"] = "OUTLINE",
["Anchor"] = "CENTER",
["NameRelativeTo"] = "TOP",
["NameFont"] = "Expressway",
["ShowBar"] = false,
["NumberFont"] = "Expressway",
["relativeTo"] = "CENTER",
["InterruptSound"] = "|cFF4BAAC8Interrupt|r",
["NumberxOffset"] = 0,
["NameFontSize"] = 30,
["NumberyOffset"] = 0,
["InterruptNowTextColor"] = {
1,
0,
0,
1,
},
["Width"] = 100,
["NumberFontFlags"] = "OUTLINE",
["InterruptNextTextColor"] = {
1,
0,
0,
1,
},
["NamexOffset"] = 0,
["NameAnchor"] = "BOTTOM",
["Height"] = 100,
["InterruptNowColor"] = {
0,
1,
0,
1,
},
["NameyOffset"] = 10,
["NumberAnchor"] = "CENTER",
},
["EncounterAlerts"] = {
[3421] = {
[14] = {
["DebuffOverview"] = {
["extraOptions"] = {
{
["set"] = "return function(NSI, r, g, b, a) for i = 14, 16 do NSRT.EncounterAlerts[3421][i].DebuffOverview.BarColor = {r, g, b, a} end NSI:CreateDebuffOverviewContainers(\"HARMFUL\", {isFromPlayerOrPlayerPet = false, isBossOrRoleAura = false}, 1, 1, \"TwinFangsDebuffOverview\", false, false, true, 10, {barColors = {r, g, b, a}}, true) end",
["Type"] = "Color",
["get"] = "return function() local a = NSRT.EncounterAlerts[3421][16].DebuffOverview local c = a.BarColor or NSRT.ReminderSettings.DebuffOverviewSettings.barColors return c[1], c[2], c[3], c[4] end",
["label"] = "Bar Color",
},
{
["min"] = 10,
["Type"] = "Slider",
["label"] = "Bar Height",
["max"] = 100,
["set"] = "return function(NSI, value) for i = 14, 16 do NSRT.EncounterAlerts[3421][i].DebuffOverview.BarHeight = value end NSI:CreateDebuffOverviewContainers(\"HARMFUL\", {isFromPlayerOrPlayerPet = false, isBossOrRoleAura = false}, 1, 1, \"TwinFangsDebuffOverview\", false, false, true, 10, {height = value}, true) end",
["step"] = 1,
["get"] = "return function() local a = NSRT.EncounterAlerts[3421][16].DebuffOverview return a.BarHeight or NSRT.ReminderSettings.DebuffOverviewSettings.Height end",
},
},
["group"] = "Twin Fangs",
["BarHeight"] = 25,
["NoEdit"] = true,
["enabled"] = false,
["customIcon"] = 1290336,
["encID"] = 3421,
["Preview"] = "        return function(self)\n            local alert = NSRT.EncounterAlerts[3421][16].DebuffOverview\n            local overviewSettings = NSRT.ReminderSettings.DebuffOverviewSettings\n            self:PreviewDebuffOverviewContainers(\"HARMFUL\", {isFromPlayerOrPlayerPet = false, isBossOrRoleAura = false}, 1, 1, \"TwinFangsDebuffOverview\", false, false, true, 10, 20, {\n                barColors = alert.BarColor or overviewSettings.barColors,\n                backgroundColors = overviewSettings.backgroundColors,\n                height = alert.BarHeight or overviewSettings.Height,\n            }, true)\n        end\n    ",
["internalID"] = "DebuffOverview",
["dur"] = 5,
["TTS"] = false,
["TTSTimer"] = 5,
["BlockCopy"] = true,
["id"] = 0.3,
["phase"] = 1,
["text"] = "Eternal Venom Overview",
["difficulties"] = {
14,
15,
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["Version"] = {
{
["customIcon"] = 1290336,
},
["versionNumber"] = 1,
},
["name"] = "Eternal Venom Overview",
["IsAlert"] = true,
["timers"] = {
},
["sticky"] = 0,
["isSpecialDisplay"] = true,
["DisplayType"] = "Bar",
},
},
[16] = {
["Soak"] = {
["timers"] = {
64.7,
125.7,
219.8,
280.8,
374.9,
435.9,
},
["group"] = "Twin Fangs",
["barColors"] = {
1,
0,
0,
1,
},
["encID"] = 3421,
["internalID"] = "Soak",
["TTS"] = true,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["Ticks"] = {
4.5,
6.5,
},
["spellID"] = 1290516,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["id"] = 4,
["name"] = "Soak",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["text"] = "Soak",
["DisplayType"] = "Bar",
},
["WatchSpawns"] = {
["timers"] = {
18.3,
79.3,
172.3,
233.3,
327.3,
388.3,
},
["group"] = "Twin Fangs",
["loadConditions"] = {
["Roles"] = {
["MELEE"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3421,
["internalID"] = "WatchSpawns",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Push",
["spellID"] = 1288538,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["text"] = "Push",
["name"] = "Push",
["loadConditions"] = {
["Roles"] = {
["MELEE"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
},
["versionNumber"] = 1,
},
["name"] = "Push",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 19,
["DisplayType"] = "Text",
},
["Adds"] = {
["timers"] = {
35,
96,
190,
251,
345,
406,
},
["group"] = "Twin Fangs",
["encID"] = 3421,
["internalID"] = "Adds",
["enabled"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Adds",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1291404,
["name"] = "Adds",
["IsAlert"] = true,
["id"] = 13,
["sticky"] = 0,
["TTS"] = true,
["DisplayType"] = "Text",
},
["Soak1"] = {
["timers"] = {
61.2,
122.2,
216.3,
277.3,
371.4,
432.4,
},
["group"] = "Twin Fangs",
["encID"] = 3421,
["internalID"] = "Soak1",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Soak",
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["spellID"] = 1290516,
["name"] = "Soak 1",
["IsAlert"] = true,
["id"] = 5,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Soak2"] = {
["timers"] = {
63.2,
124.2,
218.3,
279.3,
373.4,
434.4,
},
["group"] = "Twin Fangs",
["encID"] = 3421,
["internalID"] = "Soak2",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Soak",
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["spellID"] = 1290516,
["name"] = "Soak 2",
["IsAlert"] = true,
["id"] = 6,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["PreSpread"] = {
["timers"] = {
43.8,
104.8,
198.7,
259.6,
353.7,
414.6,
},
["group"] = "Twin Fangs",
["loadConditions"] = {
["Roles"] = {
["DAMAGER"] = true,
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3421,
["internalID"] = "PreSpread",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Pre-Spread",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1290809,
["name"] = "Pre-Spread",
["IsAlert"] = true,
["id"] = 9,
["sticky"] = 0,
["TTS"] = "Spread",
["DisplayType"] = "Circle",
},
["TankSoak"] = {
["timers"] = {
30.4,
91.4,
185.4,
246.4,
342.4,
403.4,
},
["group"] = "Twin Fangs",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss2\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss2.",
},
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3421,
["internalID"] = "TankSoak",
["TTS"] = true,
["TTSTimer"] = 12,
["dur"] = 12,
["phase"] = 1,
["text"] = "Soak",
["spellID"] = 1288538,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["id"] = 17,
["name"] = "Tank Soak",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["Ticks"] = {
6,
9,
},
["DisplayType"] = "Bar",
},
["WatchSide"] = {
["timers"] = {
136,
290.9,
},
["group"] = "Twin Fangs",
["encID"] = 3421,
["internalID"] = "WatchSide",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Watch Side",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1294293,
["name"] = "Watch Side",
["IsAlert"] = true,
["id"] = 11,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Defensives"] = {
["timers"] = {
46.9,
107.9,
202,
263.1,
357.1,
418,
},
["group"] = "Twin Fangs",
["loadConditions"] = {
["Roles"] = {
["DAMAGER"] = true,
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3421,
["internalID"] = "Defensives",
["enabled"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Defensives",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1290956,
["name"] = "Defensives",
["IsAlert"] = true,
["id"] = 2,
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Text",
},
["Knock"] = {
["timers"] = {
9,
70,
164,
225,
318.9,
379.9,
},
["group"] = "Twin Fangs",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss1.",
},
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3421,
["internalID"] = "Knock",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Knock",
["spellID"] = 1289192,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["id"] = 21,
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["name"] = "Knock",
["DisplayType"] = "Text",
},
["Orbs"] = {
["timers"] = {
11.9,
72.9,
166.7,
227.7,
321.6,
382.6,
},
["group"] = "Twin Fangs",
["encID"] = 3421,
["internalID"] = "Orbs",
["enabled"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Orbs",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1289994,
["name"] = "Orbs",
["IsAlert"] = true,
["id"] = 15,
["sticky"] = 0,
["TTS"] = true,
["DisplayType"] = "Text",
},
["InterruptWhisperAlert"] = {
["extraOptions"] = {
{
["text"] = "This Alert shows up whenever you receive any whisper at all during the encounter. It cannot filter out wrong whispers.",
["Type"] = "Label",
},
{
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3421][16].InterruptWhisperAlert.ShowWhisperSender = value end",
["Type"] = "Checkbox",
["get"] = "return function() return NSRT.EncounterAlerts[3421][16].InterruptWhisperAlert.ShowWhisperSender or false end",
["label"] = "Show Whisper Sender",
},
},
["group"] = "Twin Fangs",
["isSpecialDisplay"] = true,
["timers"] = {
},
["BlockCopy"] = true,
["encID"] = 3421,
["Preview"] = "        return function(self)\n            local alert = NSRT.EncounterAlerts[3421][16].InterruptWhisperAlert\n            local info = self:CreateReminder(CopyTable(alert), true)\n            if alert.ShowWhisperSender then\n                local sender = secretwrap(UnitName(\"player\"))\n                local senderGUID = secretwrap(UnitGUID(\"player\"))\n                local classFilename = senderGUID and select(2, UnitClassFromGUID(senderGUID))\n                local classColor = classFilename and C_ClassColor.GetClassColor(classFilename)\n                local senderNameClassColored = classColor and C_ColorUtil.WrapTextInColor(sender, classColor) or sender\n                info.text = string.format(\"%s - %s\", alert.text, senderNameClassColored)\n            end\n            self:DisplayReminder(info)\n        end\n    ",
["internalID"] = "InterruptWhisperAlert",
["phase"] = 1,
["enabled"] = false,
["TTSTimer"] = 3,
["dur"] = 3,
["difficulties"] = {
16,
},
["id"] = 0.4,
["pinned"] = true,
["ShowWhisperSender"] = false,
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["TTS"] = false,
["name"] = "Interrupt Whisper Alert",
["IsAlert"] = true,
["text"] = "Your Interrupt",
["sticky"] = 0,
["sound"] = "|cFF4BAAC8Interrupt|r",
["DisplayType"] = "Text",
},
["Soak3"] = {
["timers"] = {
64.7,
125.7,
219.8,
280.8,
374.9,
435.9,
},
["group"] = "Twin Fangs",
["encID"] = 3421,
["internalID"] = "Soak3",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Soak",
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["spellID"] = 1290516,
["name"] = "Soak 3",
["IsAlert"] = true,
["id"] = 7,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["DebuffOverview"] = {
["extraOptions"] = {
{
["set"] = "return function(NSI, r, g, b, a) for i = 14, 16 do NSRT.EncounterAlerts[3421][i].DebuffOverview.BarColor = {r, g, b, a} end NSI:CreateDebuffOverviewContainers(\"HARMFUL\", {isFromPlayerOrPlayerPet = false, isBossOrRoleAura = false}, 1, 1, \"TwinFangsDebuffOverview\", false, false, true, 10, {barColors = {r, g, b, a}}, true) end",
["Type"] = "Color",
["get"] = "return function() local a = NSRT.EncounterAlerts[3421][16].DebuffOverview local c = a.BarColor or NSRT.ReminderSettings.DebuffOverviewSettings.barColors return c[1], c[2], c[3], c[4] end",
["label"] = "Bar Color",
},
{
["min"] = 10,
["Type"] = "Slider",
["label"] = "Bar Height",
["max"] = 100,
["set"] = "return function(NSI, value) for i = 14, 16 do NSRT.EncounterAlerts[3421][i].DebuffOverview.BarHeight = value end NSI:CreateDebuffOverviewContainers(\"HARMFUL\", {isFromPlayerOrPlayerPet = false, isBossOrRoleAura = false}, 1, 1, \"TwinFangsDebuffOverview\", false, false, true, 10, {height = value}, true) end",
["step"] = 1,
["get"] = "return function() local a = NSRT.EncounterAlerts[3421][16].DebuffOverview return a.BarHeight or NSRT.ReminderSettings.DebuffOverviewSettings.Height end",
},
},
["group"] = "Twin Fangs",
["BarHeight"] = 25,
["NoEdit"] = true,
["enabled"] = false,
["customIcon"] = 1290336,
["encID"] = 3421,
["Preview"] = "        return function(self)\n            local alert = NSRT.EncounterAlerts[3421][16].DebuffOverview\n            local overviewSettings = NSRT.ReminderSettings.DebuffOverviewSettings\n            self:PreviewDebuffOverviewContainers(\"HARMFUL\", {isFromPlayerOrPlayerPet = false, isBossOrRoleAura = false}, 1, 1, \"TwinFangsDebuffOverview\", false, false, true, 10, 20, {\n                barColors = alert.BarColor or overviewSettings.barColors,\n                backgroundColors = overviewSettings.backgroundColors,\n                height = alert.BarHeight or overviewSettings.Height,\n            }, true)\n        end\n    ",
["internalID"] = "DebuffOverview",
["dur"] = 5,
["TTS"] = false,
["TTSTimer"] = 5,
["BlockCopy"] = true,
["id"] = 0.3,
["phase"] = 1,
["text"] = "Eternal Venom Overview",
["difficulties"] = {
14,
15,
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["Version"] = {
{
["customIcon"] = 1290336,
},
["versionNumber"] = 1,
},
["name"] = "Eternal Venom Overview",
["IsAlert"] = true,
["timers"] = {
},
["sticky"] = 0,
["isSpecialDisplay"] = true,
["DisplayType"] = "Bar",
},
},
[15] = {
["Soak"] = {
["timers"] = {
71.4,
139.1,
240.8,
308.6,
410.3,
478.1,
},
["group"] = "Twin Fangs",
["barColors"] = {
1,
0,
0,
1,
},
["encID"] = 3421,
["internalID"] = "Soak",
["TTS"] = true,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["Ticks"] = {
4.5,
6.5,
},
["spellID"] = 1290516,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["id"] = 3,
["name"] = "Soak",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["text"] = "Soak",
["DisplayType"] = "Bar",
},
["WatchSpawns"] = {
["timers"] = {
20.5,
88.3,
190,
257.8,
359.4,
427.2,
},
["group"] = "Twin Fangs",
["loadConditions"] = {
["Roles"] = {
["MELEE"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3421,
["internalID"] = "WatchSpawns",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Push",
["spellID"] = 1288538,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["text"] = "Push",
["name"] = "Push",
["loadConditions"] = {
["Roles"] = {
["MELEE"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
},
["versionNumber"] = 1,
},
["name"] = "Push",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 18,
["DisplayType"] = "Text",
},
["Adds"] = {
["timers"] = {
39.7,
107.5,
209.2,
276.9,
378.6,
446.4,
},
["group"] = "Twin Fangs",
["encID"] = 3421,
["internalID"] = "Adds",
["enabled"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Adds",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1291404,
["name"] = "Adds",
["IsAlert"] = true,
["id"] = 12,
["sticky"] = 0,
["TTS"] = true,
["DisplayType"] = "Text",
},
["Orbs"] = {
["timers"] = {
12.9,
80.7,
182.4,
250.2,
351.8,
419.6,
},
["group"] = "Twin Fangs",
["encID"] = 3421,
["internalID"] = "Orbs",
["enabled"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Orbs",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1289994,
["name"] = "Orbs",
["IsAlert"] = true,
["id"] = 14,
["sticky"] = 0,
["TTS"] = true,
["DisplayType"] = "Text",
},
["PreSpread"] = {
["timers"] = {
48.5,
116.2,
218,
285.7,
387.4,
455.1,
},
["group"] = "Twin Fangs",
["loadConditions"] = {
["Roles"] = {
["DAMAGER"] = true,
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3421,
["internalID"] = "PreSpread",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Pre-Spread",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1290809,
["name"] = "Pre-Spread",
["IsAlert"] = true,
["id"] = 8,
["sticky"] = 0,
["TTS"] = "Spread",
["DisplayType"] = "Circle",
},
["TankSoak"] = {
["timers"] = {
32.5,
100.3,
202,
269.8,
371.4,
439.2,
},
["group"] = "Twin Fangs",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss2\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss2.",
},
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3421,
["internalID"] = "TankSoak",
["TTS"] = true,
["TTSTimer"] = 12,
["dur"] = 12,
["phase"] = 1,
["text"] = "Soak",
["spellID"] = 1288538,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["id"] = 16,
["name"] = "Tank Soak",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["Ticks"] = {
6,
9,
},
["DisplayType"] = "Bar",
},
["WatchSide"] = {
["timers"] = {
150.5,
319.9,
},
["group"] = "Twin Fangs",
["encID"] = 3421,
["internalID"] = "WatchSide",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Watch Side",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1294293,
["name"] = "Watch Side",
["IsAlert"] = true,
["id"] = 10,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Knock"] = {
["timers"] = {
9.9,
77.7,
179.4,
247.2,
348.8,
416.6,
},
["group"] = "Twin Fangs",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss1.",
},
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3421,
["internalID"] = "Knock",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Knock",
["spellID"] = 1289192,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["id"] = 20,
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["name"] = "Knock",
["DisplayType"] = "Text",
},
["Defensives"] = {
["timers"] = {
52.3,
120,
221.7,
289.5,
391.2,
458.9,
},
["group"] = "Twin Fangs",
["loadConditions"] = {
["Roles"] = {
["DAMAGER"] = true,
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3421,
["internalID"] = "Defensives",
["enabled"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Defensives",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1290956,
["name"] = "Defensives",
["IsAlert"] = true,
["id"] = 1,
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Text",
},
["DebuffOverview"] = {
["extraOptions"] = {
{
["set"] = "return function(NSI, r, g, b, a) for i = 14, 16 do NSRT.EncounterAlerts[3421][i].DebuffOverview.BarColor = {r, g, b, a} end NSI:CreateDebuffOverviewContainers(\"HARMFUL\", {isFromPlayerOrPlayerPet = false, isBossOrRoleAura = false}, 1, 1, \"TwinFangsDebuffOverview\", false, false, true, 10, {barColors = {r, g, b, a}}, true) end",
["Type"] = "Color",
["get"] = "return function() local a = NSRT.EncounterAlerts[3421][16].DebuffOverview local c = a.BarColor or NSRT.ReminderSettings.DebuffOverviewSettings.barColors return c[1], c[2], c[3], c[4] end",
["label"] = "Bar Color",
},
{
["min"] = 10,
["Type"] = "Slider",
["label"] = "Bar Height",
["max"] = 100,
["set"] = "return function(NSI, value) for i = 14, 16 do NSRT.EncounterAlerts[3421][i].DebuffOverview.BarHeight = value end NSI:CreateDebuffOverviewContainers(\"HARMFUL\", {isFromPlayerOrPlayerPet = false, isBossOrRoleAura = false}, 1, 1, \"TwinFangsDebuffOverview\", false, false, true, 10, {height = value}, true) end",
["step"] = 1,
["get"] = "return function() local a = NSRT.EncounterAlerts[3421][16].DebuffOverview return a.BarHeight or NSRT.ReminderSettings.DebuffOverviewSettings.Height end",
},
},
["group"] = "Twin Fangs",
["BarHeight"] = 25,
["NoEdit"] = true,
["enabled"] = false,
["customIcon"] = 1290336,
["encID"] = 3421,
["Preview"] = "        return function(self)\n            local alert = NSRT.EncounterAlerts[3421][16].DebuffOverview\n            local overviewSettings = NSRT.ReminderSettings.DebuffOverviewSettings\n            self:PreviewDebuffOverviewContainers(\"HARMFUL\", {isFromPlayerOrPlayerPet = false, isBossOrRoleAura = false}, 1, 1, \"TwinFangsDebuffOverview\", false, false, true, 10, 20, {\n                barColors = alert.BarColor or overviewSettings.barColors,\n                backgroundColors = overviewSettings.backgroundColors,\n                height = alert.BarHeight or overviewSettings.Height,\n            }, true)\n        end\n    ",
["internalID"] = "DebuffOverview",
["dur"] = 5,
["TTS"] = false,
["TTSTimer"] = 5,
["BlockCopy"] = true,
["id"] = 0.3,
["phase"] = 1,
["text"] = "Eternal Venom Overview",
["difficulties"] = {
14,
15,
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["Version"] = {
{
["customIcon"] = 1290336,
},
["versionNumber"] = 1,
},
["name"] = "Eternal Venom Overview",
["IsAlert"] = true,
["timers"] = {
},
["sticky"] = 0,
["isSpecialDisplay"] = true,
["DisplayType"] = "Bar",
},
},
},
[3177] = {
},
[3179] = {
},
[3429] = {
[14] = {
["P2_5WrongTarget"] = {
["isSpecialDisplay"] = true,
["group"] = "Coiled Altar",
["HideTimer"] = true,
["encID"] = 3429,
["internalID"] = "P2_5WrongTarget",
["BlockCopy"] = true,
["TTS"] = false,
["TTSTimer"] = 50,
["dur"] = 50,
["id"] = 40,
["phase"] = 2.5,
["text"] = "WRONG TARGET",
["timers"] = {
0,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["textColors"] = {
1,
0,
0,
1,
},
["name"] = "Wrong Target",
["IsAlert"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["sticky"] = 50,
["enabled"] = true,
["DisplayType"] = "Text",
},
["DebuffCircle"] = {
["isSpecialDisplay"] = true,
["group"] = "Coiled Altar",
["CircleTexture"] = "Interface\\AddOns\\NorthernSkyRaidTools\\Media\\Textures\\circle_8px.png",
["encID"] = 3429,
["Preview"] = "return function(NSI)\n        local alert = NSRT.EncounterAlerts[3429][16].DebuffCircle\n        NSI:PreviewReminderCircle(\"CoiledAltarDebuffCirclePreview\", 5, alert.CircleColor, alert.CircleTexture)\n    end",
["internalID"] = "DebuffCircle",
["CircleColor"] = {
1,
0,
0,
1,
},
["TTS"] = false,
["NoEdit"] = true,
["BlockCopy"] = true,
["phase"] = 1,
["id"] = 19,
["difficulties"] = {
14,
15,
16,
},
["text"] = "",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["timers"] = {
},
["name"] = "Orb/Bomb Circle",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["extraOptions"] = {
{
["set"] = "return function(NSI, r, g, b, a) for difficultyID = 14, 16 do NSRT.EncounterAlerts[3429][difficultyID].DebuffCircle.CircleColor = {r, g, b, a} end NSI:UpdateCoiledAltarDebuffCircle() NSI:UpdateReminderCirclePreview(\"CoiledAltarDebuffCirclePreview\", NSRT.EncounterAlerts[3429][16].DebuffCircle.CircleColor, NSRT.EncounterAlerts[3429][16].DebuffCircle.CircleTexture) end",
["Type"] = "Color",
["get"] = "return function() local alert = NSRT.EncounterAlerts[3429][16].DebuffCircle local c = alert.CircleColor or {1, 0, 0, 1} return c[1], c[2], c[3], c[4] end",
["label"] = "Ring Color",
},
{
["Type"] = "Dropdown",
["values"] = "return function() return {\n                {label = \"2 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_2px.png\"},\n                {label = \"5 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_5px.png\"},\n                {label = \"8 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_8px.png\"},\n                {label = \"10 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_10px.png\"},\n                {label = \"15 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_15px.png\"},\n            } end",
["label"] = "Ring Size",
["set"] = "return function(NSI, value) for difficultyID = 14, 16 do NSRT.EncounterAlerts[3429][difficultyID].DebuffCircle.CircleTexture = value end NSI:UpdateCoiledAltarDebuffCircle() NSI:UpdateReminderCirclePreview(\"CoiledAltarDebuffCirclePreview\", NSRT.EncounterAlerts[3429][16].DebuffCircle.CircleColor, value) end",
["get"] = "return function() local alert = NSRT.EncounterAlerts[3429][16].DebuffCircle return alert.CircleTexture or \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_8px.png\" end",
},
},
["DisplayType"] = "Circle",
},
},
[16] = {
["P2_5WrongTarget"] = {
["isSpecialDisplay"] = true,
["group"] = "Coiled Altar",
["HideTimer"] = true,
["encID"] = 3429,
["internalID"] = "P2_5WrongTarget",
["BlockCopy"] = true,
["TTS"] = false,
["TTSTimer"] = 50,
["dur"] = 50,
["id"] = 41,
["phase"] = 2.5,
["text"] = "WRONG TARGET",
["timers"] = {
0,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["textColors"] = {
1,
0,
0,
1,
},
["name"] = "Wrong Target",
["IsAlert"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["sticky"] = 50,
["enabled"] = true,
["DisplayType"] = "Text",
},
["P1Frontal"] = {
["timers"] = {
23,
40,
60,
77.1,
108.1,
125,
145,
162.1,
},
["group"] = "Coiled Altar",
["isConditional"] = {
["func"] = "return function() if UnitGroupRolesAssigned(\"player\") ~= \"TANK\" then return true end local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you are not a tank or have threat on boss1.",
},
["encID"] = 3429,
["internalID"] = "P1Frontal",
["TTS"] = true,
["TTSTimer"] = 8,
["dur"] = 8,
["spellID"] = 1299684,
["id"] = 2,
["text"] = "Frontal",
["enabled"] = false,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P1 Frontal",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["sticky"] = 0,
["phase"] = 1,
["DisplayType"] = "Text",
},
["P2Frontal"] = {
["timers"] = {
38,
69,
123,
154,
208,
},
["group"] = "Coiled Altar",
["isConditional"] = {
["func"] = "return function() if UnitGroupRolesAssigned(\"player\") ~= \"TANK\" then return true end local threat = UnitThreatSituation(\"player\", \"boss2\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you are not a tank or have threat on boss2.",
},
["encID"] = 3429,
["internalID"] = "P2Frontal",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["spellID"] = 1286620,
["id"] = 12,
["text"] = "Frontal",
["enabled"] = false,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P2 Frontal",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["sticky"] = 0,
["phase"] = 2,
["DisplayType"] = "Text",
},
["P1Soak"] = {
["timers"] = {
48,
133,
},
["group"] = "Coiled Altar",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3429,
["internalID"] = "P1Soak",
["TTS"] = true,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Soak",
["spellID"] = 1283489,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["name"] = "P1 Soak",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 8,
["DisplayType"] = "Text",
},
["MindControls"] = {
["timers"] = {
8.1,
44.7,
93.1,
129,
177.4,
},
["group"] = "Coiled Altar",
["encID"] = 3429,
["internalID"] = "MindControls",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 2,
["text"] = "Mind Controls",
["spellID"] = 1285643,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P2 Mind Controls",
["Version"] = {
{
["group"] = "Coiled Altar",
["name"] = "P2 Mind Controls",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 10,
["DisplayType"] = "Text",
},
["EternalNightfallAbsorb"] = {
["extraOptions"] = {
{
["set"] = "return function(NSI, r, g, b, a) for difficultyID = 15, 16 do NSRT.EncounterAlerts[3429][difficultyID].EternalNightfallAbsorb.BarColor = {r, g, b, a} end NSI:UpdateCoiledAltarEternalNightfall() end",
["Type"] = "Color",
["get"] = "return function() local alert = NSRT.EncounterAlerts[3429][16].EternalNightfallAbsorb local c = alert.BarColor or NSRT.ReminderSettings.BarSettings.barColors return c[1], c[2], c[3], c[4] end",
["label"] = "Bar Color",
},
{
["min"] = 50,
["Type"] = "Slider",
["label"] = "Bar Width",
["max"] = 600,
["set"] = "return function(NSI, value) for difficultyID = 15, 16 do NSRT.EncounterAlerts[3429][difficultyID].EternalNightfallAbsorb.BarWidth = value end NSI:UpdateCoiledAltarEternalNightfall() end",
["step"] = 1,
["get"] = "return function() local alert = NSRT.EncounterAlerts[3429][16].EternalNightfallAbsorb return alert.BarWidth or NSRT.ReminderSettings.BarSettings.Width end",
},
{
["min"] = 10,
["Type"] = "Slider",
["label"] = "Bar Height",
["max"] = 100,
["set"] = "return function(NSI, value) for difficultyID = 15, 16 do NSRT.EncounterAlerts[3429][difficultyID].EternalNightfallAbsorb.BarHeight = value end NSI:UpdateCoiledAltarEternalNightfall() end",
["step"] = 1,
["get"] = "return function() local alert = NSRT.EncounterAlerts[3429][16].EternalNightfallAbsorb return alert.BarHeight or NSRT.ReminderSettings.BarSettings.Height end",
},
},
["xOffset"] = 0,
["yOffset"] = -300,
["customIcon"] = 1286918,
["internalID"] = "EternalNightfallAbsorb",
["enabled"] = true,
["text"] = "",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Eternal Nightfall Absorb",
["IsAlert"] = true,
["DisplayType"] = "Bar",
["timers"] = {
},
["group"] = "Coiled Altar",
["BarHeight"] = 40,
["encID"] = 3429,
["Preview"] = "return function(NSI)\n        NSI:PreviewCoiledAltarEternalNightfall()\n    end",
["TTSTimer"] = 15,
["BlockCopy"] = true,
["BarWidth"] = 300,
["id"] = 23,
["isSpecialDisplay"] = true,
["BarColor"] = {
0.6235,
0.251,
1,
1,
},
["phase"] = 1,
["difficulties"] = {
15,
16,
},
["TTS"] = false,
["Version"] = {
{
["group"] = "Coiled Altar",
},
{
["customIcon"] = 1286918,
},
["versionNumber"] = 2,
},
["Anchor"] = "TOP",
["relativeTo"] = "TOP",
["sticky"] = 0,
["dur"] = 15,
["NoEdit"] = true,
},
["P3Taunt"] = {
["timers"] = {
36,
67,
100.4,
137,
168.1,
200.3,
},
["group"] = "Coiled Altar",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat < 2 end",
["text"] = "This Alert only shows if you do not have threat on boss1.",
},
["customIcon"] = 355,
["encID"] = 3429,
["internalID"] = "P3Taunt",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["TTS"] = true,
["TTSTimer"] = 0,
["dur"] = 6,
["enabled"] = false,
["phase"] = 3,
["text"] = "Taunt",
["textColors"] = {
0,
1,
0,
1,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["isTaunt"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["name"] = "P3 Taunt",
["sticky"] = 3,
["id"] = 39,
["DisplayType"] = "Text",
},
["P2Taunt"] = {
["timers"] = {
38.6,
69.5,
123.6,
154.5,
208.6,
},
["group"] = "Coiled Altar",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss2\") return threat and threat < 2 end",
["text"] = "This Alert only shows if you do not have threat on boss2.",
},
["customIcon"] = 355,
["encID"] = 3429,
["internalID"] = "P2Taunt",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["TTS"] = true,
["TTSTimer"] = 0,
["dur"] = 6,
["enabled"] = false,
["phase"] = 2,
["text"] = "Taunt",
["textColors"] = {
0,
1,
0,
1,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["isTaunt"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["name"] = "P2 Taunt",
["sticky"] = 3,
["id"] = 14,
["DisplayType"] = "Text",
},
["P3OrbDeadline"] = {
["timers"] = {
29.5,
60.5,
93.9,
130.5,
161.6,
193.8,
},
["group"] = "Coiled Altar",
["customIcon"] = 1299838,
["encID"] = 3429,
["internalID"] = "P3OrbDeadline",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 3,
["difficulties"] = {
16,
},
["id"] = 28,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
{
["customIcon"] = 1299838,
},
["versionNumber"] = 2,
},
["name"] = "P3 Orb deadline",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["text"] = "Orb deadline",
["DisplayType"] = "Text",
},
["DebuffCircle"] = {
["isSpecialDisplay"] = true,
["group"] = "Coiled Altar",
["CircleTexture"] = "Interface\\AddOns\\NorthernSkyRaidTools\\Media\\Textures\\circle_8px.png",
["encID"] = 3429,
["Preview"] = "return function(NSI)\n        local alert = NSRT.EncounterAlerts[3429][16].DebuffCircle\n        NSI:PreviewReminderCircle(\"CoiledAltarDebuffCirclePreview\", 5, alert.CircleColor, alert.CircleTexture)\n    end",
["internalID"] = "DebuffCircle",
["CircleColor"] = {
1,
0,
0,
1,
},
["TTS"] = false,
["NoEdit"] = true,
["BlockCopy"] = true,
["phase"] = 1,
["id"] = 21,
["difficulties"] = {
14,
15,
16,
},
["text"] = "",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["timers"] = {
},
["name"] = "Orb/Bomb Circle",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["extraOptions"] = {
{
["set"] = "return function(NSI, r, g, b, a) for difficultyID = 14, 16 do NSRT.EncounterAlerts[3429][difficultyID].DebuffCircle.CircleColor = {r, g, b, a} end NSI:UpdateCoiledAltarDebuffCircle() NSI:UpdateReminderCirclePreview(\"CoiledAltarDebuffCirclePreview\", NSRT.EncounterAlerts[3429][16].DebuffCircle.CircleColor, NSRT.EncounterAlerts[3429][16].DebuffCircle.CircleTexture) end",
["Type"] = "Color",
["get"] = "return function() local alert = NSRT.EncounterAlerts[3429][16].DebuffCircle local c = alert.CircleColor or {1, 0, 0, 1} return c[1], c[2], c[3], c[4] end",
["label"] = "Ring Color",
},
{
["Type"] = "Dropdown",
["values"] = "return function() return {\n                {label = \"2 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_2px.png\"},\n                {label = \"5 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_5px.png\"},\n                {label = \"8 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_8px.png\"},\n                {label = \"10 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_10px.png\"},\n                {label = \"15 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_15px.png\"},\n            } end",
["label"] = "Ring Size",
["set"] = "return function(NSI, value) for difficultyID = 14, 16 do NSRT.EncounterAlerts[3429][difficultyID].DebuffCircle.CircleTexture = value end NSI:UpdateCoiledAltarDebuffCircle() NSI:UpdateReminderCirclePreview(\"CoiledAltarDebuffCirclePreview\", NSRT.EncounterAlerts[3429][16].DebuffCircle.CircleColor, value) end",
["get"] = "return function() local alert = NSRT.EncounterAlerts[3429][16].DebuffCircle return alert.CircleTexture or \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_8px.png\" end",
},
},
["DisplayType"] = "Circle",
},
["InterruptAdds"] = {
["timers"] = {
13,
46,
98,
131,
183,
},
["group"] = "Coiled Altar",
["encID"] = 3429,
["internalID"] = "InterruptAdds",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 2,
["text"] = "Ghosts",
["spellID"] = 1286399,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P2 Interrupt Adds",
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 25,
["DisplayType"] = "Text",
},
["P1OrbDeadline"] = {
["timers"] = {
17,
34,
54,
71.1,
102.1,
119,
139,
156.1,
},
["group"] = "Coiled Altar",
["customIcon"] = 1299838,
["encID"] = 3429,
["internalID"] = "P1OrbDeadline",
["enabled"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Orb deadline",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["id"] = 4,
["name"] = "Orb deadline",
["IsAlert"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
{
["customIcon"] = 1299838,
},
["versionNumber"] = 2,
},
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Text",
},
["P2Shield"] = {
["timers"] = {
70,
155,
},
["group"] = "Coiled Altar",
["encID"] = 3429,
["internalID"] = "P2Shield",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 2,
["text"] = "Shield",
["spellID"] = 1286918,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P2 Shield",
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 18,
["DisplayType"] = "Text",
},
["P3Shield"] = {
["timers"] = {
38,
134.8,
},
["group"] = "Coiled Altar",
["encID"] = 3429,
["internalID"] = "P3Shield",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 3,
["text"] = "Shield",
["spellID"] = 1310752,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P3 Shield",
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 32,
["DisplayType"] = "Text",
},
["P1Taunt"] = {
["timers"] = {
23.5,
40.5,
60.5,
77.6,
108.6,
125.5,
145.5,
162.6,
},
["group"] = "Coiled Altar",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat < 2 end",
["text"] = "This Alert only shows if you do not have threat on boss1.",
},
["customIcon"] = 355,
["encID"] = 3429,
["internalID"] = "P1Taunt",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["TTS"] = true,
["TTSTimer"] = 0,
["dur"] = 6,
["enabled"] = false,
["phase"] = 1,
["text"] = "Taunt",
["textColors"] = {
0,
1,
0,
1,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["isTaunt"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["name"] = "P1 Taunt",
["sticky"] = 3,
["id"] = 6,
["DisplayType"] = "Text",
},
["P3Soak"] = {
["timers"] = {
21.8,
86.3,
186.3,
},
["group"] = "Coiled Altar",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3429,
["internalID"] = "P3Soak",
["TTS"] = true,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 3,
["text"] = "Soak",
["spellID"] = 1299266,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["name"] = "P3 Soak",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 30,
["DisplayType"] = "Text",
},
["P3Frontal"] = {
["timers"] = {
35.5,
66.5,
99.9,
136.5,
167.6,
199.8,
},
["group"] = "Coiled Altar",
["isConditional"] = {
["func"] = "return function() if UnitGroupRolesAssigned(\"player\") ~= \"TANK\" then return true end local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you are not a tank or have threat on boss1.",
},
["encID"] = 3429,
["internalID"] = "P3Frontal",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["spellID"] = 1307292,
["id"] = 27,
["text"] = "Frontal",
["enabled"] = false,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P3 Frontal",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["sticky"] = 0,
["phase"] = 3,
["DisplayType"] = "Text",
},
["P3InterruptAdds"] = {
["timers"] = {
62.2,
159.2,
},
["group"] = "Coiled Altar",
["encID"] = 3429,
["internalID"] = "P3InterruptAdds",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["spellID"] = 1286399,
["id"] = 35,
["difficulties"] = {
16,
},
["phase"] = 3,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["name"] = "P3 Interrupt Adds",
["IsAlert"] = true,
["text"] = "Ghosts",
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["P2Debuffs"] = {
["timers"] = {
20,
57,
105,
142,
190,
},
["group"] = "Coiled Altar",
["loadConditions"] = {
["Roles"] = {
["DAMAGER"] = true,
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3429,
["internalID"] = "P2Debuffs",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 2,
["text"] = "Debuffs",
["spellID"] = 1286895,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["name"] = "P2 Debuffs",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 16,
["DisplayType"] = "Text",
},
["P3Debuffs"] = {
["timers"] = {
26.8,
71.1,
109,
172.2,
},
["group"] = "Coiled Altar",
["loadConditions"] = {
["Roles"] = {
["DAMAGER"] = true,
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3429,
["internalID"] = "P3Debuffs",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 3,
["text"] = "Debuffs",
["spellID"] = 1310881,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["name"] = "P3 Debuffs",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 34,
["DisplayType"] = "Text",
},
["P3MindControls"] = {
["timers"] = {
57.8,
154.1,
201.9,
},
["group"] = "Coiled Altar",
["encID"] = 3429,
["internalID"] = "P3MindControls",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 3,
["text"] = "Mind Controls",
["spellID"] = 1297445,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P3 Mind Controls",
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 37,
["DisplayType"] = "Text",
},
["InterruptAssignments"] = {
["extraOptions"] = {
{
["height"] = 120,
["Type"] = "Label",
["text"] = "The first interrupt line will be assigned to the add with no raidmarker. The second interrupt line will be assigned to the add with any raidmarker. The usual strat is that you have one person instantly putting a raidmarker on the ranged add so the correct box shows and counts up.\n\nOptionally, add two more lines for separate P3 assignments, or four more lines for separate assignments for the first and second P3 add waves. Each pair follows the same order: unmarked add first, marked add second.",
},
{
["min"] = 8,
["Type"] = "Slider",
["label"] = "Number Font Size",
["max"] = 40,
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3429][16].InterruptAssignments.NumberFontSize = value NSI:UpdateCoiledAltarInterruptDisplay() NSI:UpdateCoiledAltarInterruptPreview() end",
["step"] = 1,
["get"] = "return function(NSI) local alert = NSRT.EncounterAlerts[3429][16].InterruptAssignments return alert.NumberFontSize or alert.FontSize or 12 end",
},
{
["min"] = 8,
["Type"] = "Slider",
["label"] = "Name Font Size",
["max"] = 40,
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3429][16].InterruptAssignments.NameFontSize = value NSI:UpdateCoiledAltarInterruptDisplay() NSI:UpdateCoiledAltarInterruptPreview() end",
["step"] = 1,
["get"] = "return function(NSI) local alert = NSRT.EncounterAlerts[3429][16].InterruptAssignments return alert.NameFontSize or alert.FontSize or 12 end",
},
{
["min"] = 30,
["Type"] = "Slider",
["label"] = "Box Size",
["max"] = 150,
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3429][16].InterruptAssignments.BoxSize = value NSI:UpdateCoiledAltarInterruptDisplay() NSI:UpdateCoiledAltarInterruptPreview() end",
["step"] = 1,
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3429][16].InterruptAssignments.BoxSize or 30 end",
},
{
["Type"] = "Dropdown",
["values"] = "return function()\n                    return {\n                        {label = \"Top\", value = \"TOP\"},\n                        {label = \"Center\", value = \"CENTER\"},\n                        {label = \"Left\", value = \"LEFT\"},\n                        {label = \"Right\", value = \"RIGHT\"},\n                        {label = \"Bottom\", value = \"BOTTOM\"},\n                    }\n                end",
["label"] = "Nameplate Anchor",
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3429][16].InterruptAssignments.NameplateAnchor = value NSI:UpdateCoiledAltarInterruptDisplay() NSI:UpdateCoiledAltarInterruptPreview() end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3429][16].InterruptAssignments.NameplateAnchor or \"TOP\" end",
},
{
["min"] = -200,
["Type"] = "Slider",
["label"] = "Nameplate X Offset",
["max"] = 200,
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3429][16].InterruptAssignments.NameplateXOffset = value NSI:UpdateCoiledAltarInterruptDisplay() NSI:UpdateCoiledAltarInterruptPreview() end",
["step"] = 1,
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3429][16].InterruptAssignments.NameplateXOffset or 0 end",
},
{
["min"] = -200,
["Type"] = "Slider",
["label"] = "Nameplate Y Offset",
["max"] = 200,
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3429][16].InterruptAssignments.NameplateYOffset = value NSI:UpdateCoiledAltarInterruptDisplay() NSI:UpdateCoiledAltarInterruptPreview() end",
["step"] = 1,
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3429][16].InterruptAssignments.NameplateYOffset or 0 end",
},
{
["Type"] = "Checkbox",
["label"] = "Show All",
["tooltip"] = {
["title"] = "Show All",
["desc"] = "Show the assignment boxes for both interrupt lines.",
},
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3429][16].InterruptAssignments.ShowAll = value NSI:UpdateCoiledAltarInterruptDisplay() NSI:UpdateCoiledAltarInterruptPreview() end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3429][16].InterruptAssignments.ShowAll or false end",
},
{
["Type"] = "Checkbox",
["label"] = "Display static box",
["tooltip"] = {
["title"] = "Display static box",
["desc"] = "Use the global Interrupt Display settings for the static box.",
},
["set"] = "return function(NSI, value) local alert = NSRT.EncounterAlerts[3429][16].InterruptAssignments alert.DisplayStaticBox = value NSI:UpdateCoiledAltarInterruptDisplay() NSI:UpdateCoiledAltarInterruptPreview() end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3429][16].InterruptAssignments.DisplayStaticBox or false end",
},
{
["Type"] = "Checkbox",
["label"] = "Hide nameplate box",
["tooltip"] = {
["title"] = "Hide nameplate box",
["desc"] = "Hide the nameplate box while keeping the static box visible.",
},
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3429][16].InterruptAssignments.HideNameplateBox = value NSI:UpdateCoiledAltarInterruptDisplay() end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3429][16].InterruptAssignments.HideNameplateBox or false end",
},
},
["NumberFontSize"] = 12,
["customIcon"] = 6552,
["ShowAll"] = false,
["internalID"] = "InterruptAssignments",
["TTS"] = false,
["NameplateYOffset"] = 0,
["text"] = "Interrupts",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Interrupt Assignments",
["IsAlert"] = true,
["NameplateAnchor"] = "TOP",
["DisplayType"] = "Text",
["timers"] = {
},
["group"] = "Coiled Altar",
["HideNameplateBox"] = false,
["encID"] = 3429,
["Preview"] = "return function(NSI)\n            if NSI:PreviewCoiledAltarInterruptDisplay() then\n                local alert = NSRT.EncounterAlerts[3429][16].InterruptAssignments\n                local message = alert.DisplayStaticBox and \"|cFF00FFFFNSRT:|r the live display uses the global Interrupt Display settings during phases 2 and 3.\" or \"|cFF00FFFFNSRT:|r the live display is shown on add nameplates during phases 2 and 3. Its size may also change with the nameplate frame scale.\"\n                print(NSI:Loc(message))\n            end\n        end",
["DisplayStaticBox"] = false,
["TTSTimer"] = 35,
["BlockCopy"] = true,
["BoxSize"] = 30,
["phase"] = 2,
["id"] = 43,
["difficulties"] = {
16,
},
["isSpecialDisplay"] = true,
["NameFontSize"] = 12,
["pinned"] = true,
["Version"] = {
{
["BoxSize"] = 30,
},
{
["NumberFontSize"] = 12,
["NameFontSize"] = 12,
},
{
["group"] = "Coiled Altar",
},
{
["customIcon"] = 6552,
},
["versionNumber"] = 4,
},
["enabled"] = true,
["NameplateXOffset"] = 0,
["sticky"] = 0,
["dur"] = 35,
["NoEdit"] = true,
},
},
[15] = {
["P2_5WrongTarget"] = {
["isSpecialDisplay"] = true,
["group"] = "Coiled Altar",
["HideTimer"] = true,
["encID"] = 3429,
["internalID"] = "P2_5WrongTarget",
["BlockCopy"] = true,
["TTS"] = false,
["TTSTimer"] = 50,
["dur"] = 50,
["id"] = 42,
["phase"] = 2.5,
["text"] = "WRONG TARGET",
["timers"] = {
0,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["textColors"] = {
1,
0,
0,
1,
},
["name"] = "Wrong Target",
["IsAlert"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["sticky"] = 50,
["enabled"] = true,
["DisplayType"] = "Text",
},
["P1Frontal"] = {
["timers"] = {
23,
40,
60,
77.1,
108.1,
125,
145,
162.1,
},
["group"] = "Coiled Altar",
["isConditional"] = {
["func"] = "return function() if UnitGroupRolesAssigned(\"player\") ~= \"TANK\" then return true end local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you are not a tank or have threat on boss1.",
},
["encID"] = 3429,
["internalID"] = "P1Frontal",
["TTS"] = true,
["TTSTimer"] = 8,
["dur"] = 8,
["spellID"] = 1299684,
["id"] = 1,
["text"] = "Frontal",
["enabled"] = false,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P1 Frontal",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["sticky"] = 0,
["phase"] = 1,
["DisplayType"] = "Text",
},
["P2Frontal"] = {
["timers"] = {
38,
69,
123,
154,
},
["group"] = "Coiled Altar",
["isConditional"] = {
["func"] = "return function() if UnitGroupRolesAssigned(\"player\") ~= \"TANK\" then return true end local threat = UnitThreatSituation(\"player\", \"boss2\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you are not a tank or have threat on boss2.",
},
["encID"] = 3429,
["internalID"] = "P2Frontal",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["spellID"] = 1286620,
["id"] = 11,
["text"] = "Frontal",
["enabled"] = false,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P2 Frontal",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["sticky"] = 0,
["phase"] = 2,
["DisplayType"] = "Text",
},
["P1Soak"] = {
["timers"] = {
48,
133,
},
["group"] = "Coiled Altar",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3429,
["internalID"] = "P1Soak",
["TTS"] = true,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Soak",
["spellID"] = 1283489,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["name"] = "P1 Soak",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 7,
["DisplayType"] = "Text",
},
["MindControls"] = {
["timers"] = {
8.1,
44.7,
93.1,
129,
},
["group"] = "Coiled Altar",
["encID"] = 3429,
["internalID"] = "MindControls",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 2,
["text"] = "Mind Controls",
["spellID"] = 1285643,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P2 Mind Controls",
["Version"] = {
{
["group"] = "Coiled Altar",
["name"] = "P2 Mind Controls",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 9,
["DisplayType"] = "Text",
},
["P3Taunt"] = {
["timers"] = {
39.6,
71.8,
106.3,
144.2,
176.4,
},
["group"] = "Coiled Altar",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat < 2 end",
["text"] = "This Alert only shows if you do not have threat on boss1.",
},
["customIcon"] = 355,
["encID"] = 3429,
["internalID"] = "P3Taunt",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["TTS"] = true,
["TTSTimer"] = 0,
["dur"] = 6,
["enabled"] = false,
["phase"] = 3,
["text"] = "Taunt",
["textColors"] = {
0,
1,
0,
1,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["isTaunt"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["name"] = "P3 Taunt",
["sticky"] = 3,
["id"] = 38,
["DisplayType"] = "Text",
},
["P2Taunt"] = {
["timers"] = {
38.6,
69.5,
123.6,
154.5,
},
["group"] = "Coiled Altar",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss2\") return threat and threat < 2 end",
["text"] = "This Alert only shows if you do not have threat on boss2.",
},
["customIcon"] = 355,
["encID"] = 3429,
["internalID"] = "P2Taunt",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["TTS"] = true,
["TTSTimer"] = 0,
["dur"] = 6,
["enabled"] = false,
["phase"] = 2,
["text"] = "Taunt",
["textColors"] = {
0,
1,
0,
1,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["isTaunt"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["name"] = "P2 Taunt",
["sticky"] = 3,
["id"] = 13,
["DisplayType"] = "Text",
},
["InterruptAdds"] = {
["timers"] = {
13,
46.1,
98,
},
["group"] = "Coiled Altar",
["encID"] = 3429,
["internalID"] = "InterruptAdds",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 2,
["text"] = "Ghosts",
["spellID"] = 1286399,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P2 Interrupt Adds",
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 24,
["DisplayType"] = "Text",
},
["DebuffCircle"] = {
["isSpecialDisplay"] = true,
["group"] = "Coiled Altar",
["CircleTexture"] = "Interface\\AddOns\\NorthernSkyRaidTools\\Media\\Textures\\circle_8px.png",
["encID"] = 3429,
["Preview"] = "return function(NSI)\n        local alert = NSRT.EncounterAlerts[3429][16].DebuffCircle\n        NSI:PreviewReminderCircle(\"CoiledAltarDebuffCirclePreview\", 5, alert.CircleColor, alert.CircleTexture)\n    end",
["internalID"] = "DebuffCircle",
["CircleColor"] = {
1,
0,
0,
1,
},
["TTS"] = false,
["NoEdit"] = true,
["BlockCopy"] = true,
["phase"] = 1,
["id"] = 20,
["difficulties"] = {
14,
15,
16,
},
["text"] = "",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["timers"] = {
},
["name"] = "Orb/Bomb Circle",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["extraOptions"] = {
{
["set"] = "return function(NSI, r, g, b, a) for difficultyID = 14, 16 do NSRT.EncounterAlerts[3429][difficultyID].DebuffCircle.CircleColor = {r, g, b, a} end NSI:UpdateCoiledAltarDebuffCircle() NSI:UpdateReminderCirclePreview(\"CoiledAltarDebuffCirclePreview\", NSRT.EncounterAlerts[3429][16].DebuffCircle.CircleColor, NSRT.EncounterAlerts[3429][16].DebuffCircle.CircleTexture) end",
["Type"] = "Color",
["get"] = "return function() local alert = NSRT.EncounterAlerts[3429][16].DebuffCircle local c = alert.CircleColor or {1, 0, 0, 1} return c[1], c[2], c[3], c[4] end",
["label"] = "Ring Color",
},
{
["Type"] = "Dropdown",
["values"] = "return function() return {\n                {label = \"2 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_2px.png\"},\n                {label = \"5 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_5px.png\"},\n                {label = \"8 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_8px.png\"},\n                {label = \"10 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_10px.png\"},\n                {label = \"15 px\", value = \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_15px.png\"},\n            } end",
["label"] = "Ring Size",
["set"] = "return function(NSI, value) for difficultyID = 14, 16 do NSRT.EncounterAlerts[3429][difficultyID].DebuffCircle.CircleTexture = value end NSI:UpdateCoiledAltarDebuffCircle() NSI:UpdateReminderCirclePreview(\"CoiledAltarDebuffCirclePreview\", NSRT.EncounterAlerts[3429][16].DebuffCircle.CircleColor, value) end",
["get"] = "return function() local alert = NSRT.EncounterAlerts[3429][16].DebuffCircle return alert.CircleTexture or \"Interface\\\\AddOns\\\\NorthernSkyRaidTools\\\\Media\\\\Textures\\\\circle_8px.png\" end",
},
},
["DisplayType"] = "Circle",
},
["P2Shield"] = {
["timers"] = {
70,
155,
},
["group"] = "Coiled Altar",
["encID"] = 3429,
["internalID"] = "P2Shield",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 2,
["text"] = "Shield",
["spellID"] = 1286918,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P2 Shield",
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 17,
["DisplayType"] = "Text",
},
["P3Shield"] = {
["timers"] = {
41.9,
141.8,
},
["group"] = "Coiled Altar",
["encID"] = 3429,
["internalID"] = "P3Shield",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 3,
["text"] = "Shield",
["spellID"] = 1310752,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P3 Shield",
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 31,
["DisplayType"] = "Text",
},
["EternalNightfallAbsorb"] = {
["extraOptions"] = {
{
["set"] = "return function(NSI, r, g, b, a) for difficultyID = 15, 16 do NSRT.EncounterAlerts[3429][difficultyID].EternalNightfallAbsorb.BarColor = {r, g, b, a} end NSI:UpdateCoiledAltarEternalNightfall() end",
["Type"] = "Color",
["get"] = "return function() local alert = NSRT.EncounterAlerts[3429][16].EternalNightfallAbsorb local c = alert.BarColor or NSRT.ReminderSettings.BarSettings.barColors return c[1], c[2], c[3], c[4] end",
["label"] = "Bar Color",
},
{
["min"] = 50,
["Type"] = "Slider",
["label"] = "Bar Width",
["max"] = 600,
["set"] = "return function(NSI, value) for difficultyID = 15, 16 do NSRT.EncounterAlerts[3429][difficultyID].EternalNightfallAbsorb.BarWidth = value end NSI:UpdateCoiledAltarEternalNightfall() end",
["step"] = 1,
["get"] = "return function() local alert = NSRT.EncounterAlerts[3429][16].EternalNightfallAbsorb return alert.BarWidth or NSRT.ReminderSettings.BarSettings.Width end",
},
{
["min"] = 10,
["Type"] = "Slider",
["label"] = "Bar Height",
["max"] = 100,
["set"] = "return function(NSI, value) for difficultyID = 15, 16 do NSRT.EncounterAlerts[3429][difficultyID].EternalNightfallAbsorb.BarHeight = value end NSI:UpdateCoiledAltarEternalNightfall() end",
["step"] = 1,
["get"] = "return function() local alert = NSRT.EncounterAlerts[3429][16].EternalNightfallAbsorb return alert.BarHeight or NSRT.ReminderSettings.BarSettings.Height end",
},
},
["xOffset"] = 0,
["yOffset"] = -300,
["customIcon"] = 1286918,
["internalID"] = "EternalNightfallAbsorb",
["enabled"] = true,
["text"] = "",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Eternal Nightfall Absorb",
["IsAlert"] = true,
["DisplayType"] = "Bar",
["timers"] = {
},
["group"] = "Coiled Altar",
["BarHeight"] = 40,
["encID"] = 3429,
["Preview"] = "return function(NSI)\n        NSI:PreviewCoiledAltarEternalNightfall()\n    end",
["TTSTimer"] = 15,
["BlockCopy"] = true,
["BarWidth"] = 300,
["id"] = 22,
["isSpecialDisplay"] = true,
["BarColor"] = {
0.6235,
0.251,
1,
1,
},
["phase"] = 1,
["difficulties"] = {
15,
16,
},
["TTS"] = false,
["Version"] = {
{
["group"] = "Coiled Altar",
},
{
["customIcon"] = 1286918,
},
["versionNumber"] = 2,
},
["Anchor"] = "TOP",
["relativeTo"] = "TOP",
["sticky"] = 0,
["dur"] = 15,
["NoEdit"] = true,
},
["P1OrbDeadline"] = {
["timers"] = {
17,
34,
54,
71.1,
102.1,
119,
139,
156.1,
},
["group"] = "Coiled Altar",
["customIcon"] = 1299838,
["encID"] = 3429,
["internalID"] = "P1OrbDeadline",
["enabled"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Orb deadline",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["id"] = 3,
["name"] = "Orb deadline",
["IsAlert"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
{
["customIcon"] = 1299838,
},
["versionNumber"] = 2,
},
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Text",
},
["P1Taunt"] = {
["timers"] = {
23.5,
40.5,
60.5,
77.6,
108.6,
125.5,
145.5,
162.6,
},
["group"] = "Coiled Altar",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat < 2 end",
["text"] = "This Alert only shows if you do not have threat on boss1.",
},
["customIcon"] = 355,
["encID"] = 3429,
["internalID"] = "P1Taunt",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["TTS"] = true,
["TTSTimer"] = 0,
["dur"] = 6,
["enabled"] = false,
["phase"] = 1,
["text"] = "Taunt",
["textColors"] = {
0,
1,
0,
1,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["isTaunt"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["name"] = "P1 Taunt",
["sticky"] = 3,
["id"] = 5,
["DisplayType"] = "Text",
},
["P3Soak"] = {
["timers"] = {
22.3,
191.3,
},
["group"] = "Coiled Altar",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3429,
["internalID"] = "P3Soak",
["TTS"] = true,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 3,
["text"] = "Soak",
["spellID"] = 1299266,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["name"] = "P3 Soak",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 29,
["DisplayType"] = "Text",
},
["P2Debuffs"] = {
["timers"] = {
24.1,
62.1,
109,
147,
},
["group"] = "Coiled Altar",
["loadConditions"] = {
["Roles"] = {
["DAMAGER"] = true,
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3429,
["internalID"] = "P2Debuffs",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 2,
["text"] = "Debuffs",
["spellID"] = 1286895,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["name"] = "P2 Debuffs",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 15,
["DisplayType"] = "Text",
},
["P3Debuffs"] = {
["timers"] = {
31.2,
81.8,
115.1,
181.8,
},
["group"] = "Coiled Altar",
["loadConditions"] = {
["Roles"] = {
["DAMAGER"] = true,
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3429,
["internalID"] = "P3Debuffs",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 3,
["text"] = "Debuffs",
["spellID"] = 1310881,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["name"] = "P3 Debuffs",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 33,
["DisplayType"] = "Text",
},
["P3Frontal"] = {
["timers"] = {
36.3,
68.5,
103,
140.9,
173.1,
},
["group"] = "Coiled Altar",
["isConditional"] = {
["func"] = "return function() if UnitGroupRolesAssigned(\"player\") ~= \"TANK\" then return true end local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you are not a tank or have threat on boss1.",
},
["encID"] = 3429,
["internalID"] = "P3Frontal",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["spellID"] = 1307292,
["id"] = 26,
["text"] = "Frontal",
["enabled"] = false,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P3 Frontal",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["sticky"] = 0,
["phase"] = 3,
["DisplayType"] = "Text",
},
["P3MindControls"] = {
["timers"] = {
66.3,
167.5,
},
["group"] = "Coiled Altar",
["encID"] = 3429,
["internalID"] = "P3MindControls",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 3,
["text"] = "Mind Controls",
["spellID"] = 1297445,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P3 Mind Controls",
["Version"] = {
{
["group"] = "Coiled Altar",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 36,
["DisplayType"] = "Text",
},
},
},
[3183] = {
},
[3497] = {
[15] = {
["FrostfireVolley"] = {
["timers"] = {
},
["group"] = "Scrollsage Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss4\" end",
["text"] = "This Alert only shows if Scrollsage Iku was empowered",
},
["encID"] = 3497,
["internalID"] = "FrostfireVolley",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Frostfire Debuffs",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
8,
35,
},
{
8,
35,
},
{
8,
35,
},
},
["id"] = 5,
["name"] = "Frostfire Volley",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1295891,
},
["ShellSpinTrader"] = {
["timers"] = {
},
["group"] = "First Mate Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss1\" end",
["text"] = "This Alert only shows if Trader Gebbo was empowered",
},
["loadConditions"] = {
["Roles"] = {
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
[260] = true,
[261] = true,
[70] = true,
[251] = true,
[263] = true,
[577] = true,
[71] = true,
[269] = true,
[255] = true,
[270] = true,
[72] = true,
[252] = true,
[65] = true,
[259] = true,
[103] = true,
},
},
["encID"] = 3497,
["internalID"] = "ShellSpinTrader",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Bait",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
7,
39,
},
{
7,
39,
},
{
7,
39,
},
},
["id"] = 11,
["name"] = "Shell Spin - Trader Empowered",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1296062,
},
["Fish-Spawn"] = {
["timers"] = {
},
["group"] = "Trader Abilities",
["encID"] = 3497,
["internalID"] = "Fish-Spawn",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Fish Spawn",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
34,
},
{
94,
},
{
94,
},
{
94,
},
},
["id"] = 15,
["name"] = "Fish Spawn",
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1295817,
},
["ShellSpinNormal"] = {
["timers"] = {
},
["group"] = "First Mate Abilities",
["loadConditions"] = {
["Roles"] = {
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
[260] = true,
[261] = true,
[70] = true,
[251] = true,
[263] = true,
[577] = true,
[71] = true,
[269] = true,
[255] = true,
[270] = true,
[72] = true,
[252] = true,
[65] = true,
[259] = true,
[103] = true,
},
},
["encID"] = 3497,
["internalID"] = "ShellSpinNormal",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Bait",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
18,
34,
49,
},
{
78,
94,
109,
},
{
78,
94,
109,
},
{
78,
94,
109,
},
},
["id"] = 7,
["name"] = "Shell Spin Normal",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1296062,
},
["ShellSpinScroll"] = {
["timers"] = {
},
["group"] = "First Mate Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss4\" end",
["text"] = "This Alert only shows if Scrollsage Iku was empowered",
},
["loadConditions"] = {
["Roles"] = {
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
[260] = true,
[261] = true,
[70] = true,
[251] = true,
[263] = true,
[577] = true,
[71] = true,
[269] = true,
[255] = true,
[270] = true,
[72] = true,
[252] = true,
[65] = true,
[259] = true,
[103] = true,
},
},
["encID"] = 3497,
["internalID"] = "ShellSpinScroll",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Bait",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
17,
44,
},
{
17,
44,
},
{
17,
44,
},
},
["id"] = 9,
["name"] = "Shell Spin - Scroll Empowered",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1296062,
},
["TimeToThrowNonConditional"] = {
["timers"] = {
},
["group"] = "Trader Abilities",
["encID"] = 3497,
["internalID"] = "TimeToThrowNonConditional",
["TTS"] = false,
["TTSTimer"] = 7,
["dur"] = 7,
["phase"] = {
1,
2,
3,
},
["text"] = "Time to Throw",
["DisplayType"] = "Text",
["DefaultEnabled"] = false,
["phaseTimers"] = {
{
61,
},
{
121,
},
{
121,
},
},
["id"] = 25,
["name"] = "non-conditional Time to throw Fish",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1295817,
},
["MightyThud"] = {
["timers"] = {
},
["group"] = "First Mate Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss3\" end",
["text"] = "This Alert only shows if First Mate Nama was empowered",
},
["encID"] = 3497,
["internalID"] = "MightyThud",
["TTS"] = false,
["TTSTimer"] = 15.5,
["dur"] = 15.5,
["DisplayType"] = "Bar",
["id"] = 13,
["text"] = "Soaks",
["phase"] = {
1,
2,
3,
4,
},
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
17.6,
50.1,
},
{
17.6,
50.1,
},
{
17.6,
50.1,
},
},
["enabled"] = false,
["name"] = "Soaks",
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["Ticks"] = {
11.5,
13.5,
},
["spellID"] = 1296133,
},
["TimeToThrow"] = {
["timers"] = {
},
["group"] = "Trader Abilities",
["isConditional"] = {
["func"] = "return function() return C_ActionBar.HasExtraActionBar() end",
["text"] = "This Alert only shows if you are holding the fish at the time.",
},
["encID"] = 3497,
["internalID"] = "TimeToThrow",
["TTS"] = false,
["TTSTimer"] = 7,
["dur"] = 7,
["phase"] = {
1,
2,
3,
},
["text"] = "Time to Throw",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
61,
},
{
121,
},
{
121,
},
},
["id"] = 23,
["name"] = "Time to throw Fish",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1295817,
},
["ShreddingShards"] = {
["timers"] = {
},
["group"] = "Scrollsage Abilities",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss4\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss4.",
},
["encID"] = 3497,
["internalID"] = "ShreddingShards",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["DisplayType"] = "Text",
["id"] = 1,
["text"] = "Tank-Hit",
["enabled"] = false,
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
30,
},
{
30.5,
90.5,
},
{
30.5,
90.5,
},
{
30.5,
90.5,
},
},
["name"] = "Tank-Hit",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["phase"] = {
1,
2,
3,
4,
},
["spellID"] = 1295854,
},
["BlinkNova"] = {
["timers"] = {
},
["group"] = "Scrollsage Abilities",
["encID"] = 3497,
["internalID"] = "BlinkNova",
["enabled"] = false,
["TTSTimer"] = 9,
["dur"] = 9,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Blink Nova",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
17,
48,
},
{
77,
108,
},
{
77,
108,
},
{
77,
108,
},
},
["id"] = 3,
["name"] = "Blink Nova",
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1296021,
},
["ExplosiveSurprise"] = {
["timers"] = {
},
["group"] = "Trader Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss1\" end",
["text"] = "This Alert only shows if Trader Gebbo was empowered",
},
["encID"] = 3497,
["internalID"] = "ExplosiveSurprise",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Bomb inc",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
13,
45.5,
},
{
13,
45.5,
},
{
13,
45.5,
},
},
["id"] = 19,
["name"] = "Bomb Debuff",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1296249,
},
["MushroomBait"] = {
["timers"] = {
},
["group"] = "Trader Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss1\" end",
["text"] = "This Alert only shows if Trader Gebbo was empowered",
},
["loadConditions"] = {
["Roles"] = {
["RANGED"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3497,
["internalID"] = "MushroomBait",
["TTS"] = false,
["TTSTimer"] = 7,
["dur"] = 7,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Bait",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
10,
42,
},
{
10,
42,
},
{
10,
42,
},
},
["id"] = 17,
["name"] = "Mushroom Bait",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1292105,
},
["MushroomJump"] = {
["timers"] = {
},
["group"] = "Trader Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss1\" end",
["text"] = "This Alert only shows if Trader Gebbo was empowered",
},
["encID"] = 3497,
["internalID"] = "MushroomJump",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Jump",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
34,
65,
},
{
34,
65,
},
{
34,
65,
},
},
["id"] = 21,
["name"] = "Mushroom Jump",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1299855,
},
},
[16] = {
["FrostfireVolley"] = {
["timers"] = {
},
["group"] = "Scrollsage Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss4\" end",
["text"] = "This Alert only shows if Scrollsage Iku was empowered",
},
["encID"] = 3497,
["internalID"] = "FrostfireVolley",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Frostfire Debuffs",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
8,
35,
},
{
8,
35,
},
{
8,
35,
},
},
["id"] = 6,
["name"] = "Frostfire Volley",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1295891,
},
["ShellSpinTrader"] = {
["timers"] = {
},
["group"] = "First Mate Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss1\" end",
["text"] = "This Alert only shows if Trader Gebbo was empowered",
},
["loadConditions"] = {
["Roles"] = {
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
[260] = true,
[261] = true,
[70] = true,
[251] = true,
[263] = true,
[577] = true,
[71] = true,
[269] = true,
[255] = true,
[270] = true,
[72] = true,
[252] = true,
[65] = true,
[259] = true,
[103] = true,
},
},
["encID"] = 3497,
["internalID"] = "ShellSpinTrader",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Bait",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
7,
39,
},
{
7,
39,
},
{
7,
39,
},
},
["id"] = 12,
["name"] = "Shell Spin - Trader Empowered",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1296062,
},
["Fish-Spawn"] = {
["timers"] = {
},
["group"] = "Trader Abilities",
["encID"] = 3497,
["internalID"] = "Fish-Spawn",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Fish Spawn",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
34,
},
{
94,
},
{
94,
},
{
94,
},
},
["id"] = 16,
["name"] = "Fish Spawn",
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1295817,
},
["ShellSpinNormal"] = {
["timers"] = {
},
["group"] = "First Mate Abilities",
["loadConditions"] = {
["Roles"] = {
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
[260] = true,
[261] = true,
[70] = true,
[251] = true,
[263] = true,
[577] = true,
[71] = true,
[269] = true,
[255] = true,
[270] = true,
[72] = true,
[252] = true,
[65] = true,
[259] = true,
[103] = true,
},
},
["encID"] = 3497,
["internalID"] = "ShellSpinNormal",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Bait",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
18,
34,
49,
},
{
78,
94,
109,
},
{
78,
94,
109,
},
{
78,
94,
109,
},
},
["id"] = 8,
["name"] = "Shell Spin Normal",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1296062,
},
["ShellSpinScroll"] = {
["timers"] = {
},
["group"] = "First Mate Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss4\" end",
["text"] = "This Alert only shows if Scrollsage Iku was empowered",
},
["loadConditions"] = {
["Roles"] = {
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
[260] = true,
[261] = true,
[70] = true,
[251] = true,
[263] = true,
[577] = true,
[71] = true,
[269] = true,
[255] = true,
[270] = true,
[72] = true,
[252] = true,
[65] = true,
[259] = true,
[103] = true,
},
},
["encID"] = 3497,
["internalID"] = "ShellSpinScroll",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Bait",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
17,
44,
},
{
17,
44,
},
{
17,
44,
},
},
["id"] = 10,
["name"] = "Shell Spin - Scroll Empowered",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1296062,
},
["TimeToThrowNonConditional"] = {
["timers"] = {
},
["group"] = "Trader Abilities",
["encID"] = 3497,
["internalID"] = "TimeToThrowNonConditional",
["TTS"] = false,
["TTSTimer"] = 7,
["dur"] = 7,
["phase"] = {
1,
2,
3,
},
["text"] = "Time to Throw",
["DisplayType"] = "Text",
["DefaultEnabled"] = false,
["phaseTimers"] = {
{
61,
},
{
121,
},
{
121,
},
},
["id"] = 26,
["name"] = "non-conditional Time to throw Fish",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1295817,
},
["MightyThud"] = {
["timers"] = {
},
["group"] = "First Mate Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss3\" end",
["text"] = "This Alert only shows if First Mate Nama was empowered",
},
["encID"] = 3497,
["internalID"] = "MightyThud",
["TTS"] = false,
["TTSTimer"] = 15.5,
["dur"] = 15.5,
["DisplayType"] = "Bar",
["id"] = 14,
["text"] = "Soaks",
["phase"] = {
1,
2,
3,
4,
},
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
17.6,
47.6,
},
{
17.6,
47.6,
},
{
17.6,
47.6,
},
},
["enabled"] = false,
["name"] = "Soaks",
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["Ticks"] = {
11.5,
13.5,
},
["spellID"] = 1296133,
},
["TimeToThrow"] = {
["timers"] = {
},
["group"] = "Trader Abilities",
["isConditional"] = {
["func"] = "return function() return C_ActionBar.HasExtraActionBar() end",
["text"] = "This Alert only shows if you are holding the fish at the time.",
},
["encID"] = 3497,
["internalID"] = "TimeToThrow",
["TTS"] = false,
["TTSTimer"] = 7,
["dur"] = 7,
["phase"] = {
1,
2,
3,
},
["text"] = "Time to Throw",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
61,
},
{
121,
},
{
121,
},
},
["id"] = 24,
["name"] = "Time to throw Fish",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1295817,
},
["ShreddingShards"] = {
["timers"] = {
},
["group"] = "Scrollsage Abilities",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss4\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss4.",
},
["encID"] = 3497,
["internalID"] = "ShreddingShards",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["DisplayType"] = "Text",
["id"] = 2,
["text"] = "Tank-Hit",
["enabled"] = false,
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
30,
},
{
30.5,
90.5,
},
{
30.5,
90.5,
},
{
30.5,
90.5,
},
},
["name"] = "Tank-Hit",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["phase"] = {
1,
2,
3,
4,
},
["spellID"] = 1295854,
},
["BlinkNova"] = {
["timers"] = {
},
["group"] = "Scrollsage Abilities",
["encID"] = 3497,
["internalID"] = "BlinkNova",
["enabled"] = false,
["TTSTimer"] = 9,
["dur"] = 9,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Blink Nova",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
17,
48,
},
{
77,
108,
},
{
77,
108,
},
{
77,
108,
},
},
["id"] = 4,
["name"] = "Blink Nova",
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1296021,
},
["ExplosiveSurprise"] = {
["timers"] = {
},
["group"] = "Trader Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss1\" end",
["text"] = "This Alert only shows if Trader Gebbo was empowered",
},
["encID"] = 3497,
["internalID"] = "ExplosiveSurprise",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Bomb inc",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
13,
45.5,
},
{
13,
45.5,
},
{
13,
45.5,
},
},
["id"] = 20,
["name"] = "Bomb Debuff",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1296249,
},
["MushroomBait"] = {
["timers"] = {
},
["group"] = "Trader Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss1\" end",
["text"] = "This Alert only shows if Trader Gebbo was empowered",
},
["loadConditions"] = {
["Roles"] = {
["RANGED"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3497,
["internalID"] = "MushroomBait",
["TTS"] = false,
["TTSTimer"] = 7,
["dur"] = 7,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Bait",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
10,
42,
},
{
10,
42,
},
{
10,
42,
},
},
["id"] = 18,
["name"] = "Mushroom Bait",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1292105,
},
["MushroomJump"] = {
["timers"] = {
},
["group"] = "Trader Abilities",
["isConditional"] = {
["func"] = "return function() return NSAPI and NSAPI.ActiveBoss and NSAPI.ActiveBoss == \"boss1\" end",
["text"] = "This Alert only shows if Trader Gebbo was empowered",
},
["encID"] = 3497,
["internalID"] = "MushroomJump",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = {
1,
2,
3,
4,
},
["text"] = "Jump",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
},
{
35,
66,
},
{
35,
66,
},
{
35,
66,
},
},
["id"] = 22,
["name"] = "Mushroom Jump",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1299855,
},
},
},
[3470] = {
[15] = {
["RestlessAmani"] = {
["timers"] = {
},
["group"] = "Nek'zali",
["encID"] = 3470,
["internalID"] = "RestlessAmani",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = {
1,
1.5,
1.75,
2,
},
["text"] = "Adds",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
54,
121,
},
{
35,
75,
115,
},
[1.5] = {
56.6,
},
[1.75] = {
56.6,
},
},
["id"] = 9,
["name"] = "Add-Spawn",
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1295397,
},
["Debuffs"] = {
["timers"] = {
},
["group"] = "Nek'zali",
["loadConditions"] = {
["Roles"] = {
["DAMAGER"] = true,
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3470,
["internalID"] = "Debuffs",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = {
1,
2,
},
["text"] = "Debuffs",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
14.5,
54.5,
85.5,
125.5,
},
{
54.6,
},
},
["id"] = 3,
["name"] = "Essence Rend",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1287434,
},
["HungeringPyre"] = {
["timers"] = {
},
["group"] = "Nek'zali",
["encID"] = 3470,
["internalID"] = "HungeringPyre",
["enabled"] = false,
["TTSTimer"] = 7.5,
["dur"] = 7.5,
["phase"] = {
1.5,
1.75,
},
["text"] = "Soak",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
[1.5] = {
36,
},
[1.75] = {
25.5,
},
},
["id"] = 7,
["name"] = "Hungering Pyre",
["IsAlert"] = true,
["TTS"] = true,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1289855,
},
["SoulcoilIgnition"] = {
["timers"] = {
76.8,
},
["group"] = "Nek'zali",
["loadConditions"] = {
["Roles"] = {
["DAMAGER"] = true,
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3470,
["internalID"] = "SoulcoilIgnition",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "AoE",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1293664,
["name"] = "Soulcoil Ignition",
["IsAlert"] = true,
["id"] = 5,
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Text",
},
["Invoke"] = {
["timers"] = {
18,
66,
98,
},
["group"] = "Nek'zali",
["encID"] = 3470,
["internalID"] = "Invoke",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 2,
["text"] = "Dodge",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1299673,
["name"] = "Invoke",
["IsAlert"] = true,
["id"] = 11,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Barrage"] = {
["timers"] = {
},
["group"] = "Nek'zali",
["encID"] = 3470,
["internalID"] = "Barrage",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["DisplayType"] = "Text",
["id"] = 1,
["text"] = "Frontal",
["phase"] = {
1,
2,
},
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
34,
70,
105,
141,
176,
212,
},
{
51,
79,
},
},
["name"] = "Barrage",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["enabled"] = false,
["spellID"] = 1284103,
},
},
[16] = {
["RestlessAmani"] = {
["timers"] = {
},
["group"] = "Nek'zali",
["encID"] = 3470,
["internalID"] = "RestlessAmani",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = {
1,
1.5,
1.75,
2,
},
["text"] = "Adds",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
54,
121,
},
{
35,
75,
115,
},
[1.5] = {
56.6,
},
[1.75] = {
56.6,
},
},
["id"] = 10,
["name"] = "Add-Spawn",
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1295397,
},
["Debuffs"] = {
["timers"] = {
},
["group"] = "Nek'zali",
["loadConditions"] = {
["Roles"] = {
["DAMAGER"] = true,
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3470,
["internalID"] = "Debuffs",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = {
1,
2,
},
["text"] = "Debuffs",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
14.5,
54.5,
85.5,
125.5,
},
{
54.6,
},
},
["id"] = 4,
["name"] = "Essence Rend",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1287434,
},
["InvokeMythic"] = {
["timers"] = {
18,
66,
98,
},
["group"] = "Nek'zali",
["encID"] = 3470,
["internalID"] = "InvokeMythic",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 2,
["text"] = "Stop Cast",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1299673,
["name"] = "Invoke",
["IsAlert"] = true,
["id"] = 12,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["HungeringPyre"] = {
["timers"] = {
},
["group"] = "Nek'zali",
["encID"] = 3470,
["internalID"] = "HungeringPyre",
["enabled"] = false,
["TTSTimer"] = 7.5,
["dur"] = 7.5,
["phase"] = {
1.5,
1.75,
},
["text"] = "Soak",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
[1.5] = {
36,
},
[1.75] = {
25.5,
},
},
["id"] = 8,
["name"] = "Hungering Pyre",
["IsAlert"] = true,
["TTS"] = true,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1289855,
},
["SoulcoilIgnition"] = {
["timers"] = {
76.8,
},
["group"] = "Nek'zali",
["loadConditions"] = {
["Roles"] = {
["DAMAGER"] = true,
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3470,
["internalID"] = "SoulcoilIgnition",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "AoE",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1293664,
["name"] = "Soulcoil Ignition",
["IsAlert"] = true,
["id"] = 6,
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Text",
},
["Barrage"] = {
["timers"] = {
},
["group"] = "Nek'zali",
["encID"] = 3470,
["internalID"] = "Barrage",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["DisplayType"] = "Text",
["id"] = 2,
["text"] = "Frontal",
["phase"] = {
1,
2,
},
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
34,
70,
105,
141,
176,
212,
},
{
51,
79,
},
},
["name"] = "Barrage",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["enabled"] = false,
["spellID"] = 1284103,
},
},
},
[3379] = {
[15] = {
["Taunt"] = {
["timers"] = {
35,
57,
79,
101,
145,
167,
189,
211,
255,
277,
299,
321,
365,
387.1,
409,
431,
},
["group"] = "Nymrissa",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat < 2 end",
["text"] = "This Alert only shows if you do not have threat on boss1.",
},
["customIcon"] = 355,
["encID"] = 3379,
["internalID"] = "Taunt",
["TTS"] = true,
["TTSTimer"] = 0,
["dur"] = 5,
["phase"] = 1,
["id"] = 13,
["text"] = "Taunt",
["name"] = "Taunt",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["isTaunt"] = true,
["textColors"] = {
0,
1,
0,
1,
},
["IsAlert"] = true,
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["sticky"] = 3,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Knockback"] = {
["timers"] = {
120,
230,
340,
450,
},
["group"] = "Nymrissa",
["encID"] = 3379,
["internalID"] = "Knockback",
["TTS"] = true,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Knock",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1258150,
["name"] = "Knockback",
["IsAlert"] = true,
["id"] = 5,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Adds"] = {
["timers"] = {
26.2,
70.1,
137.1,
180.1,
246.3,
290,
356.2,
401.2,
},
["group"] = "Nymrissa",
["encID"] = 3379,
["internalID"] = "Adds",
["TTS"] = true,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Adds",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 208309,
["name"] = "Add-Spawn",
["IsAlert"] = true,
["id"] = 1,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["WaterFlurry"] = {
["timers"] = {
29,
51,
73,
95,
139,
161,
183,
205,
249,
271,
293,
315,
359,
381.1,
403,
425,
},
["group"] = "Nymrissa",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss1.",
},
["encID"] = 3379,
["internalID"] = "WaterFlurry",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["spellID"] = 1282937,
["id"] = 12,
["text"] = "Tank-Hit",
["enabled"] = false,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Iceblade Flurry",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["Version"] = {
{
["name"] = "Iceblade Flurry",
},
["versionNumber"] = 1,
},
["sticky"] = 0,
["phase"] = 1,
["DisplayType"] = "Text",
},
["AbyssalRain"] = {
["timers"] = {
10,
43,
87,
153,
197,
263,
307,
373.1,
417,
},
["group"] = "Nymrissa",
["encID"] = 3379,
["internalID"] = "AbyssalRain",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "AoE",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1260837,
["name"] = "Abyssal Rain",
["IsAlert"] = true,
["id"] = 9,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["ChillingFrost"] = {
["timers"] = {
37,
81,
147,
191,
257,
301,
367,
411,
},
["group"] = "Nymrissa",
["encID"] = 3379,
["internalID"] = "ChillingFrost",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Debuffs",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1313393,
["name"] = "Chilling Frost",
["IsAlert"] = true,
["id"] = 7,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Waves"] = {
["timers"] = {
111,
221,
331,
441,
},
["group"] = "Nymrissa",
["encID"] = 3379,
["internalID"] = "Waves",
["TTS"] = "Dodge",
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Waves",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1258673,
["name"] = "Waves",
["IsAlert"] = true,
["id"] = 3,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
},
[16] = {
["Taunt"] = {
["timers"] = {
36.5,
58.5,
80.5,
102.5,
146.5,
168.5,
190.5,
212.5,
256.5,
278.5,
300.5,
322.5,
366.5,
388.5,
410.5,
432.5,
},
["group"] = "Nymrissa",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat < 2 end",
["text"] = "This Alert only shows if you do not have threat on boss1.",
},
["customIcon"] = 355,
["encID"] = 3379,
["internalID"] = "Taunt",
["TTS"] = true,
["TTSTimer"] = 0,
["dur"] = 5,
["phase"] = 1,
["id"] = 14,
["text"] = "Taunt",
["name"] = "Taunt",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["isTaunt"] = true,
["textColors"] = {
0,
1,
0,
1,
},
["IsAlert"] = true,
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["sticky"] = 3,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Knockback"] = {
["timers"] = {
120,
230,
340,
450,
},
["group"] = "Nymrissa",
["encID"] = 3379,
["internalID"] = "Knockback",
["TTS"] = true,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Knock",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1258150,
["name"] = "Knockback",
["IsAlert"] = true,
["id"] = 6,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Adds"] = {
["timers"] = {
18,
68,
128,
188,
248,
308,
368,
428,
},
["group"] = "Nymrissa",
["encID"] = 3379,
["internalID"] = "Adds",
["TTS"] = true,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Adds",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 208309,
["name"] = "Add-Spawn",
["IsAlert"] = true,
["id"] = 2,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Waves"] = {
["timers"] = {
111,
221,
331,
441,
},
["group"] = "Nymrissa",
["encID"] = 3379,
["internalID"] = "Waves",
["TTS"] = "Dodge",
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Waves",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1258673,
["name"] = "Waves",
["IsAlert"] = true,
["id"] = 4,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["AbyssalRain"] = {
["timers"] = {
10,
43,
87.1,
153,
197,
263,
307,
373,
417.1,
461.1,
},
["group"] = "Nymrissa",
["encID"] = 3379,
["internalID"] = "AbyssalRain",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "AoE",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1260837,
["name"] = "Abyssal Rain",
["IsAlert"] = true,
["id"] = 10,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["ChillingFrost"] = {
["timers"] = {
38.1,
82,
148.1,
192.1,
258.1,
302.1,
368.1,
412.1,
478.1,
},
["group"] = "Nymrissa",
["encID"] = 3379,
["internalID"] = "ChillingFrost",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Debuffs",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1313393,
["name"] = "Chilling Frost",
["IsAlert"] = true,
["id"] = 8,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["WaterJet"] = {
["timers"] = {
30,
52,
74,
96,
140,
162,
184,
206,
250,
272,
294,
316,
360,
382,
404,
426,
},
["group"] = "Nymrissa",
["isConditional"] = {
["func"] = "return function() if UnitGroupRolesAssigned(\"player\") ~= \"TANK\" then return true end local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you are not a tank or have threat on boss1.",
},
["encID"] = 3379,
["internalID"] = "WaterJet",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Frontal",
["spellID"] = 1258901,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["id"] = 11,
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["name"] = "Water Jet",
["DisplayType"] = "Text",
},
},
},
[3445] = {
[14] = {
["Radar"] = {
["FontSize"] = 20,
["xOffset"] = 0,
["NearColor"] = {
1,
0.1,
0.1,
1,
},
["yOffset"] = 250,
["loadConditions"] = {
["Roles"] = {
["RANGED"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["internalID"] = "Radar",
["TTS"] = false,
["SafeDistance"] = 40,
["difficulties"] = {
14,
15,
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["name"] = "Radar",
["IsAlert"] = true,
["DisplayType"] = "Text",
["timers"] = {
},
["group"] = "Sentinels",
["BorderColor"] = {
0,
0,
0,
1,
},
["Anchor"] = "CENTER",
["Preview"] = "        return function(self)\n            if self.SentinelsRadarPreview then\n                self.EncounterAlertStop[3445](self)\n            else\n                self.EncounterAlertStart[3445](self, 16, \"Radar\")\n            end\n        end\n    ",
["BackgroundColor"] = {
0.06,
0.06,
0.06,
0.9,
},
["encID"] = 3445,
["dur"] = 5,
["relativeTo"] = "CENTER",
["BlockCopy"] = true,
["extraOptions"] = {
{
["text"] = "Radar",
["Type"] = "Label",
},
{
["Type"] = "Slider",
["decimals"] = 2,
["label"] = "Scale",
["max"] = 3,
["step"] = 0.05,
["min"] = 0.5,
["usedecimals"] = true,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.Scale = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.Scale or 1 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "xOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.xOffset = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.xOffset or 0 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "yOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.yOffset = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.yOffset or 250 end",
},
{
["min"] = 8,
["Type"] = "Slider",
["label"] = "FontSize",
["max"] = 60,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.FontSize = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.FontSize or 20 end",
},
{
["min"] = 5,
["Type"] = "Slider",
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.SafeDistance = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["label"] = "SafeDistance",
["max"] = 45,
["tooltip"] = {
["title"] = "SafeDistance",
["desc"] = "Distance in yards at which a boss' number turns from the close color to the far color.",
},
["step"] = 1,
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.SafeDistance or 40 end",
},
{
["Type"] = "Slider",
["decimals"] = 2,
["label"] = "UpdateInterval",
["max"] = 1,
["tooltip"] = {
["title"] = "UpdateInterval",
["desc"] = "Seconds between distance refreshes. Lower is smoother but costs more CPU.",
},
["step"] = 0.05,
["min"] = 0.05,
["usedecimals"] = true,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.UpdateInterval = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.UpdateInterval or 0.1 end",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.TickColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.TickColor or {0.13,0.85,0.13,1} return c[1],c[2],c[3],c[4] end",
["label"] = "TickColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.FarColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.FarColor or {0.13,1,0.13,1} return c[1],c[2],c[3],c[4] end",
["label"] = "FarColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.NearColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.NearColor or {1,0.1,0.1,1} return c[1],c[2],c[3],c[4] end",
["label"] = "NearColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.BackgroundColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.BackgroundColor or {0.06,0.06,0.06,0.9} return c[1],c[2],c[3],c[4] end",
["label"] = "BackgroundColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.BorderColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.BorderColor or {0,0,0,1} return c[1],c[2],c[3],c[4] end",
["label"] = "BorderColor",
},
},
["id"] = 0,
["UpdateInterval"] = 0.5,
["enabled"] = false,
["Scale"] = 1,
["TickColor"] = {
0.13,
0.85,
0.13,
1,
},
["customIcon"] = 1284500,
["FarColor"] = {
0.13,
1,
0.13,
1,
},
["phase"] = 1,
["isSpecialDisplay"] = true,
["sticky"] = 0,
["TTSTimer"] = 5,
["NoEdit"] = true,
},
},
[16] = {
["PoisonAdd"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local minRange = NSAPI and NSAPI:GetRange(\"boss1\") return minRange and minRange < 40 end",
["text"] = "This Alert only shows if you are within 40y of boss1.",
},
["encID"] = 3445,
["internalID"] = "PoisonAdd",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["DisplayType"] = "Text",
["id"] = 14,
["text"] = "Poison Add",
["enabled"] = false,
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
12.4,
},
{
11.5,
63.7,
},
{
12.5,
64.8,
},
{
12.7,
64.9,
},
{
12.6,
64.8,
},
},
["textColors"] = {
0.62,
1,
0.25,
1,
},
["name"] = "Poison Add",
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["phase"] = {
1,
2,
3,
4,
5,
},
["spellID"] = 1284251,
},
["BloodHits"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss2\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss2.",
},
["encID"] = 3445,
["internalID"] = "BloodHits",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["DisplayType"] = "Text",
["id"] = 4,
["text"] = "Tank-Hit",
["enabled"] = false,
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
7.5,
29.4,
},
{
7.9,
30.9,
52.8,
74.6,
},
{
7.6,
29.5,
51.4,
73.3,
},
{
7.9,
29.7,
51.6,
73.4,
},
{
7.8,
29.6,
51.5,
73.3,
},
},
["textColors"] = {
1,
0,
0,
1,
},
["name"] = "Blood Tank-Hit",
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["phase"] = {
1,
2,
3,
4,
5,
},
["spellID"] = 1284487,
},
["BloodDropPool"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss2\") return (threat and threat < 2) or not threat end",
["text"] = "This Alert only shows if you do not have threat on boss2.",
},
["encID"] = 3445,
["internalID"] = "BloodDropPool",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["DisplayType"] = "Text",
["id"] = 6,
["text"] = "Drop-Pool",
["phase"] = {
1,
2,
3,
4,
5,
},
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
8.5,
30.4,
},
{
8.6,
32.1,
54,
75.9,
},
{
8.6,
32.1,
54,
75.9,
},
{
8.6,
32.1,
54,
75.9,
},
{
8.6,
32.1,
54,
75.9,
},
},
["enabled"] = false,
["name"] = "Tank Drop Pool",
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["isSpecialDisplay"] = true,
["spellID"] = 1284487,
},
["Radar"] = {
["FontSize"] = 20,
["xOffset"] = 0,
["NearColor"] = {
1,
0.1,
0.1,
1,
},
["yOffset"] = 250,
["loadConditions"] = {
["Roles"] = {
["RANGED"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["internalID"] = "Radar",
["TTS"] = false,
["SafeDistance"] = 40,
["difficulties"] = {
14,
15,
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["name"] = "Radar",
["IsAlert"] = true,
["DisplayType"] = "Text",
["timers"] = {
},
["group"] = "Sentinels",
["BorderColor"] = {
0,
0,
0,
1,
},
["Anchor"] = "CENTER",
["Preview"] = "        return function(self)\n            if self.SentinelsRadarPreview then\n                self.EncounterAlertStop[3445](self)\n            else\n                self.EncounterAlertStart[3445](self, 16, \"Radar\")\n            end\n        end\n    ",
["BackgroundColor"] = {
0.06,
0.06,
0.06,
0.9,
},
["encID"] = 3445,
["dur"] = 5,
["relativeTo"] = "CENTER",
["BlockCopy"] = true,
["extraOptions"] = {
{
["text"] = "Radar",
["Type"] = "Label",
},
{
["Type"] = "Slider",
["decimals"] = 2,
["label"] = "Scale",
["max"] = 3,
["step"] = 0.05,
["min"] = 0.5,
["usedecimals"] = true,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.Scale = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.Scale or 1 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "xOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.xOffset = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.xOffset or 0 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "yOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.yOffset = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.yOffset or 250 end",
},
{
["min"] = 8,
["Type"] = "Slider",
["label"] = "FontSize",
["max"] = 60,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.FontSize = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.FontSize or 20 end",
},
{
["min"] = 5,
["Type"] = "Slider",
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.SafeDistance = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["label"] = "SafeDistance",
["max"] = 45,
["tooltip"] = {
["title"] = "SafeDistance",
["desc"] = "Distance in yards at which a boss' number turns from the close color to the far color.",
},
["step"] = 1,
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.SafeDistance or 40 end",
},
{
["Type"] = "Slider",
["decimals"] = 2,
["label"] = "UpdateInterval",
["max"] = 1,
["tooltip"] = {
["title"] = "UpdateInterval",
["desc"] = "Seconds between distance refreshes. Lower is smoother but costs more CPU.",
},
["step"] = 0.05,
["min"] = 0.05,
["usedecimals"] = true,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.UpdateInterval = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.UpdateInterval or 0.1 end",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.TickColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.TickColor or {0.13,0.85,0.13,1} return c[1],c[2],c[3],c[4] end",
["label"] = "TickColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.FarColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.FarColor or {0.13,1,0.13,1} return c[1],c[2],c[3],c[4] end",
["label"] = "FarColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.NearColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.NearColor or {1,0.1,0.1,1} return c[1],c[2],c[3],c[4] end",
["label"] = "NearColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.BackgroundColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.BackgroundColor or {0.06,0.06,0.06,0.9} return c[1],c[2],c[3],c[4] end",
["label"] = "BackgroundColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.BorderColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.BorderColor or {0,0,0,1} return c[1],c[2],c[3],c[4] end",
["label"] = "BorderColor",
},
},
["id"] = 0,
["UpdateInterval"] = 0.5,
["enabled"] = false,
["Scale"] = 1,
["TickColor"] = {
0.13,
0.85,
0.13,
1,
},
["customIcon"] = 1284500,
["FarColor"] = {
0.13,
1,
0.13,
1,
},
["phase"] = 1,
["isSpecialDisplay"] = true,
["sticky"] = 0,
["TTSTimer"] = 5,
["NoEdit"] = true,
},
["ShiftingProtovenom"] = {
["timers"] = {
},
["group"] = "Sentinels",
["encID"] = 3445,
["internalID"] = "ShiftingProtovenom",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
5,
},
["text"] = "Spread",
["DisplayType"] = "Circle",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
36.9,
},
{
40.4,
81.4,
},
{
40.4,
81.4,
},
{
40.4,
81.4,
},
{
40.4,
81.4,
},
},
["id"] = 17,
["name"] = "Shifting Protovenom",
["IsAlert"] = true,
["TTS"] = "Spread",
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1296880,
},
["BloodSoak"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local minRange = NSAPI and NSAPI:GetRange(\"boss2\") return minRange and minRange < 40 end",
["text"] = "This Alert only shows if you are within 40y of boss2.",
},
["encID"] = 3445,
["internalID"] = "BloodSoak",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["DisplayType"] = "Text",
["id"] = 8,
["text"] = "Blood-Soak",
["enabled"] = false,
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
26.5,
},
{
25.6,
66.8,
},
{
26.6,
67.9,
},
{
25.6,
66.9,
},
{
26.7,
68,
},
},
["textColors"] = {
1,
0.37,
0.25,
1,
},
["name"] = "Blood Soak",
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["phase"] = {
1,
2,
3,
4,
5,
},
["spellID"] = 1288232,
},
["BloodSoakPool"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local minRange = NSAPI and NSAPI:GetRange(\"boss2\") return minRange and minRange < 40 end",
["text"] = "This Alert only shows if you are within 40y of boss2.",
},
["encID"] = 3445,
["internalID"] = "BloodSoakPool",
["DisplayType"] = "Text",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["id"] = 10,
["phase"] = {
1,
2,
3,
4,
5,
},
["text"] = "Drop Pool",
["phaseTimers"] = {
{
32.6,
},
{
32.6,
74.8,
},
{
32.6,
74.8,
},
{
32.6,
74.8,
},
{
32.6,
74.8,
},
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["dur"] = 6,
},
{
["DisplayType"] = "Text",
},
["versionNumber"] = 2,
},
["textColors"] = {
1,
0.37,
0.25,
1,
},
["IsAlert"] = true,
["name"] = "Soak-Pool",
["sticky"] = 0,
["enabled"] = false,
["spellID"] = 1288232,
},
["PoisonHits"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss1.",
},
["encID"] = 3445,
["internalID"] = "PoisonHits",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["DisplayType"] = "Text",
["id"] = 2,
["text"] = "Tank-Hit",
["enabled"] = false,
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
6.3,
28.2,
},
{
6.7,
28.5,
50.7,
72.2,
},
{
6.6,
28.5,
50.7,
72.4,
},
{
6.6,
28.5,
50.3,
72.2,
},
{
6.5,
28.4,
50.3,
72.3,
},
},
["textColors"] = {
1,
0,
0,
1,
},
["name"] = "Poison Tank-Hit",
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["phase"] = {
1,
2,
3,
4,
5,
},
["spellID"] = 1284458,
},
["OrbSpawn"] = {
["timers"] = {
},
["group"] = "Sentinels",
["encID"] = 3445,
["internalID"] = "OrbSpawn",
["TTS"] = "Bait",
["TTSTimer"] = 6,
["dur"] = 6,
["DisplayType"] = "Text",
["id"] = 16,
["text"] = "Bait Orbs",
["enabled"] = false,
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
15.3,
},
{
14.5,
47.2,
80,
},
{
15.4,
48.2,
80.5,
},
{
15.6,
48.4,
81.1,
},
{
15.5,
48.3,
80.8,
},
},
["name"] = "Orb Spawn",
["Version"] = {
{
["dur"] = 8,
["text"] = "Orbs",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["phase"] = {
1,
2,
3,
4,
5,
},
["spellID"] = 1284434,
},
["BloodDispels"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local minRange = NSAPI and NSAPI:GetRange(\"boss2\") return minRange and minRange < 40 end",
["text"] = "This Alert only shows if you are within 40y of boss2.",
},
["loadConditions"] = {
["Roles"] = {
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3445,
["internalID"] = "BloodDispels",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
5,
},
["text"] = "Dispels",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
42.5,
},
{
42.9,
},
{
43.2,
},
{
43.6,
},
{
43.2,
},
},
["id"] = 12,
["name"] = "Blood Dispels",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1284471,
},
["TransitionDebuffs"] = {
["timers"] = {
},
["group"] = "Sentinels",
["encID"] = 3445,
["internalID"] = "TransitionDebuffs",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = {
1,
2,
3,
4,
5,
},
["text"] = "Number Game",
["DisplayType"] = "Circle",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
46.6,
},
{
91.1,
},
{
91.1,
},
{
91.2,
},
{
91.1,
},
},
["id"] = 19,
["name"] = "Transition Debuffs",
["IsAlert"] = true,
["TTS"] = "Spread",
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1284590,
},
},
[15] = {
["PoisonAdd"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local minRange = NSAPI and NSAPI:GetRange(\"boss1\") return minRange and minRange < 40 end",
["text"] = "This Alert only shows if you are within 40y of boss1.",
},
["encID"] = 3445,
["internalID"] = "PoisonAdd",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["DisplayType"] = "Text",
["id"] = 13,
["text"] = "Poison Add",
["enabled"] = false,
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
15.7,
},
{
15.7,
68,
},
{
15.7,
68,
},
{
15.7,
68,
},
{
15.7,
68,
},
},
["textColors"] = {
0.62,
1,
0.25,
1,
},
["name"] = "Poison Add",
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["phase"] = {
1,
2,
3,
4,
5,
},
["spellID"] = 1284251,
},
["BloodHits"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss2\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss2.",
},
["encID"] = 3445,
["internalID"] = "BloodHits",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["DisplayType"] = "Text",
["id"] = 3,
["text"] = "Tank-Hit",
["enabled"] = false,
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
7.6,
29.5,
},
{
7.6,
29.5,
51.4,
73.3,
},
{
7.6,
29.5,
51.4,
73.3,
},
{
7.6,
29.5,
51.4,
73.3,
},
{
7.6,
29.5,
51.4,
73.3,
},
},
["textColors"] = {
1,
0,
0,
1,
},
["name"] = "Blood Tank-Hit",
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["phase"] = {
1,
2,
3,
4,
5,
},
["spellID"] = 1284487,
},
["BloodDropPool"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss2\") return (threat and threat < 2) or not threat end",
["text"] = "This Alert only shows if you do not have threat on boss2.",
},
["encID"] = 3445,
["internalID"] = "BloodDropPool",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["DisplayType"] = "Text",
["id"] = 5,
["text"] = "Drop-Pool",
["phase"] = {
1,
2,
3,
4,
5,
},
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
8.5,
30.4,
},
{
8.5,
30.4,
52.3,
74.2,
},
{
8.5,
30.4,
52.3,
74.2,
},
{
8.5,
30.4,
52.3,
74.2,
},
{
8.5,
30.4,
52.3,
74.2,
},
},
["enabled"] = false,
["name"] = "Tank Drop Pool",
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["isSpecialDisplay"] = true,
["spellID"] = 1284487,
},
["Radar"] = {
["FontSize"] = 20,
["xOffset"] = 0,
["NearColor"] = {
1,
0.1,
0.1,
1,
},
["yOffset"] = 250,
["loadConditions"] = {
["Roles"] = {
["RANGED"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["internalID"] = "Radar",
["TTS"] = false,
["SafeDistance"] = 40,
["difficulties"] = {
14,
15,
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["name"] = "Radar",
["IsAlert"] = true,
["DisplayType"] = "Text",
["timers"] = {
},
["group"] = "Sentinels",
["BorderColor"] = {
0,
0,
0,
1,
},
["Anchor"] = "CENTER",
["Preview"] = "        return function(self)\n            if self.SentinelsRadarPreview then\n                self.EncounterAlertStop[3445](self)\n            else\n                self.EncounterAlertStart[3445](self, 16, \"Radar\")\n            end\n        end\n    ",
["BackgroundColor"] = {
0.06,
0.06,
0.06,
0.9,
},
["encID"] = 3445,
["dur"] = 5,
["relativeTo"] = "CENTER",
["BlockCopy"] = true,
["extraOptions"] = {
{
["text"] = "Radar",
["Type"] = "Label",
},
{
["Type"] = "Slider",
["decimals"] = 2,
["label"] = "Scale",
["max"] = 3,
["step"] = 0.05,
["min"] = 0.5,
["usedecimals"] = true,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.Scale = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.Scale or 1 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "xOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.xOffset = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.xOffset or 0 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "yOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.yOffset = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.yOffset or 250 end",
},
{
["min"] = 8,
["Type"] = "Slider",
["label"] = "FontSize",
["max"] = 60,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.FontSize = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.FontSize or 20 end",
},
{
["min"] = 5,
["Type"] = "Slider",
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.SafeDistance = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["label"] = "SafeDistance",
["max"] = 45,
["tooltip"] = {
["title"] = "SafeDistance",
["desc"] = "Distance in yards at which a boss' number turns from the close color to the far color.",
},
["step"] = 1,
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.SafeDistance or 40 end",
},
{
["Type"] = "Slider",
["decimals"] = 2,
["label"] = "UpdateInterval",
["max"] = 1,
["tooltip"] = {
["title"] = "UpdateInterval",
["desc"] = "Seconds between distance refreshes. Lower is smoother but costs more CPU.",
},
["step"] = 0.05,
["min"] = 0.05,
["usedecimals"] = true,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.UpdateInterval = v end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3445][16].Radar.UpdateInterval or 0.1 end",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.TickColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.TickColor or {0.13,0.85,0.13,1} return c[1],c[2],c[3],c[4] end",
["label"] = "TickColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.FarColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.FarColor or {0.13,1,0.13,1} return c[1],c[2],c[3],c[4] end",
["label"] = "FarColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.NearColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.NearColor or {1,0.1,0.1,1} return c[1],c[2],c[3],c[4] end",
["label"] = "NearColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.BackgroundColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.BackgroundColor or {0.06,0.06,0.06,0.9} return c[1],c[2],c[3],c[4] end",
["label"] = "BackgroundColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3445][i].Radar.BorderColor = {r,g,b,a} end NSI.EncounterAlertStop[3445](NSI) NSI.EncounterAlertStart[3445](NSI, 16, \"Radar\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3445][16].Radar.BorderColor or {0,0,0,1} return c[1],c[2],c[3],c[4] end",
["label"] = "BorderColor",
},
},
["id"] = 0,
["UpdateInterval"] = 0.5,
["enabled"] = false,
["Scale"] = 1,
["TickColor"] = {
0.13,
0.85,
0.13,
1,
},
["customIcon"] = 1284500,
["FarColor"] = {
0.13,
1,
0.13,
1,
},
["phase"] = 1,
["isSpecialDisplay"] = true,
["sticky"] = 0,
["TTSTimer"] = 5,
["NoEdit"] = true,
},
["OrbSpawn"] = {
["timers"] = {
},
["group"] = "Sentinels",
["encID"] = 3445,
["internalID"] = "OrbSpawn",
["TTS"] = "Bait",
["TTSTimer"] = 6,
["dur"] = 6,
["DisplayType"] = "Text",
["id"] = 15,
["text"] = "Bait Orbs",
["enabled"] = false,
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
17.2,
},
{
17.2,
50,
},
{
17.2,
50,
},
{
17.2,
50,
},
{
17.2,
50,
},
},
["name"] = "Orb Spawn",
["Version"] = {
{
["dur"] = 8,
["text"] = "Orbs",
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["phase"] = {
1,
2,
3,
4,
5,
},
["spellID"] = 1284434,
},
["BloodSoakPool"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local minRange = NSAPI and NSAPI:GetRange(\"boss2\") return minRange and minRange < 40 end",
["text"] = "This Alert only shows if you are within 40y of boss2.",
},
["encID"] = 3445,
["internalID"] = "BloodSoakPool",
["DisplayType"] = "Text",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["id"] = 9,
["phase"] = {
1,
2,
3,
4,
5,
},
["text"] = "Drop Pool",
["phaseTimers"] = {
{
32.6,
},
{
32.6,
74.8,
},
{
32.6,
74.8,
},
{
32.6,
74.8,
},
{
32.6,
74.8,
},
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["dur"] = 6,
},
{
["DisplayType"] = "Text",
},
["versionNumber"] = 2,
},
["textColors"] = {
1,
0.37,
0.25,
1,
},
["IsAlert"] = true,
["name"] = "Soak-Pool",
["sticky"] = 0,
["enabled"] = false,
["spellID"] = 1288232,
},
["BloodSoak"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local minRange = NSAPI and NSAPI:GetRange(\"boss2\") return minRange and minRange < 40 end",
["text"] = "This Alert only shows if you are within 40y of boss2.",
},
["encID"] = 3445,
["internalID"] = "BloodSoak",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["DisplayType"] = "Text",
["id"] = 7,
["text"] = "Blood-Soak",
["enabled"] = false,
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
26.6,
},
{
26.6,
68.8,
},
{
26.6,
68.8,
},
{
26.6,
68.8,
},
{
26.6,
68.8,
},
},
["textColors"] = {
1,
0.37,
0.25,
1,
},
["name"] = "Blood Soak",
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["phase"] = {
1,
2,
3,
4,
5,
},
["spellID"] = 1288232,
},
["PoisonHits"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss1.",
},
["encID"] = 3445,
["internalID"] = "PoisonHits",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["DisplayType"] = "Text",
["id"] = 1,
["text"] = "Tank-Hit",
["enabled"] = false,
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
6.4,
28.3,
},
{
6.4,
28.3,
51.4,
73.3,
},
{
6.4,
28.3,
51.4,
73.3,
},
{
6.4,
28.3,
51.4,
73.3,
},
{
6.4,
28.3,
51.4,
73.3,
},
},
["textColors"] = {
1,
0,
0,
1,
},
["name"] = "Poison Tank-Hit",
["IsAlert"] = true,
["ReloeReminder"] = true,
["sticky"] = 0,
["phase"] = {
1,
2,
3,
4,
5,
},
["spellID"] = 1284458,
},
["BloodDispels"] = {
["timers"] = {
},
["group"] = "Sentinels",
["isConditional"] = {
["func"] = "return function() local minRange = NSAPI and NSAPI:GetRange(\"boss2\") return minRange and minRange < 40 end",
["text"] = "This Alert only shows if you are within 40y of boss2.",
},
["loadConditions"] = {
["Roles"] = {
["HEALER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3445,
["internalID"] = "BloodDispels",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = {
1,
2,
3,
4,
5,
},
["text"] = "Dispels",
["DisplayType"] = "Text",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
45,
},
{
45,
},
{
45,
},
{
45,
},
{
45,
},
},
["id"] = 11,
["name"] = "Blood Dispels",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1284471,
},
["TransitionDebuffs"] = {
["timers"] = {
},
["group"] = "Sentinels",
["encID"] = 3445,
["internalID"] = "TransitionDebuffs",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = {
1,
2,
3,
4,
5,
},
["text"] = "Number Game",
["DisplayType"] = "Circle",
["DefaultEnabled"] = true,
["phaseTimers"] = {
{
46.2,
},
{
91,
},
{
91,
},
{
91,
},
{
91,
},
},
["id"] = 18,
["name"] = "Transition Debuffs",
["IsAlert"] = true,
["TTS"] = "Spread",
["sticky"] = 0,
["ReloeReminder"] = true,
["spellID"] = 1284590,
},
},
},
[3420] = {
[14] = {
["WindsHelper"] = {
["isSpecialDisplay"] = true,
["Scale"] = 1,
["timers"] = {
},
["dur"] = 5,
["yOffset"] = 400,
["group"] = "Sszorak",
["extraOptions"] = {
{
["text"] = "Winds Helper",
["Type"] = "Label",
},
{
["min"] = 0.5,
["Type"] = "Slider",
["label"] = "Scale",
["max"] = 2,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.Scale    = v end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3420][16].WindsHelper.Scale or 1 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "xOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.xOffset  = v end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3420][16].WindsHelper.xOffset  or 200 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "yOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.yOffset  = v end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3420][16].WindsHelper.yOffset  or -300 end",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.BackgroundColor = {r,g,b,a} end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3420][16].WindsHelper.BackgroundColor or {0.2,0.2,0.2,1} return c[1],c[2],c[3],c[4] end",
["label"] = "BackgroundColor",
},
{
["Type"] = "Checkbox",
["label"] = "ShowSenderNames",
["tooltip"] = {
["title"] = "ShowSenderNames",
["desc"] = "Shows the sender next to each entered number.",
},
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.ShowSenderNames  = v end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3420][16].WindsHelper.ShowSenderNames  or false  end",
},
{
["Type"] = "Button",
["label"] = "Create Macros",
["tooltip"] = {
["title"] = "Create Macros",
["desc"] = "Creates macros that post numbers 1 through 8 with the matching raid-marker icons and will trigger the display mid-fight. All you have to do is press the macro's in the order you want the Knocks to be triggered during the Dmg-Amp. If new messages come in after 3 have already filled it will simply start from the beginning, that way you can fix mistakes.",
},
["func"] = "return function()\n                    local iconIDs = {\"137001\", \"137002\", \"137003\", \"137004\", \"137005\", \"137006\", \"137007\", \"137008\"}\n                    for i=1, 8 do\n                        local macroName = \"NSRT_SSZORAK_\" .. i\n                        if not GetMacroInfo(macroName) then\n                            CreateMacro(macroName, iconIDs[i], \"/raid \" .. i)\n                        else\n                            EditMacro(macroName, macroName, iconIDs[i], \"/raid \" .. i)\n                        end\n                    end\n                end",
["width"] = 150,
},
},
["encID"] = 3420,
["customIcon"] = 1285732,
["ShowSenderNames"] = false,
["Preview"] = "        return function(self, update)\n            if self.IsSszorakWindsPreview then\n                self.EncounterAlertStop[3420](self, true)\n                self.IsSszorakWindsPreview = false\n            else\n                self.EncounterAlertStart[3420](self, 16, \"Winds Helper\")\n                self.IsSszorakWindsPreview = true\n            end\n        end\n    ",
["internalID"] = "WindsHelper",
["TTS"] = false,
["enabled"] = true,
["relativeTo"] = "CENTER",
["BlockCopy"] = true,
["Anchor"] = "CENTER",
["id"] = 0,
["difficulties"] = {
14,
15,
16,
},
["BackgroundColor"] = {
0.2,
0.2,
0.2,
1,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["TTSTimer"] = 5,
["name"] = "Winds Helper",
["IsAlert"] = true,
["phase"] = 1,
["sticky"] = 0,
["xOffset"] = -500,
["DisplayType"] = "Text",
},
["VenomousSurgeTargets"] = {
["isSpecialDisplay"] = true,
["group"] = "Sszorak",
["customIcon"] = 1305959,
["encID"] = 3420,
["Preview"] = "        return function(self, update)\n            if self.IsSszorakBombPreview then\n                self.EncounterAlertStop[3420](self, true)\n                self.IsSszorakBombPreview = false\n            else\n                self.EncounterAlertStart[3420](self, 16, \"Debuff Targets\")\n                self.IsSszorakBombPreview = true\n            end\n        end\n    ",
["internalID"] = "VenomousSurgeTargets",
["enabled"] = false,
["TTSTimer"] = 10,
["BlockCopy"] = true,
["spellID"] = 1305959,
["phase"] = 1,
["difficulties"] = {
14,
15,
16,
},
["id"] = 0.1,
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["timers"] = {
},
["name"] = "Debuff Targets",
["IsAlert"] = true,
["dur"] = 10,
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Bar",
},
["MarkerMap"] = {
["extraOptions"] = {
{
["text"] = "Rotating Marker Map",
["Type"] = "Label",
},
{
["Type"] = "Slider",
["decimals"] = 2,
["label"] = "Scale",
["max"] = 2,
["step"] = 0.05,
["min"] = 0.5,
["usedecimals"] = true,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.Scale = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.Scale or 1 end",
},
{
["min"] = 140,
["Type"] = "Slider",
["label"] = "MapSize",
["max"] = 500,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.MapSize = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["step"] = 10,
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.MapSize or 240 end",
},
{
["min"] = 16,
["Type"] = "Slider",
["label"] = "MarkerSize",
["max"] = 80,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.MarkerSize = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["step"] = 1,
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerSize or 34 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "xOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.xOffset = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.xOffset or -800 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "yOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.yOffset = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.yOffset or 150 end",
},
{
["Type"] = "Slider",
["decimals"] = 2,
["label"] = "UpdateInterval",
["max"] = 0.2,
["step"] = 0.01,
["min"] = 0.01,
["usedecimals"] = true,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.UpdateInterval = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.UpdateInterval or 0.03 end",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.BackgroundColor = {r,g,b,a} end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["Type"] = "Color",
["get"] = "return function() local c = NSRT.EncounterAlerts[3420][16].MarkerMap.BackgroundColor or {0.03,0.03,0.03,0.82} return c[1],c[2],c[3],c[4] end",
["label"] = "BackgroundColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.BorderColor = {r,g,b,a} end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["Type"] = "Color",
["get"] = "return function() local c = NSRT.EncounterAlerts[3420][16].MarkerMap.BorderColor or {0.15,0.85,1,1} return c[1],c[2],c[3],c[4] end",
["label"] = "BorderColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.PlayerColor = {r,g,b,a} end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["Type"] = "Color",
["get"] = "return function() local c = NSRT.EncounterAlerts[3420][16].MarkerMap.PlayerColor or {1,1,1,1} return c[1],c[2],c[3],c[4] end",
["label"] = "PlayerColor",
},
{
["Type"] = "Checkbox",
["label"] = "ShowPlayerArrow",
["tooltip"] = {
["title"] = "ShowPlayerArrow",
["desc"] = "Shows the player at the center, facing toward the top of the map.",
},
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.ShowPlayerArrow = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.ShowPlayerArrow ~= false end",
},
{
["Type"] = "Checkbox",
["label"] = "Active Entire Fight",
["tooltip"] = {
["title"] = "Active Entire Fight",
["desc"] = "Keeps the marker map visible from pull until the encounter ends instead of only around configured mechanic windows.",
},
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.ActiveEntireFight = v end end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.ActiveEntireFight == true end",
},
{
["text"] = "World marker positions",
["Type"] = "Label",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "North",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[1] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[1] or 1\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "Northeast",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[2] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[2] or 2\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "East",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[3] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[3] or 3\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "Southeast",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[4] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[4] or 4\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "South",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[5] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[5] or 5\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "Southwest",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[6] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[6] or 6\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "West",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[7] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[7] or 7\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "Northwest",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[8] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[8] or 8\n            end",
},
},
["Scale"] = 1,
["yOffset"] = 150,
["customIcon"] = 137001,
["internalID"] = "MarkerMap",
["TTS"] = false,
["difficulties"] = {
14,
15,
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["name"] = "Marker Map",
["IsAlert"] = true,
["DisplayType"] = "Text",
["timers"] = {
},
["group"] = "Sszorak",
["MarkerSize"] = 34,
["ShowPlayerArrow"] = true,
["xOffset"] = -800,
["UpdateInterval"] = 0.03,
["Anchor"] = "CENTER",
["Preview"] = "        return function(self)\n            if self.IsSszorakMarkerMapPreview then\n                self.EncounterAlertStop[3420](self)\n            else\n                self.EncounterAlertStart[3420](self, 16, \"Marker Map\")\n            end\n        end\n    ",
["BackgroundColor"] = {
0.302,
0.302,
0.302,
0.82,
},
["id"] = 0.2,
["PlayerColor"] = {
1,
1,
1,
1,
},
["relativeTo"] = "CENTER",
["dur"] = 5,
["ActiveEntireFight"] = false,
["phase"] = 1,
["encID"] = 3420,
["BorderColor"] = {
0.15,
0.85,
1,
1,
},
["enabled"] = false,
["MapSize"] = 200,
["BlockCopy"] = true,
["Version"] = {
{
["MapSize"] = 200,
["BackgroundColor"] = {
0.302,
0.302,
0.302,
0.82,
},
},
["versionNumber"] = 1,
},
["MarkerOrder"] = {
3,
8,
4,
5,
6,
7,
1,
2,
},
["isSpecialDisplay"] = true,
["sticky"] = 0,
["TTSTimer"] = 5,
["NoEdit"] = true,
},
},
[16] = {
["WindsHelper"] = {
["isSpecialDisplay"] = true,
["Scale"] = 1,
["timers"] = {
},
["dur"] = 5,
["yOffset"] = 400,
["group"] = "Sszorak",
["extraOptions"] = {
{
["text"] = "Winds Helper",
["Type"] = "Label",
},
{
["min"] = 0.5,
["Type"] = "Slider",
["label"] = "Scale",
["max"] = 2,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.Scale    = v end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3420][16].WindsHelper.Scale or 1 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "xOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.xOffset  = v end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3420][16].WindsHelper.xOffset  or 200 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "yOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.yOffset  = v end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3420][16].WindsHelper.yOffset  or -300 end",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.BackgroundColor = {r,g,b,a} end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3420][16].WindsHelper.BackgroundColor or {0.2,0.2,0.2,1} return c[1],c[2],c[3],c[4] end",
["label"] = "BackgroundColor",
},
{
["Type"] = "Checkbox",
["label"] = "ShowSenderNames",
["tooltip"] = {
["title"] = "ShowSenderNames",
["desc"] = "Shows the sender next to each entered number.",
},
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.ShowSenderNames  = v end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3420][16].WindsHelper.ShowSenderNames  or false  end",
},
{
["Type"] = "Button",
["label"] = "Create Macros",
["tooltip"] = {
["title"] = "Create Macros",
["desc"] = "Creates macros that post numbers 1 through 8 with the matching raid-marker icons and will trigger the display mid-fight. All you have to do is press the macro's in the order you want the Knocks to be triggered during the Dmg-Amp. If new messages come in after 3 have already filled it will simply start from the beginning, that way you can fix mistakes.",
},
["func"] = "return function()\n                    local iconIDs = {\"137001\", \"137002\", \"137003\", \"137004\", \"137005\", \"137006\", \"137007\", \"137008\"}\n                    for i=1, 8 do\n                        local macroName = \"NSRT_SSZORAK_\" .. i\n                        if not GetMacroInfo(macroName) then\n                            CreateMacro(macroName, iconIDs[i], \"/raid \" .. i)\n                        else\n                            EditMacro(macroName, macroName, iconIDs[i], \"/raid \" .. i)\n                        end\n                    end\n                end",
["width"] = 150,
},
},
["encID"] = 3420,
["customIcon"] = 1285732,
["ShowSenderNames"] = false,
["Preview"] = "        return function(self, update)\n            if self.IsSszorakWindsPreview then\n                self.EncounterAlertStop[3420](self, true)\n                self.IsSszorakWindsPreview = false\n            else\n                self.EncounterAlertStart[3420](self, 16, \"Winds Helper\")\n                self.IsSszorakWindsPreview = true\n            end\n        end\n    ",
["internalID"] = "WindsHelper",
["TTS"] = false,
["enabled"] = true,
["relativeTo"] = "CENTER",
["BlockCopy"] = true,
["Anchor"] = "CENTER",
["id"] = 0,
["difficulties"] = {
14,
15,
16,
},
["BackgroundColor"] = {
0.2,
0.2,
0.2,
1,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["TTSTimer"] = 5,
["name"] = "Winds Helper",
["IsAlert"] = true,
["phase"] = 1,
["sticky"] = 0,
["xOffset"] = -500,
["DisplayType"] = "Text",
},
["Debuffs"] = {
["timers"] = {
32,
79.8,
159,
206.8,
286,
333.8,
},
["group"] = "Sszorak",
["loadConditions"] = {
["Roles"] = {
["HEALER"] = true,
["DAMAGER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3420,
["internalID"] = "Debuffs",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Debuffs",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1305963,
["name"] = "Debuffs",
["IsAlert"] = true,
["id"] = 11,
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Text",
},
["Bait"] = {
["timers"] = {
29,
76,
156,
203,
283,
330,
},
["group"] = "Sszorak",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3420,
["internalID"] = "Bait",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Bait",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1305959,
["name"] = "Bait",
["IsAlert"] = true,
["id"] = 7,
["sticky"] = 0,
["TTS"] = true,
["DisplayType"] = "Text",
},
["DamageAmp"] = {
["timers"] = {
100,
227.1,
354.2,
},
["group"] = "Sszorak",
["encID"] = 3420,
["internalID"] = "DamageAmp",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Damage Amp",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1286033,
["name"] = "Damage Amp",
["IsAlert"] = true,
["id"] = 4,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["WindDebuffs"] = {
["timers"] = {
39.7,
86.7,
166.8,
213.8,
293.9,
340.9,
},
["group"] = "Sszorak",
["encID"] = 3420,
["internalID"] = "WindDebuffs",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Wind-Debuffs",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1285419,
["name"] = "WindDebuffs",
["IsAlert"] = true,
["id"] = 9,
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Text",
},
["SetMarkers"] = {
["timers"] = {
9.9,
137,
264,
},
["group"] = "Sszorak",
["encID"] = 3420,
["internalID"] = "SetMarkers",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["difficulties"] = {
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["id"] = 5,
["name"] = "Mark Reminder",
["IsAlert"] = true,
["text"] = "Set Markers",
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["TankCombo"] = {
["timers"] = {
4.9,
52,
132,
179,
259,
306.1,
},
["group"] = "Sszorak",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3420,
["internalID"] = "TankCombo",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Tank Combo",
["spellID"] = 1277002,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Tank Combo",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 2,
["DisplayType"] = "Text",
},
["VenomousSurgeTargets"] = {
["isSpecialDisplay"] = true,
["group"] = "Sszorak",
["customIcon"] = 1305959,
["encID"] = 3420,
["Preview"] = "        return function(self, update)\n            if self.IsSszorakBombPreview then\n                self.EncounterAlertStop[3420](self, true)\n                self.IsSszorakBombPreview = false\n            else\n                self.EncounterAlertStart[3420](self, 16, \"Debuff Targets\")\n                self.IsSszorakBombPreview = true\n            end\n        end\n    ",
["internalID"] = "VenomousSurgeTargets",
["enabled"] = false,
["TTSTimer"] = 10,
["BlockCopy"] = true,
["spellID"] = 1305959,
["phase"] = 1,
["difficulties"] = {
14,
15,
16,
},
["id"] = 0.1,
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["timers"] = {
},
["name"] = "Debuff Targets",
["IsAlert"] = true,
["dur"] = 10,
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Bar",
},
["SerpentsFury"] = {
["timers"] = {
25.5,
75.5,
152.5,
202.5,
279.5,
326.6,
},
["group"] = "Sszorak",
["loadConditions"] = {
},
["encID"] = 3420,
["internalID"] = "SerpentsFury",
["TTS"] = "Stack",
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Stack Up",
["id"] = 12,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["loadConditions"] = {
},
},
{
["customIcon"] = 1297367,
},
["versionNumber"] = 2,
},
["name"] = "Serpent's Fury",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["customIcon"] = 1297367,
["DisplayType"] = "Text",
},
["MarkerMap"] = {
["extraOptions"] = {
{
["text"] = "Rotating Marker Map",
["Type"] = "Label",
},
{
["Type"] = "Slider",
["decimals"] = 2,
["label"] = "Scale",
["max"] = 2,
["step"] = 0.05,
["min"] = 0.5,
["usedecimals"] = true,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.Scale = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.Scale or 1 end",
},
{
["min"] = 140,
["Type"] = "Slider",
["label"] = "MapSize",
["max"] = 500,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.MapSize = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["step"] = 10,
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.MapSize or 240 end",
},
{
["min"] = 16,
["Type"] = "Slider",
["label"] = "MarkerSize",
["max"] = 80,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.MarkerSize = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["step"] = 1,
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerSize or 34 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "xOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.xOffset = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.xOffset or -800 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "yOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.yOffset = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.yOffset or 150 end",
},
{
["Type"] = "Slider",
["decimals"] = 2,
["label"] = "UpdateInterval",
["max"] = 0.2,
["step"] = 0.01,
["min"] = 0.01,
["usedecimals"] = true,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.UpdateInterval = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.UpdateInterval or 0.03 end",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.BackgroundColor = {r,g,b,a} end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["Type"] = "Color",
["get"] = "return function() local c = NSRT.EncounterAlerts[3420][16].MarkerMap.BackgroundColor or {0.03,0.03,0.03,0.82} return c[1],c[2],c[3],c[4] end",
["label"] = "BackgroundColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.BorderColor = {r,g,b,a} end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["Type"] = "Color",
["get"] = "return function() local c = NSRT.EncounterAlerts[3420][16].MarkerMap.BorderColor or {0.15,0.85,1,1} return c[1],c[2],c[3],c[4] end",
["label"] = "BorderColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.PlayerColor = {r,g,b,a} end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["Type"] = "Color",
["get"] = "return function() local c = NSRT.EncounterAlerts[3420][16].MarkerMap.PlayerColor or {1,1,1,1} return c[1],c[2],c[3],c[4] end",
["label"] = "PlayerColor",
},
{
["Type"] = "Checkbox",
["label"] = "ShowPlayerArrow",
["tooltip"] = {
["title"] = "ShowPlayerArrow",
["desc"] = "Shows the player at the center, facing toward the top of the map.",
},
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.ShowPlayerArrow = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.ShowPlayerArrow ~= false end",
},
{
["Type"] = "Checkbox",
["label"] = "Active Entire Fight",
["tooltip"] = {
["title"] = "Active Entire Fight",
["desc"] = "Keeps the marker map visible from pull until the encounter ends instead of only around configured mechanic windows.",
},
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.ActiveEntireFight = v end end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.ActiveEntireFight == true end",
},
{
["text"] = "World marker positions",
["Type"] = "Label",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "North",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[1] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[1] or 1\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "Northeast",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[2] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[2] or 2\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "East",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[3] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[3] or 3\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "Southeast",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[4] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[4] or 4\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "South",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[5] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[5] or 5\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "Southwest",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[6] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[6] or 6\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "West",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[7] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[7] or 7\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "Northwest",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[8] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[8] or 8\n            end",
},
},
["Scale"] = 1,
["yOffset"] = 150,
["customIcon"] = 137001,
["internalID"] = "MarkerMap",
["TTS"] = false,
["difficulties"] = {
14,
15,
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["name"] = "Marker Map",
["IsAlert"] = true,
["DisplayType"] = "Text",
["timers"] = {
},
["group"] = "Sszorak",
["MarkerSize"] = 34,
["ShowPlayerArrow"] = true,
["xOffset"] = -800,
["UpdateInterval"] = 0.03,
["Anchor"] = "CENTER",
["Preview"] = "        return function(self)\n            if self.IsSszorakMarkerMapPreview then\n                self.EncounterAlertStop[3420](self)\n            else\n                self.EncounterAlertStart[3420](self, 16, \"Marker Map\")\n            end\n        end\n    ",
["BackgroundColor"] = {
0.302,
0.302,
0.302,
0.82,
},
["id"] = 0.2,
["PlayerColor"] = {
1,
1,
1,
1,
},
["relativeTo"] = "CENTER",
["dur"] = 5,
["ActiveEntireFight"] = false,
["phase"] = 1,
["encID"] = 3420,
["BorderColor"] = {
0.15,
0.85,
1,
1,
},
["enabled"] = false,
["MapSize"] = 200,
["BlockCopy"] = true,
["Version"] = {
{
["MapSize"] = 200,
["BackgroundColor"] = {
0.302,
0.302,
0.302,
0.82,
},
},
["versionNumber"] = 1,
},
["MarkerOrder"] = {
3,
8,
4,
5,
6,
7,
1,
2,
},
["isSpecialDisplay"] = true,
["sticky"] = 0,
["TTSTimer"] = 5,
["NoEdit"] = true,
},
},
[15] = {
["WindsHelper"] = {
["isSpecialDisplay"] = true,
["Scale"] = 1,
["timers"] = {
},
["dur"] = 5,
["yOffset"] = 400,
["group"] = "Sszorak",
["extraOptions"] = {
{
["text"] = "Winds Helper",
["Type"] = "Label",
},
{
["min"] = 0.5,
["Type"] = "Slider",
["label"] = "Scale",
["max"] = 2,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.Scale    = v end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3420][16].WindsHelper.Scale or 1 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "xOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.xOffset  = v end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3420][16].WindsHelper.xOffset  or 200 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "yOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.yOffset  = v end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3420][16].WindsHelper.yOffset  or -300 end",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.BackgroundColor = {r,g,b,a} end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["Type"] = "Color",
["get"] = "return function(NSI) local c = NSRT.EncounterAlerts[3420][16].WindsHelper.BackgroundColor or {0.2,0.2,0.2,1} return c[1],c[2],c[3],c[4] end",
["label"] = "BackgroundColor",
},
{
["Type"] = "Checkbox",
["label"] = "ShowSenderNames",
["tooltip"] = {
["title"] = "ShowSenderNames",
["desc"] = "Shows the sender next to each entered number.",
},
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].WindsHelper.ShowSenderNames  = v end NSI.EncounterAlertStop[3420](NSI, true) NSI.EncounterAlertStart[3420](NSI, 16, \"Winds Helper\") end",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3420][16].WindsHelper.ShowSenderNames  or false  end",
},
{
["Type"] = "Button",
["label"] = "Create Macros",
["tooltip"] = {
["title"] = "Create Macros",
["desc"] = "Creates macros that post numbers 1 through 8 with the matching raid-marker icons and will trigger the display mid-fight. All you have to do is press the macro's in the order you want the Knocks to be triggered during the Dmg-Amp. If new messages come in after 3 have already filled it will simply start from the beginning, that way you can fix mistakes.",
},
["func"] = "return function()\n                    local iconIDs = {\"137001\", \"137002\", \"137003\", \"137004\", \"137005\", \"137006\", \"137007\", \"137008\"}\n                    for i=1, 8 do\n                        local macroName = \"NSRT_SSZORAK_\" .. i\n                        if not GetMacroInfo(macroName) then\n                            CreateMacro(macroName, iconIDs[i], \"/raid \" .. i)\n                        else\n                            EditMacro(macroName, macroName, iconIDs[i], \"/raid \" .. i)\n                        end\n                    end\n                end",
["width"] = 150,
},
},
["encID"] = 3420,
["customIcon"] = 1285732,
["ShowSenderNames"] = false,
["Preview"] = "        return function(self, update)\n            if self.IsSszorakWindsPreview then\n                self.EncounterAlertStop[3420](self, true)\n                self.IsSszorakWindsPreview = false\n            else\n                self.EncounterAlertStart[3420](self, 16, \"Winds Helper\")\n                self.IsSszorakWindsPreview = true\n            end\n        end\n    ",
["internalID"] = "WindsHelper",
["TTS"] = false,
["enabled"] = true,
["relativeTo"] = "CENTER",
["BlockCopy"] = true,
["Anchor"] = "CENTER",
["id"] = 0,
["difficulties"] = {
14,
15,
16,
},
["BackgroundColor"] = {
0.2,
0.2,
0.2,
1,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["TTSTimer"] = 5,
["name"] = "Winds Helper",
["IsAlert"] = true,
["phase"] = 1,
["sticky"] = 0,
["xOffset"] = -500,
["DisplayType"] = "Text",
},
["VenomousSurgeTargets"] = {
["isSpecialDisplay"] = true,
["group"] = "Sszorak",
["customIcon"] = 1305959,
["encID"] = 3420,
["Preview"] = "        return function(self, update)\n            if self.IsSszorakBombPreview then\n                self.EncounterAlertStop[3420](self, true)\n                self.IsSszorakBombPreview = false\n            else\n                self.EncounterAlertStart[3420](self, 16, \"Debuff Targets\")\n                self.IsSszorakBombPreview = true\n            end\n        end\n    ",
["internalID"] = "VenomousSurgeTargets",
["enabled"] = false,
["TTSTimer"] = 10,
["BlockCopy"] = true,
["spellID"] = 1305959,
["phase"] = 1,
["difficulties"] = {
14,
15,
16,
},
["id"] = 0.1,
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["timers"] = {
},
["name"] = "Debuff Targets",
["IsAlert"] = true,
["dur"] = 10,
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Bar",
},
["Bait"] = {
["timers"] = {
32.2,
84.4,
170.3,
222.6,
308.5,
360.8,
},
["group"] = "Sszorak",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3420,
["internalID"] = "Bait",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Bait",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1305959,
["name"] = "Bait",
["IsAlert"] = true,
["id"] = 6,
["sticky"] = 0,
["TTS"] = true,
["DisplayType"] = "Text",
},
["DamageAmp"] = {
["timers"] = {
111.1,
249.3,
387.5,
},
["group"] = "Sszorak",
["encID"] = 3420,
["internalID"] = "DamageAmp",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Damage Amp",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1286033,
["name"] = "Damage Amp",
["IsAlert"] = true,
["id"] = 3,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["WindDebuffs"] = {
["timers"] = {
44,
96,
182,
234,
320,
372,
},
["group"] = "Sszorak",
["encID"] = 3420,
["internalID"] = "WindDebuffs",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Wind-Debuffs",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1285419,
["name"] = "WindDebuffs",
["IsAlert"] = true,
["id"] = 8,
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Text",
},
["MarkerMap"] = {
["extraOptions"] = {
{
["text"] = "Rotating Marker Map",
["Type"] = "Label",
},
{
["Type"] = "Slider",
["decimals"] = 2,
["label"] = "Scale",
["max"] = 2,
["step"] = 0.05,
["min"] = 0.5,
["usedecimals"] = true,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.Scale = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.Scale or 1 end",
},
{
["min"] = 140,
["Type"] = "Slider",
["label"] = "MapSize",
["max"] = 500,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.MapSize = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["step"] = 10,
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.MapSize or 240 end",
},
{
["min"] = 16,
["Type"] = "Slider",
["label"] = "MarkerSize",
["max"] = 80,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.MarkerSize = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["step"] = 1,
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerSize or 34 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "xOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.xOffset = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.xOffset or -800 end",
},
{
["min"] = -2000,
["Type"] = "Slider",
["label"] = "yOffset",
["max"] = 2000,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.yOffset = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.yOffset or 150 end",
},
{
["Type"] = "Slider",
["decimals"] = 2,
["label"] = "UpdateInterval",
["max"] = 0.2,
["step"] = 0.01,
["min"] = 0.01,
["usedecimals"] = true,
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.UpdateInterval = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.UpdateInterval or 0.03 end",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.BackgroundColor = {r,g,b,a} end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["Type"] = "Color",
["get"] = "return function() local c = NSRT.EncounterAlerts[3420][16].MarkerMap.BackgroundColor or {0.03,0.03,0.03,0.82} return c[1],c[2],c[3],c[4] end",
["label"] = "BackgroundColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.BorderColor = {r,g,b,a} end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["Type"] = "Color",
["get"] = "return function() local c = NSRT.EncounterAlerts[3420][16].MarkerMap.BorderColor or {0.15,0.85,1,1} return c[1],c[2],c[3],c[4] end",
["label"] = "BorderColor",
},
{
["set"] = "return function(NSI, r,g,b,a) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.PlayerColor = {r,g,b,a} end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["Type"] = "Color",
["get"] = "return function() local c = NSRT.EncounterAlerts[3420][16].MarkerMap.PlayerColor or {1,1,1,1} return c[1],c[2],c[3],c[4] end",
["label"] = "PlayerColor",
},
{
["Type"] = "Checkbox",
["label"] = "ShowPlayerArrow",
["tooltip"] = {
["title"] = "ShowPlayerArrow",
["desc"] = "Shows the player at the center, facing toward the top of the map.",
},
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.ShowPlayerArrow = v end NSI.EncounterAlertStop[3420](NSI) NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\") end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.ShowPlayerArrow ~= false end",
},
{
["Type"] = "Checkbox",
["label"] = "Active Entire Fight",
["tooltip"] = {
["title"] = "Active Entire Fight",
["desc"] = "Keeps the marker map visible from pull until the encounter ends instead of only around configured mechanic windows.",
},
["set"] = "return function(NSI, v) for i=14, 16 do NSRT.EncounterAlerts[3420][i].MarkerMap.ActiveEntireFight = v end end",
["get"] = "return function() return NSRT.EncounterAlerts[3420][16].MarkerMap.ActiveEntireFight == true end",
},
{
["text"] = "World marker positions",
["Type"] = "Label",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "North",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[1] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[1] or 1\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "Northeast",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[2] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[2] or 2\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "East",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[3] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[3] or 3\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "Southeast",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[4] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[4] or 4\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "South",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[5] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[5] or 5\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "Southwest",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[6] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[6] or 6\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "West",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[7] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[7] or 7\n            end",
},
{
["Type"] = "Dropdown",
["values"] = "return function(NSI)\n        local values = {{label = \"None\", value = 0}}\n        for markerID = 1, 8 do\n            values[#values + 1] = {\n                label = string.format(\"|TInterface\\\\TargetingFrame\\\\UI-RaidTargetingIcon_%d:18:18|t %s\", markerID, _G[\"RAID_TARGET_\" .. markerID]),\n                value = markerID,\n            }\n        end\n        return values\n    end",
["label"] = "Northwest",
["set"] = "return function(NSI, value)\n                value = tonumber(value)\n                if value == nil or value < 0 or value > 8 then return end\n                for difficultyID = 14, 16 do\n                    local settings = NSRT.EncounterAlerts[3420][difficultyID].MarkerMap\n                    settings.MarkerOrder = settings.MarkerOrder or {3,8,4,5,6,7,1,2}\n                    settings.MarkerOrder[8] = value\n                end\n                NSI.EncounterAlertStop[3420](NSI)\n                NSI.EncounterAlertStart[3420](NSI, 16, \"Marker Map\")\n            end",
["get"] = "return function()\n                local order = NSRT.EncounterAlerts[3420][16].MarkerMap.MarkerOrder\n                return order and order[8] or 8\n            end",
},
},
["Scale"] = 1,
["yOffset"] = 150,
["customIcon"] = 137001,
["internalID"] = "MarkerMap",
["TTS"] = false,
["difficulties"] = {
14,
15,
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["name"] = "Marker Map",
["IsAlert"] = true,
["DisplayType"] = "Text",
["timers"] = {
},
["group"] = "Sszorak",
["MarkerSize"] = 34,
["ShowPlayerArrow"] = true,
["xOffset"] = -800,
["UpdateInterval"] = 0.03,
["Anchor"] = "CENTER",
["Preview"] = "        return function(self)\n            if self.IsSszorakMarkerMapPreview then\n                self.EncounterAlertStop[3420](self)\n            else\n                self.EncounterAlertStart[3420](self, 16, \"Marker Map\")\n            end\n        end\n    ",
["BackgroundColor"] = {
0.302,
0.302,
0.302,
0.82,
},
["id"] = 0.2,
["PlayerColor"] = {
1,
1,
1,
1,
},
["relativeTo"] = "CENTER",
["dur"] = 5,
["ActiveEntireFight"] = false,
["phase"] = 1,
["encID"] = 3420,
["BorderColor"] = {
0.15,
0.85,
1,
1,
},
["enabled"] = false,
["MapSize"] = 200,
["BlockCopy"] = true,
["Version"] = {
{
["MapSize"] = 200,
["BackgroundColor"] = {
0.302,
0.302,
0.302,
0.82,
},
},
["versionNumber"] = 1,
},
["MarkerOrder"] = {
3,
8,
4,
5,
6,
7,
1,
2,
},
["isSpecialDisplay"] = true,
["sticky"] = 0,
["TTSTimer"] = 5,
["NoEdit"] = true,
},
["Debuffs"] = {
["timers"] = {
37.2,
89.5,
175.4,
227.6,
313.5,
365.8,
},
["group"] = "Sszorak",
["loadConditions"] = {
["Roles"] = {
["HEALER"] = true,
["DAMAGER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3420,
["internalID"] = "Debuffs",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Debuffs",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1305963,
["name"] = "Debuffs",
["IsAlert"] = true,
["id"] = 10,
["sticky"] = 0,
["TTS"] = false,
["DisplayType"] = "Text",
},
["TankCombo"] = {
["timers"] = {
5.5,
57.7,
143.7,
196,
282,
334,
},
["group"] = "Sszorak",
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3420,
["internalID"] = "TankCombo",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Tank Combo",
["spellID"] = 1277002,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Tank Combo",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 1,
["DisplayType"] = "Text",
},
},
},
[3455] = {
[15] = {
["TankHits"] = {
["timers"] = {
10.1,
39.1,
66.1,
94.1,
123.1,
150.1,
178.1,
207.1,
234.1,
262.1,
291.1,
318.1,
346.1,
375.1,
402.1,
430.1,
459.1,
},
["group"] = "Vashnik",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss1.",
},
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3455,
["internalID"] = "TankHits",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Tank-Hit",
["spellID"] = 1280935,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Tank-Hits",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 1,
["DisplayType"] = "Text",
},
["Infection"] = {
["timers"] = {
42.3,
95,
126,
171,
210.3,
263,
294.6,
347,
378.1,
431,
462.3,
},
["group"] = "Vashnik",
["encID"] = 3455,
["internalID"] = "Infection",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Infection",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1282114,
["name"] = "Infection",
["IsAlert"] = true,
["id"] = 6,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["WaveSpread"] = {
["timers"] = {
13,
54,
87,
138,
171,
222,
255,
306,
339,
390,
423,
474,
},
["group"] = "Vashnik",
["loadConditions"] = {
["Roles"] = {
["HEALER"] = true,
["DAMAGER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3455,
["internalID"] = "WaveSpread",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Pre-Spread",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1281908,
["name"] = "Wave-Spread",
["IsAlert"] = true,
["id"] = 14,
["sticky"] = 0,
["TTS"] = "Spread",
["DisplayType"] = "Circle",
},
["Taunts"] = {
["timers"] = {
10.6,
39.6,
66.6,
94.6,
123.6,
150.6,
178.6,
207.6,
234.6,
262.6,
291.6,
318.6,
346.6,
375.6,
402.6,
430.6,
459.6,
},
["group"] = "Vashnik",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat < 2 end",
["text"] = "This Alert only shows if you do not have threat on boss1.",
},
["customIcon"] = 355,
["encID"] = 3455,
["internalID"] = "Taunts",
["TTS"] = true,
["TTSTimer"] = 0,
["dur"] = 6,
["phase"] = 1,
["id"] = 3,
["text"] = "Taunt",
["name"] = "Taunt",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["isTaunt"] = true,
["textColors"] = {
0,
1,
0,
1,
},
["IsAlert"] = true,
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["sticky"] = 3,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Soaks"] = {
["timers"] = {
42,
81,
126.1,
165,
210.1,
249.1,
294.1,
333.1,
378.1,
417.2,
462.2,
},
["group"] = "Vashnik",
["encID"] = 3455,
["internalID"] = "Soaks",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Soaks",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1282602,
["name"] = "Soaks",
["IsAlert"] = true,
["id"] = 10,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["AoE"] = {
["timers"] = {
35,
74,
119.1,
158,
203.1,
242.1,
287.1,
326.1,
371.1,
410.2,
455.2,
},
["group"] = "Vashnik",
["encID"] = 3455,
["internalID"] = "AoE",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "AoE",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1282516,
["name"] = "AoE",
["IsAlert"] = true,
["id"] = 8,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Waves"] = {
["timers"] = {
21,
62,
95,
146,
179,
230,
263,
314,
347,
398,
431,
482,
},
["group"] = "Vashnik",
["encID"] = 3455,
["internalID"] = "Waves",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Waves",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1281908,
["name"] = "Waves",
["IsAlert"] = true,
["id"] = 12,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
},
[16] = {
["WaveSpread"] = {
["timers"] = {
13,
54.1,
87.1,
138.1,
171.1,
222.1,
255.1,
306.1,
339.1,
390.1,
423.1,
474.1,
},
["group"] = "Vashnik",
["loadConditions"] = {
["Roles"] = {
["HEALER"] = true,
["DAMAGER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3455,
["internalID"] = "WaveSpread",
["enabled"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Pre-Spread",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1281908,
["name"] = "Wave-Spread",
["IsAlert"] = true,
["id"] = 15,
["sticky"] = 0,
["TTS"] = "Spread",
["DisplayType"] = "Circle",
},
["Infection"] = {
["timers"] = {
42.4,
94.3,
126.3,
178.3,
210.4,
262.6,
294.4,
346.5,
378.6,
430.6,
462.6,
},
["group"] = "Vashnik",
["encID"] = 3455,
["internalID"] = "Infection",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Infection",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1282114,
["name"] = "Infection",
["IsAlert"] = true,
["id"] = 7,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Adds"] = {
["timers"] = {
24,
108,
192,
276.1,
360.1,
444.1,
462.1,
473,
483.1,
},
["group"] = "Vashnik",
["encID"] = 3455,
["internalID"] = "Adds",
["TTS"] = true,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Adds",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1284663,
["name"] = "Adds",
["IsAlert"] = true,
["id"] = 5,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["WavesLine"] = {
["extraOptions"] = {
{
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3455][16].WavesLine.ShowEntireFight = value NSI.EncounterAlertStop[3455](NSI) NSI.EncounterAlertStart[3455](NSI, 16, \"Waves Line\") end",
["Type"] = "Checkbox",
["get"] = "return function(NSI) return NSRT.EncounterAlerts[3455][16].WavesLine.ShowEntireFight end",
["label"] = "Show Entire Fight",
},
},
["group"] = "Vashnik",
["loadConditions"] = {
["Roles"] = {
["HEALER"] = true,
["DAMAGER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3455,
["Preview"] = "return function(NSI) NSI:PreviewVashnikWavesLine() end",
["internalID"] = "WavesLine",
["NoEdit"] = true,
["enabled"] = false,
["isSpecialDisplay"] = true,
["BlockCopy"] = true,
["difficulties"] = {
16,
},
["phase"] = 1,
["text"] = "",
["timers"] = {
},
["ShowEntireFight"] = false,
["ReloeReminder"] = true,
["Version"] = {
{
["enabled"] = false,
["difficulties"] = {
16,
},
["loadConditions"] = {
["Roles"] = {
["HEALER"] = true,
["DAMAGER"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
},
["versionNumber"] = 1,
},
["name"] = "Waves Line",
["IsAlert"] = true,
["DefaultEnabled"] = false,
["sticky"] = 0,
["id"] = 16,
["DisplayType"] = "Text",
},
["Soaks"] = {
["timers"] = {
42,
81,
126,
165.1,
210.1,
249.1,
294.1,
333.1,
378.1,
417.2,
462.2,
},
["group"] = "Vashnik",
["encID"] = 3455,
["internalID"] = "Soaks",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Soaks",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1282602,
["name"] = "Soaks",
["IsAlert"] = true,
["id"] = 11,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Taunts"] = {
["timers"] = {
10.5,
39.5,
66.5,
94.5,
123.6,
150.6,
178.6,
207.6,
234.6,
262.6,
291.6,
318.6,
346.6,
375.6,
402.6,
430.6,
459.6,
},
["group"] = "Vashnik",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat < 2 end",
["text"] = "This Alert only shows if you do not have threat on boss1.",
},
["customIcon"] = 355,
["encID"] = 3455,
["internalID"] = "Taunts",
["TTS"] = true,
["TTSTimer"] = 0,
["dur"] = 6,
["phase"] = 1,
["id"] = 4,
["text"] = "Taunt",
["name"] = "Taunt",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["isTaunt"] = true,
["textColors"] = {
0,
1,
0,
1,
},
["IsAlert"] = true,
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["sticky"] = 3,
["enabled"] = false,
["DisplayType"] = "Text",
},
["TankHits"] = {
["timers"] = {
10,
39,
66,
94,
123.1,
150.1,
178.1,
207.1,
234.1,
262.1,
291.1,
318.1,
346.1,
375.1,
402.1,
430.1,
459.1,
},
["group"] = "Vashnik",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss1.",
},
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3455,
["internalID"] = "TankHits",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Tank-Hit",
["spellID"] = 1280935,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Tank-Hits",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 2,
["DisplayType"] = "Text",
},
["AoE"] = {
["timers"] = {
35,
74,
119,
158.1,
203.1,
242.1,
287.1,
326.1,
371.1,
410.2,
455.2,
},
["group"] = "Vashnik",
["encID"] = 3455,
["internalID"] = "AoE",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "AoE",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1282516,
["name"] = "AoE",
["IsAlert"] = true,
["id"] = 9,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Waves"] = {
["timers"] = {
19,
60.1,
93.1,
144.1,
177.1,
228.1,
261.1,
312.1,
345.1,
396.1,
429.1,
480.1,
},
["group"] = "Vashnik",
["encID"] = 3455,
["internalID"] = "Waves",
["TTS"] = false,
["TTSTimer"] = 6,
["dur"] = 6,
["phase"] = 1,
["text"] = "Waves",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1281908,
["name"] = "Waves",
["IsAlert"] = true,
["id"] = 13,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
},
},
[3178] = {
},
[3180] = {
},
[3492] = {
[15] = {
["Debuffs"] = {
["timers"] = {
392.7,
463.7,
500.6,
560.7,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "Debuffs",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Debuffs",
["spellID"] = 1288879,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Serpent's Bite",
["Version"] = {
{
["dur"] = 8,
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 13,
["DisplayType"] = "Text",
},
["GraspingFangsOverview"] = {
["isSpecialDisplay"] = true,
["RightGroups"] = "3,4",
["internalID"] = "GraspingFangsOverview",
["LeftInactiveColor"] = {
0.32,
0.02,
0.02,
0.85,
},
["enabled"] = false,
["RightBackgroundColor"] = {
0,
0.45,
1,
1,
},
["difficulties"] = {
15,
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["name"] = "Grasping Fangs Overview",
["IsAlert"] = true,
["DisplayType"] = "Bar",
["timers"] = {
180,
},
["group"] = "Ula'tek",
["SortByRole"] = true,
["Preview"] = "return function(self) self:PreviewUlatekGraspingFangsOverviews() end",
["LeftBackgroundColor"] = {
1,
0,
0,
1,
},
["TTSTimer"] = 30,
["BlockCopy"] = true,
["RightInactiveColor"] = {
0.02,
0.155,
0.32,
0.85,
},
["id"] = 0.2,
["extraOptions"] = {
{
["set"] = "return function(NSI, r, g, b, a) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.LeftBackgroundColor = {r, g, b, a} end NSI:UpdateUlatekGraspingFangsOverviews() end",
["Type"] = "Color",
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview local c = a.LeftBackgroundColor or {1, 0, 0, 1} return c[1], c[2], c[3], c[4] end",
["label"] = "Left Side Background Color",
},
{
["set"] = "return function(NSI, r, g, b, a) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.RightBackgroundColor = {r, g, b, a} end NSI:UpdateUlatekGraspingFangsOverviews() end",
["Type"] = "Color",
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview local c = a.RightBackgroundColor or {0, 0.45, 1, 1} return c[1], c[2], c[3], c[4] end",
["label"] = "Right Side Background Color",
},
{
["Type"] = "Color",
["label"] = "Left Side Inactive Color",
["tooltip"] = {
["title"] = "Left Side Inactive Color",
["desc"] = "Color of the left side's rows while that player does not have the debuff.",
},
["set"] = "return function(NSI, r, g, b, a) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.LeftInactiveColor = {r, g, b, a} end NSI:UpdateUlatekGraspingFangsOverviews() end",
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview local c = a.LeftInactiveColor or {0.32, 0.02, 0.02, 0.85} return c[1], c[2], c[3], c[4] end",
},
{
["Type"] = "Color",
["label"] = "Right Side Inactive Color",
["tooltip"] = {
["title"] = "Right Side Inactive Color",
["desc"] = "Color of the right side's rows while that player does not have the debuff.",
},
["set"] = "return function(NSI, r, g, b, a) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.RightInactiveColor = {r, g, b, a} end NSI:UpdateUlatekGraspingFangsOverviews() end",
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview local c = a.RightInactiveColor or {0.02, 0.155, 0.32, 0.85} return c[1], c[2], c[3], c[4] end",
},
{
["Type"] = "Checkbox",
["label"] = "Show All Players",
["tooltip"] = {
["title"] = "Show All Players",
["desc"] = "Keeps a row up for every player on that side, in the inactive color, and switches it to the regular color while they have the debuff. Turn off to only show players who currently have the debuff.",
},
["set"] = "return function(NSI, value) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.ShowAllPlayers = value and true or false end NSI:UpdateUlatekGraspingFangsOverviews() end",
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview return a.ShowAllPlayers ~= false end",
},
{
["Type"] = "TextEntry",
["inputWidth"] = 120,
["label"] = "Left Side Groups",
["tooltip"] = {
["title"] = "Left Side Groups",
["desc"] = "Raid subgroups shown on the left side, comma separated. Use 1,2 and 3,4 to split by halves, or 1,3,5,7 and 2,4,6,8 for odds and evens.",
},
["set"] = "return function(NSI, value) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.LeftGroups = value end NSI:UpdateUlatekGraspingFangsOverviews() end",
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].GraspingFangsOverview.LeftGroups or \"1,2,3,4,5,6,7,8\" end",
},
{
["Type"] = "TextEntry",
["inputWidth"] = 120,
["label"] = "Right Side Groups",
["tooltip"] = {
["title"] = "Right Side Groups",
["desc"] = "Raid subgroups shown on the right side, comma separated. Empty by default, so only the left side is shown. A group left out of both sides is not tracked.",
},
["set"] = "return function(NSI, value) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.RightGroups = value end NSI:UpdateUlatekGraspingFangsOverviews() end",
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].GraspingFangsOverview.RightGroups or \"\" end",
},
{
["set"] = "return function(NSI, value) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.SortByRole = value and true or false end NSI:UpdateUlatekGraspingFangsOverviews() end",
["Type"] = "Checkbox",
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview return a.SortByRole == true end",
["label"] = "Sort by Role",
},
{
["min"] = 10,
["Type"] = "Slider",
["label"] = "Bar Height",
["max"] = 100,
["set"] = "return function(NSI, value) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.BarHeight = value end NSI:UpdateUlatekGraspingFangsOverviews() end",
["step"] = 1,
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview return a.BarHeight or NSRT.ReminderSettings.DebuffOverviewSettings.Height end",
},
},
["ShowAllPlayers"] = true,
["dur"] = 30,
["spellID"] = 1311611,
["encID"] = 3492,
["Version"] = {
{
["LeftGroups"] = "1,2",
["RightGroups"] = "3,4",
["RightBackgroundColor"] = {
0,
0.45,
1,
1,
},
["LeftInactiveColor"] = {
0.32,
0.02,
0.02,
0.85,
},
["SortByRole"] = true,
["RightInactiveColor"] = {
0.02,
0.155,
0.32,
0.85,
},
["ShowAllPlayers"] = true,
["LeftBackgroundColor"] = {
1,
0,
0,
1,
},
},
{
["dur"] = 30,
},
["versionNumber"] = 2,
},
["LeftGroups"] = "1,2",
["TTS"] = false,
["sticky"] = 0,
["phase"] = 1,
["NoEdit"] = true,
},
["PlatformBreak"] = {
["timers"] = {
430.1,
481.2,
542.1,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "PlatformBreak",
["enabled"] = false,
["TTSTimer"] = 10,
["dur"] = 10,
["phase"] = 1,
["text"] = "Platform Break",
["spellID"] = 1315341,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Circling Prey",
["Version"] = {
{
["dur"] = 10,
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 11,
["DisplayType"] = "Text",
},
["Sweep"] = {
["timers"] = {
38.9,
90.9,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "Sweep",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Sweep",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1296301,
["name"] = "Sweep",
["IsAlert"] = true,
["id"] = 22,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["DamageAmpIn"] = {
["timers"] = {
135.4,
284.5,
573.5,
},
["group"] = "Ula'tek",
["customIcon"] = 1299526,
["encID"] = 3492,
["internalID"] = "DamageAmpIn",
["TTS"] = false,
["TTSTimer"] = 15,
["dur"] = 15,
["phase"] = 1,
["text"] = "Dmg amp in",
["spellID"] = 1286860,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["dur"] = 15,
},
{
["customIcon"] = 1299526,
},
{
["name"] = "Dmg amp in",
},
["versionNumber"] = 3,
},
["name"] = "Dmg amp in",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 5,
["DisplayType"] = "Text",
},
["DamageAmp"] = {
["timers"] = {
155.4,
304.5,
597,
},
["group"] = "Ula'tek",
["barColors"] = {
1,
0,
0,
1,
},
["customIcon"] = 1299526,
["encID"] = 3492,
["internalID"] = "DamageAmp",
["TTS"] = false,
["TTSTimer"] = 20,
["dur"] = 20,
["phase"] = 1,
["text"] = "Dmg amp",
["spellID"] = 1299526,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["customIcon"] = 1299526,
},
{
["name"] = "Dmg amp Timer",
},
["versionNumber"] = 2,
},
["name"] = "Dmg amp Timer",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 7,
["DisplayType"] = "Bar",
},
["TransitionSoakSecond"] = {
["timers"] = {
329.6,
337.3,
344.6,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "TransitionSoakSecond",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["spellID"] = 1299010,
["id"] = 27,
["text"] = "Second Soak",
["enabled"] = false,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["textColors"] = {
1,
0,
0,
1,
},
["name"] = "Second Soak",
["IsAlert"] = true,
["difficulties"] = {
15,
},
["sticky"] = 0,
["phase"] = 1,
["DisplayType"] = "Text",
},
["WrongTarget"] = {
["timers"] = {
135.4,
284.5,
573.5,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["HideTimer"] = true,
["dur"] = 20,
["TTS"] = false,
["TTSTimer"] = 20,
["BlockCopy"] = true,
["enabled"] = false,
["phase"] = 1,
["text"] = "WRONG TARGET",
["internalID"] = "WrongTarget",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Wrong Target",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["isSpecialDisplay"] = true,
["sticky"] = 20,
["id"] = 9,
["DisplayType"] = "Text",
},
["Waves"] = {
["timers"] = {
48,
100,
416.7,
471.7,
521.7,
566.7,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "Waves",
["TTS"] = "Waves",
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Waves",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1292403,
["name"] = "Caustic Wave",
["IsAlert"] = true,
["id"] = 3,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Soak"] = {
["timers"] = {
28,
30.4,
122.8,
125.6,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "Soak",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Soak",
["spellID"] = 1299010,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Soak",
["Version"] = {
{
["dur"] = 8,
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 24,
["DisplayType"] = "Text",
},
["Adds"] = {
["timers"] = {
372.2,
402.1,
447.1,
507.2,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "Adds",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Adds",
["spellID"] = 1300751,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P3 Adds",
["Version"] = {
{
["dur"] = 8,
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = true,
["sticky"] = 0,
["id"] = 19,
["DisplayType"] = "Text",
},
["HitKnock"] = {
["timers"] = {
14.9,
81.9,
118.9,
377.1,
452.1,
528.1,
617.1,
732.1,
828.1,
},
["group"] = "Ula'tek",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss1.",
},
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3492,
["internalID"] = "HitKnock",
["TTS"] = "Knock",
["TTSTimer"] = 10,
["dur"] = 10,
["spellID"] = 1298367,
["id"] = 1,
["text"] = "Hit+Knock",
["textColors"] = {
1,
0,
0,
1,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["dur"] = 10,
},
["versionNumber"] = 1,
},
["name"] = "Mother's Wrath",
["IsAlert"] = true,
["phase"] = 1,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["TransitionSoakFirst"] = {
["timers"] = {
326.3,
334.5,
341.4,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "TransitionSoakFirst",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["spellID"] = 1299010,
["id"] = 26,
["text"] = "First Soak",
["enabled"] = false,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["textColors"] = {
0,
1,
0,
1,
},
["name"] = "First Soak",
["IsAlert"] = true,
["difficulties"] = {
15,
},
["sticky"] = 0,
["phase"] = 1,
["DisplayType"] = "Text",
},
["Eggs"] = {
["timers"] = {
82,
319,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "Eggs",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Eggs",
["spellID"] = 1304012,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Eggs",
["Version"] = {
{
["dur"] = 8,
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 17,
["DisplayType"] = "Text",
},
},
[16] = {
["Debuffs"] = {
["timers"] = {
397,
461,
565,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "Debuffs",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Debuffs",
["spellID"] = 1288879,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Serpent's Bite",
["Version"] = {
{
["dur"] = 8,
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 14,
["DisplayType"] = "Text",
},
["Adds"] = {
["timers"] = {
382.3,
412.1,
510.2,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "Adds",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Adds",
["spellID"] = 1300751,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "P3 Adds",
["Version"] = {
{
["dur"] = 8,
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = true,
["sticky"] = 0,
["id"] = 20,
["DisplayType"] = "Text",
},
["WaveDirectionPrompt"] = {
["isSpecialDisplay"] = true,
["group"] = "Ula'tek",
["NoEdit"] = true,
["encID"] = 3492,
["HideTimer"] = true,
["internalID"] = "WaveDirectionPrompt",
["TTS"] = false,
["TTSTimer"] = 12,
["dur"] = 12,
["phase"] = 1,
["id"] = 29,
["text"] = "Input Direction",
["difficulties"] = {
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["enabled"] = false,
["name"] = "Wave Direction Input",
["IsAlert"] = true,
["timers"] = {
},
["sticky"] = 0,
["BlockCopy"] = true,
["DisplayType"] = "Text",
},
["InterruptAssignments"] = {
["extraOptions"] = {
{
["height"] = 60,
["Type"] = "Label",
["text"] = "The Interrupt display will be displayed for the add that you focused. The order of lines in the interrupt note does not matter since it's not assigned to an actual boss unit but just to whatever you focus. Use raidmarker to ensure that people are focusing the same add.",
},
{
["min"] = 8,
["Type"] = "Slider",
["label"] = "Number Font Size",
["max"] = 40,
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].InterruptAssignments.NumberFontSize = value NSI:UpdateUlatekInterruptDisplay() end",
["step"] = 1,
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].InterruptAssignments.NumberFontSize or 12 end",
},
{
["min"] = 8,
["Type"] = "Slider",
["label"] = "Name Font Size",
["max"] = 40,
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].InterruptAssignments.NameFontSize = value NSI:UpdateUlatekInterruptDisplay() end",
["step"] = 1,
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].InterruptAssignments.NameFontSize or 12 end",
},
{
["min"] = 30,
["Type"] = "Slider",
["label"] = "Box Size",
["max"] = 150,
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].InterruptAssignments.BoxSize = value NSI:UpdateUlatekInterruptDisplay() end",
["step"] = 1,
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].InterruptAssignments.BoxSize or 30 end",
},
{
["Type"] = "Dropdown",
["values"] = {
{
["value"] = "TOP",
["label"] = "Top",
},
{
["value"] = "CENTER",
["label"] = "Center",
},
{
["value"] = "LEFT",
["label"] = "Left",
},
{
["value"] = "RIGHT",
["label"] = "Right",
},
{
["value"] = "BOTTOM",
["label"] = "Bottom",
},
},
["label"] = "Nameplate Anchor",
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].InterruptAssignments.NameplateAnchor = value NSI:UpdateUlatekInterruptDisplay() end",
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].InterruptAssignments.NameplateAnchor or \"TOP\" end",
},
{
["min"] = -200,
["Type"] = "Slider",
["label"] = "Nameplate X Offset",
["max"] = 200,
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].InterruptAssignments.NameplateXOffset = value NSI:UpdateUlatekInterruptDisplay() end",
["step"] = 1,
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].InterruptAssignments.NameplateXOffset or 0 end",
},
{
["min"] = -200,
["Type"] = "Slider",
["label"] = "Nameplate Y Offset",
["max"] = 200,
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].InterruptAssignments.NameplateYOffset = value NSI:UpdateUlatekInterruptDisplay() end",
["step"] = 1,
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].InterruptAssignments.NameplateYOffset or 0 end",
},
{
["Type"] = "Checkbox",
["label"] = "Hide nameplate box",
["tooltip"] = {
["title"] = "Hide nameplate box",
["desc"] = "Hide the nameplate box while keeping the static box visible.",
},
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].InterruptAssignments.HideNameplateBox = value NSI:UpdateUlatekInterruptDisplay() end",
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].InterruptAssignments.HideNameplateBox or false end",
},
},
["NumberFontSize"] = 12,
["customIcon"] = 6552,
["internalID"] = "InterruptAssignments",
["TTS"] = false,
["NameplateYOffset"] = 0,
["difficulties"] = {
16,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Interrupt Assignments",
["IsAlert"] = true,
["NameplateAnchor"] = "TOP",
["DisplayType"] = "Text",
["timers"] = {
},
["group"] = "Ula'tek",
["HideNameplateBox"] = false,
["encID"] = 3492,
["Preview"] = "return function(NSI)\n        if NSI:PreviewUlatekInterruptDisplay() then\n            print(NSI:Loc(\"|cFF00FFFFNSRT:|r the live display uses the global Interrupt Display settings during this encounter.\"))\n        end\n    end",
["TTSTimer"] = 1,
["dur"] = 1,
["BoxSize"] = 30,
["phase"] = 1,
["id"] = 30,
["text"] = "Interrupts",
["isSpecialDisplay"] = true,
["NameFontSize"] = 12,
["enabled"] = true,
["Version"] = {
{
["group"] = "Ula'tek",
},
["versionNumber"] = 1,
},
["NameplateXOffset"] = 0,
["pinned"] = true,
["sticky"] = 0,
["BlockCopy"] = true,
["NoEdit"] = true,
},
["EggDeadline"] = {
["timers"] = {
29,
113,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "EggDeadline",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["difficulties"] = {
16,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["id"] = 15,
["name"] = "Egg Deadline",
["IsAlert"] = true,
["text"] = "Egg Deadline",
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Waves"] = {
["timers"] = {
56,
113,
426.5,
481.6,
531.8,
575.6,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "Waves",
["TTS"] = "Waves",
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Waves",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1292403,
["name"] = "Caustic Wave",
["IsAlert"] = true,
["id"] = 4,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["Soak"] = {
["timers"] = {
40.3,
43.5,
134.3,
138,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "Soak",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Soak",
["spellID"] = 1299010,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Soak",
["Version"] = {
{
["dur"] = 8,
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 25,
["DisplayType"] = "Text",
},
["GraspingFangsOverview"] = {
["isSpecialDisplay"] = true,
["RightGroups"] = "3,4",
["internalID"] = "GraspingFangsOverview",
["LeftInactiveColor"] = {
0.32,
0.02,
0.02,
0.85,
},
["enabled"] = false,
["RightBackgroundColor"] = {
0,
0.45,
1,
1,
},
["difficulties"] = {
15,
16,
},
["DefaultEnabled"] = false,
["ReloeReminder"] = true,
["name"] = "Grasping Fangs Overview",
["IsAlert"] = true,
["DisplayType"] = "Bar",
["timers"] = {
190,
},
["group"] = "Ula'tek",
["SortByRole"] = true,
["Preview"] = "return function(self) self:PreviewUlatekGraspingFangsOverviews() end",
["LeftBackgroundColor"] = {
1,
0,
0,
1,
},
["TTSTimer"] = 30,
["BlockCopy"] = true,
["RightInactiveColor"] = {
0.02,
0.155,
0.32,
0.85,
},
["id"] = 0.2,
["extraOptions"] = {
{
["set"] = "return function(NSI, r, g, b, a) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.LeftBackgroundColor = {r, g, b, a} end NSI:UpdateUlatekGraspingFangsOverviews() end",
["Type"] = "Color",
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview local c = a.LeftBackgroundColor or {1, 0, 0, 1} return c[1], c[2], c[3], c[4] end",
["label"] = "Left Side Background Color",
},
{
["set"] = "return function(NSI, r, g, b, a) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.RightBackgroundColor = {r, g, b, a} end NSI:UpdateUlatekGraspingFangsOverviews() end",
["Type"] = "Color",
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview local c = a.RightBackgroundColor or {0, 0.45, 1, 1} return c[1], c[2], c[3], c[4] end",
["label"] = "Right Side Background Color",
},
{
["Type"] = "Color",
["label"] = "Left Side Inactive Color",
["tooltip"] = {
["title"] = "Left Side Inactive Color",
["desc"] = "Color of the left side's rows while that player does not have the debuff.",
},
["set"] = "return function(NSI, r, g, b, a) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.LeftInactiveColor = {r, g, b, a} end NSI:UpdateUlatekGraspingFangsOverviews() end",
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview local c = a.LeftInactiveColor or {0.32, 0.02, 0.02, 0.85} return c[1], c[2], c[3], c[4] end",
},
{
["Type"] = "Color",
["label"] = "Right Side Inactive Color",
["tooltip"] = {
["title"] = "Right Side Inactive Color",
["desc"] = "Color of the right side's rows while that player does not have the debuff.",
},
["set"] = "return function(NSI, r, g, b, a) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.RightInactiveColor = {r, g, b, a} end NSI:UpdateUlatekGraspingFangsOverviews() end",
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview local c = a.RightInactiveColor or {0.02, 0.155, 0.32, 0.85} return c[1], c[2], c[3], c[4] end",
},
{
["Type"] = "Checkbox",
["label"] = "Show All Players",
["tooltip"] = {
["title"] = "Show All Players",
["desc"] = "Keeps a row up for every player on that side, in the inactive color, and switches it to the regular color while they have the debuff. Turn off to only show players who currently have the debuff.",
},
["set"] = "return function(NSI, value) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.ShowAllPlayers = value and true or false end NSI:UpdateUlatekGraspingFangsOverviews() end",
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview return a.ShowAllPlayers ~= false end",
},
{
["Type"] = "TextEntry",
["inputWidth"] = 120,
["label"] = "Left Side Groups",
["tooltip"] = {
["title"] = "Left Side Groups",
["desc"] = "Raid subgroups shown on the left side, comma separated. Use 1,2 and 3,4 to split by halves, or 1,3,5,7 and 2,4,6,8 for odds and evens.",
},
["set"] = "return function(NSI, value) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.LeftGroups = value end NSI:UpdateUlatekGraspingFangsOverviews() end",
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].GraspingFangsOverview.LeftGroups or \"1,2,3,4,5,6,7,8\" end",
},
{
["Type"] = "TextEntry",
["inputWidth"] = 120,
["label"] = "Right Side Groups",
["tooltip"] = {
["title"] = "Right Side Groups",
["desc"] = "Raid subgroups shown on the right side, comma separated. Empty by default, so only the left side is shown. A group left out of both sides is not tracked.",
},
["set"] = "return function(NSI, value) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.RightGroups = value end NSI:UpdateUlatekGraspingFangsOverviews() end",
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].GraspingFangsOverview.RightGroups or \"\" end",
},
{
["set"] = "return function(NSI, value) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.SortByRole = value and true or false end NSI:UpdateUlatekGraspingFangsOverviews() end",
["Type"] = "Checkbox",
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview return a.SortByRole == true end",
["label"] = "Sort by Role",
},
{
["min"] = 10,
["Type"] = "Slider",
["label"] = "Bar Height",
["max"] = 100,
["set"] = "return function(NSI, value) for i = 15, 16 do NSRT.EncounterAlerts[3492][i].GraspingFangsOverview.BarHeight = value end NSI:UpdateUlatekGraspingFangsOverviews() end",
["step"] = 1,
["get"] = "return function() local a = NSRT.EncounterAlerts[3492][16].GraspingFangsOverview return a.BarHeight or NSRT.ReminderSettings.DebuffOverviewSettings.Height end",
},
},
["ShowAllPlayers"] = true,
["dur"] = 30,
["spellID"] = 1311611,
["encID"] = 3492,
["Version"] = {
{
["LeftGroups"] = "1,2",
["RightGroups"] = "3,4",
["RightBackgroundColor"] = {
0,
0.45,
1,
1,
},
["LeftInactiveColor"] = {
0.32,
0.02,
0.02,
0.85,
},
["SortByRole"] = true,
["RightInactiveColor"] = {
0.02,
0.155,
0.32,
0.85,
},
["ShowAllPlayers"] = true,
["LeftBackgroundColor"] = {
1,
0,
0,
1,
},
},
{
["dur"] = 30,
},
["versionNumber"] = 2,
},
["LeftGroups"] = "1,2",
["TTS"] = false,
["sticky"] = 0,
["phase"] = 1,
["NoEdit"] = true,
},
["Eggs"] = {
["timers"] = {
82,
319,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "Eggs",
["enabled"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["text"] = "Eggs",
["spellID"] = 1304012,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Eggs",
["Version"] = {
{
["dur"] = 8,
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 18,
["DisplayType"] = "Text",
},
["Sweep"] = {
["timers"] = {
49,
106,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "Sweep",
["TTS"] = false,
["TTSTimer"] = 5,
["dur"] = 5,
["phase"] = 1,
["text"] = "Sweep",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["spellID"] = 1296301,
["name"] = "Sweep",
["IsAlert"] = true,
["id"] = 23,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["DamageAmpIn"] = {
["timers"] = {
145.4,
294.5,
583.6,
},
["group"] = "Ula'tek",
["customIcon"] = 1299526,
["encID"] = 3492,
["internalID"] = "DamageAmpIn",
["TTS"] = false,
["TTSTimer"] = 15,
["dur"] = 15,
["phase"] = 1,
["text"] = "Dmg amp in",
["spellID"] = 1286860,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["dur"] = 15,
},
{
["customIcon"] = 1299526,
},
{
["name"] = "Dmg amp in",
},
["versionNumber"] = 3,
},
["name"] = "Dmg amp in",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 6,
["DisplayType"] = "Text",
},
["BossSpawn"] = {
["timers"] = {
284.5,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "BossSpawn",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["difficulties"] = {
16,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["id"] = 21,
["name"] = "Boss Spawn",
["IsAlert"] = true,
["text"] = "Boss Spawn",
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["DamageAmp"] = {
["timers"] = {
165.4,
314.5,
603.6,
},
["group"] = "Ula'tek",
["barColors"] = {
1,
0,
0,
1,
},
["customIcon"] = 1299526,
["encID"] = 3492,
["internalID"] = "DamageAmp",
["TTS"] = false,
["TTSTimer"] = 20,
["dur"] = 20,
["phase"] = 1,
["text"] = "Dmg amp",
["spellID"] = 1299526,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["customIcon"] = 1299526,
},
{
["name"] = "Dmg amp Timer",
},
["versionNumber"] = 2,
},
["name"] = "Dmg amp Timer",
["IsAlert"] = true,
["enabled"] = false,
["sticky"] = 0,
["id"] = 8,
["DisplayType"] = "Bar",
},
["WrongTarget"] = {
["timers"] = {
145.4,
294.5,
583.6,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["HideTimer"] = true,
["dur"] = 20,
["TTS"] = false,
["TTSTimer"] = 20,
["BlockCopy"] = true,
["enabled"] = false,
["phase"] = 1,
["text"] = "WRONG TARGET",
["internalID"] = "WrongTarget",
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Wrong Target",
["textColors"] = {
1,
0,
0,
1,
},
["IsAlert"] = true,
["isSpecialDisplay"] = true,
["sticky"] = 20,
["id"] = 10,
["DisplayType"] = "Text",
},
["AddSoak"] = {
["timers"] = {
39.6,
72.6,
107.6,
141.6,
465.4,
497.4,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "AddSoak",
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["phase"] = 1,
["difficulties"] = {
16,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["id"] = 16,
["name"] = "Add Soak",
["IsAlert"] = true,
["text"] = "Add Soak",
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["WaveLines"] = {
["timers"] = {
423,
488,
590,
},
["group"] = "Ula'tek",
["NoEdit"] = true,
["encID"] = 3492,
["Preview"] = "return function(NSI) NSI:PreviewUlatekWaveLines() end",
["internalID"] = "WaveLines",
["id"] = 31,
["enabled"] = true,
["TTSTimer"] = 10,
["BlockCopy"] = true,
["text"] = "",
["phase"] = 1,
["difficulties"] = {
16,
},
["dur"] = 10,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["TTS"] = false,
["name"] = "P3 Wave lines",
["IsAlert"] = true,
["DisplayType"] = "Text",
["sticky"] = 0,
["isSpecialDisplay"] = true,
["spellID"] = 1316356,
},
["PlatformBreak"] = {
["timers"] = {
440.7,
491.7,
552.7,
},
["group"] = "Ula'tek",
["encID"] = 3492,
["internalID"] = "PlatformBreak",
["enabled"] = false,
["TTSTimer"] = 10,
["dur"] = 10,
["phase"] = 1,
["text"] = "Platform Break",
["spellID"] = 1315341,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["name"] = "Circling Prey",
["Version"] = {
{
["dur"] = 10,
},
["versionNumber"] = 1,
},
["IsAlert"] = true,
["TTS"] = false,
["sticky"] = 0,
["id"] = 12,
["DisplayType"] = "Text",
},
["WaveDirection"] = {
["extraOptions"] = {
{
["height"] = 70,
["Type"] = "Label",
["text"] = "Use /yell for left or /raid for Right during the input windows, from 6 seconds before until 6 seconds after each wave and submerge. The second wave combines the submerge and wave inputs.",
},
{
["Type"] = "Button",
["label"] = "Create Macros",
["tooltip"] = {
["title"] = "Create Macros",
["desc"] = "Creates a yell macro for Left and a raid macro for Right, or updates the existing macros.",
},
["func"] = "return function(NSI)\n                local macros = {\n                    {name = NSI:Loc(\"Ula'tek Left\"), icon = 450906, message = \"/yell \".. NSI:Loc(\"Left\")},\n                    {name = NSI:Loc(\"Ula'tek Right\"), icon = 450908, message = \"/raid \".. NSI:Loc(\"Right\")},\n                }\n                for macroIndex, macro in ipairs(macros) do\n                    if GetMacroInfo(macro.name) then\n                        EditMacro(macro.name, macro.name, macro.icon, macro.message)\n                    else\n                        CreateMacro(macro.name, macro.icon, macro.message)\n                    end\n                end\n            end",
["width"] = 180,
},
{
["min"] = 1,
["Type"] = "Slider",
["label"] = "Duration Seconds",
["max"] = 30,
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].WaveDirection.dur = value end",
["step"] = 1,
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].WaveDirection.dur end",
},
{
["Type"] = "TextEntry",
["inputWidth"] = 250,
["label"] = "1st Wave - Left",
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].WaveDirection[\"FirstWaveLeft\"] = value end",
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].WaveDirection[\"FirstWaveLeft\"] or \"< Left + Dodge\" end",
},
{
["Type"] = "TextEntry",
["inputWidth"] = 250,
["label"] = "1st Wave - Right",
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].WaveDirection[\"FirstWaveRight\"] = value end",
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].WaveDirection[\"FirstWaveRight\"] or \"Right > + Dodge\" end",
},
{
["Type"] = "TextEntry",
["inputWidth"] = 250,
["label"] = "2nd Wave - Submerge Left, Wave Left",
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].WaveDirection[\"SecondWaveLeftLeft\"] = value end",
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].WaveDirection[\"SecondWaveLeftLeft\"] or \"< Left + Dodge\" end",
},
{
["Type"] = "TextEntry",
["inputWidth"] = 250,
["label"] = "2nd Wave - Submerge Left, Wave Right",
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].WaveDirection[\"SecondWaveLeftRight\"] = value end",
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].WaveDirection[\"SecondWaveLeftRight\"] or \"Right > + Dodge\" end",
},
{
["Type"] = "TextEntry",
["inputWidth"] = 250,
["label"] = "2nd Wave - Submerge Right, Wave Left",
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].WaveDirection[\"SecondWaveRightLeft\"] = value end",
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].WaveDirection[\"SecondWaveRightLeft\"] or \"< Left + Dodge\" end",
},
{
["Type"] = "TextEntry",
["inputWidth"] = 250,
["label"] = "2nd Wave - Submerge Right, Wave Right",
["set"] = "return function(NSI, value) NSRT.EncounterAlerts[3492][16].WaveDirection[\"SecondWaveRightRight\"] = value end",
["get"] = "return function() return NSRT.EncounterAlerts[3492][16].WaveDirection[\"SecondWaveRightRight\"] or \"Right > + Dodge\" end",
},
},
["group"] = "Ula'tek",
["NoEdit"] = true,
["timers"] = {
56,
113,
},
["enabled"] = true,
["encID"] = 3492,
["Preview"] = "return function(self) self:PreviewUlatekWaveDirection() end",
["HideTimer"] = true,
["BlockCopy"] = true,
["TTS"] = false,
["TTSTimer"] = 8,
["dur"] = 8,
["text"] = "",
["id"] = 28,
["pinned"] = true,
["Version"] = {
{
["enabled"] = true,
["pinned"] = true,
["name"] = "Wave Direction Display",
},
["versionNumber"] = 1,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["isSpecialDisplay"] = true,
["name"] = "Wave Direction Display",
["IsAlert"] = true,
["phase"] = 1,
["sticky"] = 0,
["internalID"] = "WaveDirection",
["DisplayType"] = "Text",
},
["HitKnock"] = {
["timers"] = {
27,
97,
387.1,
472.1,
540.1,
},
["group"] = "Ula'tek",
["isConditional"] = {
["func"] = "return function() local threat = UnitThreatSituation(\"player\", \"boss1\") return threat and threat >= 2 end",
["text"] = "This Alert only shows if you have threat on boss1.",
},
["loadConditions"] = {
["Roles"] = {
["TANK"] = true,
},
["Names"] = {
},
["Classes"] = {
},
["SpecIDs"] = {
},
},
["encID"] = 3492,
["internalID"] = "HitKnock",
["TTS"] = "Knock",
["TTSTimer"] = 10,
["dur"] = 10,
["spellID"] = 1298367,
["id"] = 2,
["text"] = "Hit+Knock",
["textColors"] = {
1,
0,
0,
1,
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["Version"] = {
{
["dur"] = 10,
},
["versionNumber"] = 1,
},
["name"] = "Mother's Wrath",
["IsAlert"] = true,
["phase"] = 1,
["sticky"] = 0,
["enabled"] = false,
["DisplayType"] = "Text",
},
["TransitionPatternSoaks"] = {
["extraOptions"] = {
{
["build"] = nil --[[ skipped inline function ]],
["Type"] = "Custom",
},
{
["Type"] = "Link",
["label"] = "Copy Group Assignment Image Link",
["url"] = "https://i.imgur.com/kHYHnkv.png",
["tooltip"] = {
["title"] = "Copy Group Assignment Image Link",
["desc"] = "https://i.imgur.com/kHYHnkv.png",
},
["width"] = 250,
},
{
["Type"] = "Button",
["label"] = "Create Macros",
["tooltip"] = {
["title"] = "Create Macros",
["desc"] = "Creates the three chat macros used to select Ula'tek's transition pattern.",
},
["func"] = "return function(NSI)\n                local macros = {\n                    {name = NSI:Loc(\"Ula'tek Pattern Yell\"), icon = 137007, message = NSI:Loc(\"/yell X\")},\n                    {name = NSI:Loc(\"Ula'tek Pattern Raid\"), icon = 137001, message = NSI:Loc(\"/raid Star\")},\n                    {name = NSI:Loc(\"Ula'tek Pattern Warning\"), icon = 137004, message = NSI:Loc(\"/rw Triangle\")},\n                }\n                for _, macro in ipairs(macros) do\n                    if GetMacroInfo(macro.name) then\n                        EditMacro(macro.name, macro.name, macro.icon, macro.message)\n                    else\n                        CreateMacro(macro.name, macro.icon, macro.message)\n                    end\n                end\n            end",
["width"] = 180,
},
},
["group"] = "Ula'tek",
["NoEdit"] = true,
["isSpecialDisplay"] = true,
["dur"] = 8,
["encID"] = 3492,
["Preview"] = "return function(NSI) NSI:PreviewUlatekTransitionSoak() end",
["internalID"] = "TransitionPatternSoaks",
["pinned"] = true,
["enabled"] = true,
["TTSTimer"] = 8,
["BlockCopy"] = true,
["text"] = "Soak",
["phase"] = 1,
["difficulties"] = {
16,
},
["timers"] = {
},
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["TTS"] = false,
["name"] = "Transition Soaks",
["IsAlert"] = true,
["id"] = 32,
["sticky"] = 0,
["spellID"] = 1299010,
["DisplayType"] = "Text",
},
["TransitionPatternArrow"] = {
["isSpecialDisplay"] = true,
["xOffset"] = 0,
["timers"] = {
},
["yOffset"] = 0,
["NoEdit"] = true,
["encID"] = 3492,
["text"] = "",
["Anchor"] = "CENTER",
["Preview"] = "return function(NSI) NSI:PreviewUlatekTransitionArrow() end",
["internalID"] = "TransitionPatternArrow",
["phase"] = 1,
["TTS"] = false,
["relativeTo"] = "CENTER",
["BlockCopy"] = true,
["pinned"] = true,
["id"] = 33,
["difficulties"] = {
16,
},
["enabled"] = true,
["DefaultEnabled"] = true,
["ReloeReminder"] = true,
["TTSTimer"] = 8,
["name"] = "Transition Arrow",
["IsAlert"] = true,
["dur"] = 8,
["sticky"] = 0,
["group"] = "Ula'tek",
["DisplayType"] = "Text",
},
},
},
[3306] = {
},
[3176] = {
},
[3182] = {
},
[3181] = {
},
},
["Alerts"] = {
["Language"] = "enUS",
["Groups"] = {
["3455|Vashnik"] = {
["collapsed"] = false,
},
},
["ReloeReminders"] = false,
},
["AuraSounds"] = {
["1301118:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1301118,
},
["1305963:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1305963,
},
["1286294:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1286294,
},
["1241339:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1241339,
},
["1253834:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1253834,
},
["1280064:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1280064,
},
["1253024:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1253024,
},
["1241292:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1241292,
},
["NextCustomCategoryID"] = 1,
["1251775:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1251775,
},
["1310498:player:removed"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "removed",
["spellID"] = 1310498,
},
["1242091:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1242091,
},
["1238708:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1238708,
},
["1252675:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1252675,
},
["1241992:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1241992,
},
["1214352:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1214352,
},
["1253531:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1253531,
},
["1249020:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1249020,
},
["1282678:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1282678,
},
["1253979:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1253979,
},
["1311611:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1311611,
},
["1296025:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1296025,
},
["1288260:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1288260,
},
["1295935:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1295935,
},
["1260643:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1260643,
},
["1239111:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1239111,
},
["1287427:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1287427,
},
["1312967:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1312967,
},
["385518:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 385518,
},
["1295224:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1295224,
},
["1236747:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1236747,
},
["1240222:player:applied:default:2"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1240222,
},
["1248985:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1248985,
},
["1283506:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1283506,
},
["SoundChannel"] = "Master",
["1232470:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1232470,
},
["267618:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 267618,
},
["1310498:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1310498,
},
["1296880:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1296880,
},
["CustomCategories"] = {
},
["1299508:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1299508,
},
["1264756:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1264756,
},
["UseDefaultRaidAuraSounds"] = false,
["1283485:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1283485,
},
["1296092:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1296092,
},
["1294933:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1294933,
},
["1285453:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1285453,
},
["1313393:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1313393,
},
["1242869:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1242869,
},
["1260203:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1260203,
},
["1253054:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1253054,
},
["1295886:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1295886,
},
["1281913:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1281913,
},
["1223958:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1223958,
},
["1281184:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1281184,
},
["1288297:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1288297,
},
["1290814:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1290814,
},
["1286837:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1286837,
},
["381862:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 381862,
},
["1280023:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1280023,
},
["1293979:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1293979,
},
["1240222:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1240222,
},
["466559:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 466559,
},
["1265426:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1265426,
},
["1260027:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1260027,
},
["470966:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 470966,
},
["1253541:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1253541,
},
["1311979:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1311979,
},
["1221639:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1221639,
},
["1297707:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1297707,
},
["1297111:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1297111,
},
["1282419:player:removed"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "removed",
["spellID"] = 1282419,
},
["1289109:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1289109,
},
["1246487:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1246487,
},
["472793:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 472793,
},
["1253709:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1253709,
},
["1249609:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1249609,
},
["474129:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 474129,
},
["CustomGroups"] = {
},
["1248697:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1248697,
},
["1298304:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1298304,
},
["1222088:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1222088,
},
["1297096:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1297096,
},
["1286837:player:removed"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "removed",
["spellID"] = 1286837,
},
["1282911:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1282911,
},
["1248721:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1248721,
},
["1262772:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1262772,
},
["1249265:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1249265,
},
["1305621:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1305621,
},
["1294994:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1294994,
},
["1255612:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1255612,
},
["1270497:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1270497,
},
["1218468:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1218468,
},
["474545:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 474545,
},
["1311981:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1311981,
},
["1264453:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1264453,
},
["1239825:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1239825,
},
["1284527:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1284527,
},
["1259861:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1259861,
},
["1279512:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1279512,
},
["1297625:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1297625,
},
["1306666:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1306666,
},
["1283236:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1283236,
},
["1282419:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1282419,
},
["1218187:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1218187,
},
["270927:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 270927,
},
["372865:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 372865,
},
["1310881:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1310881,
},
["1225792:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1225792,
},
["1237623:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1237623,
},
["1285425:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1285425,
},
["1254113:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1254113,
},
["1308865:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1308865,
},
["1283247:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1283247,
},
["1305163:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1305163,
},
["153954:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 153954,
},
["1310309:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1310309,
},
["1290030:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1290030,
},
["1257087:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1257087,
},
["1253511:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1253511,
},
["1224299:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1224299,
},
["1222098:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1222098,
},
["1293046:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1293046,
},
["1233602:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1233602,
},
["1249478:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1249478,
},
["1285510:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1285510,
},
["1288297:player:removed"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "removed",
["spellID"] = 1288297,
},
["1283069:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1283069,
},
["1225011:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1225011,
},
["1218467:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1218467,
},
["1217973:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1217973,
},
["1286901:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1286901,
},
["1261286:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1261286,
},
["1252733:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1252733,
},
["1248994:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1248994,
},
["1297876:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1297876,
},
["UseDefaultDungeonAuraSounds"] = false,
["1251785:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1251785,
},
["1218465:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1218465,
},
["1215897:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1215897,
},
["1258901:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1258901,
},
["1297435:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1297435,
},
["1306736:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1306736,
},
["1284984:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1284984,
},
["1263983:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1263983,
},
["1261276:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1261276,
},
["1285911:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1285911,
},
["1238368:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1238368,
},
["1299266:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1299266,
},
["1237091:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1237091,
},
["1299899:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1299899,
},
["1268992:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1268992,
},
["373693:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 373693,
},
["1218466:player:applied"] = {
["edited"] = false,
["unit"] = "player",
["eventType"] = "applied",
["spellID"] = 1218466,
},
},
}
NSRTTimelineData = {
}
